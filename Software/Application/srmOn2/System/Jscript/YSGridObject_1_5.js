...
function YSGridObject(name){
	...
	this.GetTopRowLevel = YSGridObject.GetTopRowLevel;
	this.HideFocus = YSGridObject.HideFocus;
	this.ShowFocus = YSGridObject.ShowFocus;
	this.SetFocusedRowBorder = YSGridObject.SetFocusedRowBorder;
	this.RestoreFocusedRowBorder = YSGridObject.RestoreFocusedRowBorder;
	...
	this.ProcessExpBtn = YSGridObject.ProcessExpBtn;
	this.ProcessExpTR = YSGridObject.ProcessExpTR;
	this.GetRowLevel = YSGridObject.GetRowLevel;	
	this.FillExpandedInput = YSGridObject.FillExpandedInput;		
	this.SetChildrenVisible = YSGridObject.SetChildrenVisible;			
}
...
YSGridObject.RestoreFocusedRowBorder = function(tr, rowLevel, topBorder, saveLeftRight, savingArray){	
...
YSGridObject.SetFocusedRowBorder = function(tr, rowLevel, topBorder, saveLeftRight, savingArray){	
	if(rowLevel < this.groupColumns.length && rowLevel != -2){
		var offset = 1;
		if(rowLevel != -1){
			while(this.visibleColumns[this.visibleColumns.length - offset].type == "RowBtnColumn")
...
YSGridObject.GetTopRowLevel = function(topBorderTR){
...
YSGridObject.SetButtonExpanded = function(td){
	var img = td.firstChild.rows[0].cells[0].firstChild;
	if(img != null && img.tagName == "IMG")
		img.src = this.imgEBCollapse;
}

YSGridObject.SetButtonCollapsed = function(td){
	var img = td.firstChild.rows[0].cells[0].firstChild;
	if(img != null && img.tagName == "IMG")
		img.src = this.imgEBExpand;
}

YSGridObject.SetChildrenVisible = function(tr){
...
YSGridObject.ProcessExpBtn = function(button){	
...
YSGridObject.ProcessExpTR = function(tr){	
	var rowLevel = this.GetRowLevel(tr);
	this.ProcessExpBtn(tr.childNodes[rowLevel + this.firstGroupIndentOffset]);
}

YSGridObject.FillExpandedInput = function(rowsTable){	
...
YSGridCollection.Clear = function (){
...
function YSGridCollection(){
	...
	this.Clear = YSGridCollection.Clear;
}
