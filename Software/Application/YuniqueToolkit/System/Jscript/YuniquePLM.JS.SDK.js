...
Yunique.Auth.getYuniqueAPI = function() {
    ...
    if(Yunique.Data.supportsStorage()) {
        ...
        try {
            API = JSON.parse(window.sessionStorage.getItem('Yunique.API'))
        }
        catch(err) {
    ...
    return API || { 'REST': '/YuniqueAPI.REST', 'Toolkit': '/Yunique.Toolkit.API' };
};
Yunique.Auth.getYuniqueUser = function() {
    ...
    if(Yunique.Data.supportsStorage()) {
        ...
        user = JSON.parse(window.sessionStorage.getItem('Yunique.User'));
    }
    ...
    try {
        ...
        result = {
            ...
            '1': {
                ...
                '2': { 'alias': 'CreatedBy', 'value': data.CUser + '  ' + data.CDate },
                '3': { 'alias': 'ModifiedBy', 'value': data.MUser + '  ' + data.MDate },
...
Yunique.Data.schemaParse = function(format, schema, data, lookups) {
...
//    }
    if(format.toLowerCase() === 'header') {
        ...
        return result;
    } else if(format.toLowerCase() === 'form') {
        var columnsCnt = 0;
        $.each(columns, function(i) {
            var dataFieldValue = data[this['@name']] || '',
                columnNum,
            ...
            try {
                if(this['@datatype'] === 'datetime') {
//                    dataFieldValue = dataFieldValue.split(' ')[0].replace(/\./g, '/');
                    if(this['@type'] === 'calendarbox') {

                        //----------------------------------------------------
                        //TODO: data format issue
                        //temporary, FIX IT!!!
                        var fieldSplit = dataFieldValue.split('/'),
                            buf = fieldSplit[0];
                        fieldSplit[0] = fieldSplit[1];
                        fieldSplit[1] = buf;
                        dataFieldValue = fieldSplit.join('/');
                        //----------------------------------------------------

                        dataFieldValue = new Date(dataFieldValue);
                    }
                }
            } catch(e) {}
            // Money
            try {
                if(this['@datatype'] === 'money') {
                    var afterPoint = 2;
                    if(this['@dataformat']) {
                        var format = this['@dataformat'].split('.');
                        afterPoint = format.length > 1 ? format[1].length : 2;
                    }
                    dataFieldValue = parseFloat(dataFieldValue).toFixed(afterPoint);
                }
            } catch(e) {}
            try {
                if(this['@datatype'] === 'percent') {
                    ...
                    dataFieldValue = dataFieldValue * 100;
                    dataFieldValue = parseFloat(dataFieldValue).toFixed(afterPoint);
                }
            } catch(e) {}
            if(this['@type'] === 'checkbox') {
                dataFieldValue = dataFieldValue == 1 ? true : false;
            }
            if(this['@type'] === 'dropdownchecklist') {
                dataFieldValue = dataFieldValue ? dataFieldValue.split(',') : '';
            }
            columnsCnt = (columnsCnt < columnNum) ? columnNum : columnsCnt;
            if(!headerObj.data) {
                headerObj.data = [];
            }
            var row = {
            ...
            //Lookups
            try {
                if(this['@type'] === 'dropdownlist' || this['@type'] === 'dropdownchecklist') {
                    if(this.lookuptable && lookups) {
                        var lookupField = this.lookuptable['@parentidfield'];
                        row.lookups = [];
                        if(lookups[lookupField]) {
                            if(typeof lookups[lookupField][0] === 'object') {
                                row.lookups = lookups[lookupField];
                            } else {
                                row.lookups = [];
                                row.lookups.push(lookups[lookupField]);
                            }
                        }
                        if(!this['@addnullfield'] || this['@addnullfield'].toLowerCase() == 'true') {
                            if(row.lookups.length == 0) {
                                row.lookups[0] = {key:'', value:''};
                            } else {
                                row.lookups.unshift({key:'', value:''});
                            }
                        }
                    }
                }
            ...
            if(this['@linkedcol'] && this['@type'] === 'dropdownlist') {
                row.linkedCol = this['@linkedcol'];
//                row.type = 'dropdownlist_depend';
            }
            headerObj.data.push(row);
        });
        headerObj.columns = columnsCnt;
        headerObj.data.sort(function (a,b) {
        ...
        for(var i = 1; i <= headerObj.columns; i++){
            buf[i] = [];
            for(var j = 0; j < headerObj.data.length; j++) {
                if(headerObj.data[j].column == i) {
                    buf[i].push(headerObj.data[j]);
                }
            }
            buf[i].sort(function (a,b) {
                if (a.order < b.order)
                    return -1;
                if (a.order > b.order)
                    return 1;
            ...
            for(var k = 0; k < buf[i].length; k++) {
                dataSorted.push(buf[i][k]);
            }
        }
        headerObj.data = dataSorted;
...
Yunique.Data.convertToProperties = function(xml, sortable) {
    var properties = [],
        allAttributes = {},
        columnArray;
    try {
        xml = JSON.parse(xml);
        columnArray = xml['Table']['column'];
    } catch(e) {
        columnArray = xml;
    }
    if(sortable) {
        columnArray = columnArray.sort(function (a, b) {
            if (a['@PK'] || b['@PK']) {
                ...
                return parseInt(a['@order']) > parseInt(b['@order']) ? 1 : parseInt(a.getAttribute('order')) < parseInt(b['order']) ? -1 : 0;
            }
        });
    }
    for (var i = 0, l = columnArray.length; i < l; i++) {
        allAttributes = {};
        for (var prop in columnArray[i]) {
            var newProp = prop.replace('@', '').toLowerCase();
            allAttributes[newProp] = columnArray[i][prop];
        }
        properties.push(allAttributes);
    }
    return properties;
};
