<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="Material_WhereUsed.aspx.vb" Inherits="plmOnApp.Material_WhereUsed" %>
...
<body>
    ...
    <asp:Panel ID="pnlStyles" runat="server">                     
        ...
        <ItemTemplate>
            <table class="TableHeader" style="border: 1px solid #ccc;" cellspacing="0" cellpadding="0" width="100%">
                    <tr>
                        <td>
                            <cc1:BWImageButton ImageUrl='<%# GetImagePath("btn_goto.gif", Me.UserProperties.PrefLang) %>' NavigateUrl='<%# String.Format("Material_StyleGoTo.aspx?SID={0}&SYID={1}", Eval("StyleID").ToString, Eval("SeasonYearID").ToString) %>'
    ...
    <div id="style-dialog-message" style="display: none;"><p><%= GetSystemText("You are about to update the version of the material in ") & "{N}" & GetSystemText(" styles. Are you sure you want to continue")%>?</p></div>
    </asp:Panel>
    ...
    <script type="text/javascript">
        ...
        function RadGridLinePlans_ColumnHidden(sender, eventArgs) {
            var tableColumns = $find("RadGridLinePlans").get_masterTableView().get_columns();
            var hiddenColumns = new Array();
            for (var i = 0; i < tableColumns.length; i++) {
                if (tableColumns[i].get_visible() == false) {
                    hiddenColumns.push(tableColumns[i].get_uniqueName());
            ...
            var strHiddenColumns = hiddenColumns.join();
            PageMethods.SaveHiddenColumns('RadGridLinePlans', '<%= aspxPageName %>', strHiddenColumns, '<%= UserProperties.TeamID %>', '<%= UserProperties.Username %>');
        }

        function RadGridLinePlans_ColumnShown(sender, eventArgs) {
            var tableColumns = $find("RadGridLinePlans").get_masterTableView().get_columns();
            var hiddenColumns = new Array();
            for (var i = 0; i < tableColumns.length; i++) {
                if (tableColumns[i].get_visible() == false) {
