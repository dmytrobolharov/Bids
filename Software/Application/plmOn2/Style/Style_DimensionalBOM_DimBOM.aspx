...
<div id="fixed_icons"><a href="../Help/Help_Folder.aspx?Folder=<%= Folder %>&HID=<%= HelpID %>" title="Help" target="_blank" id="yHelp"></a></div>
    ...
    <telerik:RadWindowManager ID="RadWindowManager1" runat="server">
        <Windows>            
            <telerik:RadWindow ID="SortColorways" runat="server" Title="Sort Colorways"
                ReloadOnShow="true" ShowContentDuringLoad="false" NavigateUrl="Style_DimensionalBOM_BOMColorway_Sort.aspx"
                Behaviors="Close, Move, Resize, Maximize" IconUrl="../System/Icons/icon_table.gif" KeepInScreenBounds="true" Modal="true" 
                OnClientPageLoad="OnRadShown" OnClientClose="OnRadClosed" />

            <telerik:RadWindow ID="RemoveColorways" runat="server" Title="Remove Colorways"
    ...
    <script type="text/javascript">
        ...
        function showColorwaySort(strStyleBOMDimensionID, strDimLevel) {
            ...
            radWndWidth = 800;
            radWndHeight = 585;
            radWnd = window.radopen("Style_DimensionalBOM_BOMColorway_Sort.aspx?SBDID=" + strStyleBOMDimensionID + "&SDL=" + strDimLevel, "SortColorways");

            var lWnd = $find("<%= SortColorways.ClientID %>");
            lWnd.add_command(OnClientCommandL);
        }
        ...
        function showColorwayRemove(strStyleBOMDimensionID, strDimLevel) {
            ...
            radWndWidth = 800;
            radWndHeight = 525;
            radWnd = window.radopen('Style_DimensionalBOM_BOMColorway_Remove.aspx?SID=<%= Request("SID") %>&SN=<%= Request("SN") %>&WFIID=<%= Request("WFIID") %>&SBDID=' + strStyleBOMDimensionID + '&SDL=' + strDimLevel, 'RemoveColorways');

            var lWnd = $find("<%= RemoveColorways.ClientID %>");
            lWnd.add_command(OnClientCommandL);
        }
        ...
        function OnClientCommandL(sender, eventArgs) {
