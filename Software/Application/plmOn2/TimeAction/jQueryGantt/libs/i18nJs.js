...
var holidayArrayForInternal = [];
Yunique.Http.get('/TACalendarHoliday', {
    pagesize: 9999
})
    .then(function (resp) {
        var apiResp = resp.YuniqueAPI.pTACalHoliday,
            tempArrayOfDates = [],
            tempArrayOfNames = [];
        for (var i = 0, k = apiResp.length; i < k; i++) {
            var holidayInternal = new Date(apiResp[i].TACalHolidayDate).format('#yyyy_MM_dd#');
            tempArrayOfDates.push(new Date(apiResp[i].TACalHolidayDate).getTime());
            tempArrayOfNames.push({ key: new Date(apiResp[i].TACalHolidayDate).getTime(), value: apiResp[i].TACalHolidayName });
            holidayArrayForInternal.push(holidayInternal);
        }
        holidayArrayData.dates = tempArrayOfDates;
        holidayArrayData.holNames = tempArrayOfNames;
    });
