/****** Object:  StoredProcedure [dbo].[dpx_StyleFolder_WHERECount_SELECT]    Script Date: 04/10/2013 11:16:18 ******/
...
BEGIN
							DECLARE @MainMaterials INT = 0
							DECLARE @BOMMainMaterials INT = 0
							/*
								SELECT COUNT(*) AS COUNT
							...
							*/
							...
							-- Selecting BOM Main Materials for all styles with BOM
							SELECT @BOMMainMaterials = COUNT(*) FROM pStyleBOM sbom
							INNER JOIN pStyleHeader sh ON sh.StyleID = sbom.StyleID
							INNER JOIN pStyleSeasonYear ssy ON ssy.StyleID = sh.StyleID AND ssy.SeasonYearID = @SeasonYearID
							WHERE sbom.MainMaterial = 1
							
							-- Selecting Main Materials for all styles without BOM
							SELECT @MainMaterials = COUNT(*) FROM pStyleMaterials sm
							INNER JOIN pStyleHeader sh ON sh.StyleID = sm.StyleID
							INNER JOIN pStyleSeasonYear ssy ON ssy.StyleID = sh.StyleID AND ssy.SeasonYearID = @SeasonYearID
							WHERE sm.MainMaterial = 1 AND NOT EXISTS(SELECT * FROM pStyleBOM WHERE StyleID = sh.StyleID)
							
							SET @MaterialCount = @BOMMainMaterials + @MainMaterials
END
...
VALUES ('DB_Version', '0.5.0000', '05521', GetDate())
GO
