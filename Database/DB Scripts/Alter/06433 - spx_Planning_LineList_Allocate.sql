...
BEGIN
	...
	SELECT DISTINCT @ConfigLevel = (CASE WHEN StyleCategoryID IS NOT NULL THEN 3 WHEN StyleTypeID IS NOT NULL THEN 2 WHEN DivisionID IS NOT NULL THEN 1 ELSE 0 END) FROM pPlanningLineListConfig WHERE PlanningID = @PlanningID
	...
	IF @ConfigLevel = 1
		...
		STUFF((SELECT ', ' + CAST(stype.StyleTypeID AS NVARCHAR(MAX))
			...
			WHERE pPlanningDivision.PlanningID = @PlanningID) stype
	...
	FROM pStyleHeader sh INNER JOIN pPlanningItem pli ON sh.StyleID = pli.StyleID
	...
	BEGIN
		...
		SELECT IDENTITY(int, 1, 1) AS rowid, tas.PlanningItemID
		INTO #StyleTable
...
VALUES ('DB_Version', '0.5.0000', '06433', GetDate())
GO
