...
SELECT     dbo.pSourcingStyle.SourcingStyleID, dbo.pSourcingStyle.SourcingHeaderID, dbo.pSourcingStyle.StyleID, dbo.pSourcingStyle.Styleset, dbo.pSourcingStyle.Progress, 
                      ...
                      ISNULL(dbo.pStyleDevelopmentItem.StyleDevelopmentName, 'Variation ' + CAST(dbo.pStyleDevelopmentItem.Variation AS nVarchar(2))) AS Variation, 
                      dbo.pStyleHeader.DivisionID, dbo.pSourcingHeader.Season as SourcingSeason, dbo.pSourcingHeader.Year as SourcingYear, 
                          (SELECT     COUNT(*) AS Expr1
                            FROM          dbo.pSampleRequestBOMTrade AS samples INNER JOIN
                                                   dbo.pStyleSeasonYear AS ssy ON samples.StyleSeasonYearID = ssy.StyleSeasonYearID INNER JOIN
                                                   dbo.pSourcingHeader AS sourcing ON sourcing.seasonYearId = ssy.SeasonYearID AND 
                                                   sourcing.SourcingHeaderID = dbo.pSourcingStyle.SourcingHeaderID
                            WHERE      (samples.StyleID = dbo.pSourcingStyle.StyleID)) AS SampleRequestsCount, CASE WHEN EXISTS
                          (SELECT     *
                            FROM          dbo.pSampleRequestBOMTrade AS samples INNER JOIN
                                                   dbo.pStyleSeasonYear AS ssy ON samples.StyleSeasonYearID = ssy.StyleSeasonYearID INNER JOIN
                                                   dbo.pSourcingHeader AS sourcing ON sourcing.seasonYearId = ssy.SeasonYearID AND 
                                                   sourcing.SourcingHeaderID = dbo.pSourcingStyle.SourcingHeaderID
                            WHERE      samples.StyleID = dbo.pSourcingStyle.StyleID) THEN 1 ELSE 0 END AS HasSampleRequests,
                          (SELECT     COUNT(*) AS Expr1
                            FROM          dbo.pStyleQuoteItem AS quote
                            WHERE      (StyleID = dbo.pSourcingStyle.StyleID) AND (SourcingHeaderID = dbo.pSourcingStyle.SourcingHeaderID)) AS QuoteCount, CASE WHEN EXISTS
                          (SELECT     *
                            FROM          dbo.pStyleQuoteItem AS quote
                            WHERE      StyleID = pSourcingStyle.StyleID AND SourcingHeaderID = pSourcingStyle.SourcingHeaderID) THEN 1 ELSE 0 END AS HasQuotes,
...
FROM         dbo.pSourcingStyle INNER JOIN
                      ...
                      dbo.pStyleDevelopmentItem ON dbo.pStyleDevelopmentItem.StyleID = dbo.pStyleHeader.StyleID INNER JOIN
                      dbo.pSourcingHeader ON dbo.pSourcingStyle.SourcingHeaderID = dbo.pSourcingHeader.SourcingHeaderID LEFT JOIN
                      dbo.iCustom1 ON dbo.pStyleHeader.DivisionID = dbo.iCustom1.CustomID LEFT JOIN
                      dbo.pStyleType ON dbo.pStyleHeader.StyleType = dbo.pStyleType.StyleTypeID LEFT JOIN
                      dbo.pStyleCategory ON dbo.pStyleHeader.StyleCategory = dbo.pStyleCategory.StyleCategoryId
GO


INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
VALUES ('DB_Version', '0.5.0000', '08219', GetDate())
GO
