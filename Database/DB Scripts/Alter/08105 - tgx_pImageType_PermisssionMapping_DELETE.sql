/****** Object:  Trigger [tgx_pImageType_PermisssionMapping_DELETE]    Script Date: 06/26/2014 10:44:56 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tgx_pImageType_PermisssionMapping_DELETE]'))
DROP TRIGGER [dbo].[tgx_pImageType_PermisssionMapping_DELETE]
GO

/****** Object:  Trigger [dbo].[tgx_pImageType_PermisssionMapping_DELETE]    Script Date: 06/26/2014 10:44:57 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[tgx_pImageType_PermisssionMapping_DELETE]
   ON  [dbo].[pImageType]
   AFTER DELETE
AS 
BEGIN
	...
	BEGIN TRY
		...
		SELECT ImageTypeID FROM deleted
		...
		BEGIN
			...
			WHERE GuidProductTypeId = @DeletedProductTypeID AND PermissionFolderTypeId = @ImageFolderTypeID
...
VALUES ('DB_Version', '0.5.0000', '08105', GetDate())
GO