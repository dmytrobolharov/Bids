...
BEGIN
	...
	SELECT @TotalWSUnits = SUM(sku.TotalWSUnits),
		@TotalRUnits = SUM(sku.TotalRUnits),
		@TotalEUnits = SUM(sku.TotalEUnits)
	FROM pSourcingQuotationBOMDetails sqsb
	INNER JOIN pStyleSKUItems sku ON sku.StyleSKUItemID = @StyleSKUItemID
		AND (sqsb.ItemDim1Id = sku.ItemDim1Id OR sqsb.ItemDim1Id IS NULL AND sku.ItemDim1Id IS NULL)
		AND (sqsb.ItemDim2Id = sku.ItemDim2ID OR sqsb.ItemDim2Id IS NULL AND sku.ItemDim2ID IS NULL)
		AND (sqsb.ItemDim3Id = sku.ItemDim3Id OR sqsb.ItemDim3Id IS NULL AND sku.ItemDim3Id IS NULL)
	WHERE sqsb.StyleQuoteItemID = @StyleQuoteItemID 
...
VALUES ('DB_Version', '0.5.0000', '08650', GetDate())
GO
