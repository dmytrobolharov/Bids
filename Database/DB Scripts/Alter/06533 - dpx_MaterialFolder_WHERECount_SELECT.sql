...
CREATE PROCEDURE [dbo].[dpx_MaterialFolder_WHERECount_SELECT] (@SeasonYearID UNIQUEIDENTIFIER = NULL)
AS
	...
	SET @StyleCount = (
			...
			WHERE (pStyleSeasonYear.SeasonYearID = @SeasonYearID)
				AND pStyleHeader.StyleID IN (
					...
					UNION
					SELECT StyleID FROM pStyleBOM INNER JOIN dbo.pMaterialSeasonYear ON dbo.pStyleBOM.MaterialID = dbo.pMaterialSeasonYear.MaterialID
					WHERE (dbo.pMaterialSeasonYear.SeasonYearID = @SeasonYearID)
					)
			)
END
...
VALUES ('DB_Version', '0.5.0000', '06533', GetDate())
GO
