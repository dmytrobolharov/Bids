...
BEGIN
	...
	ELSE IF @nDeskFolderId  = 3  
		SELECT @nDeskFolderId AS DeskFolderID, b.LineListType AS  TypeDescription , 
		...
		FROM  sAccessLineFolder  a INNER JOIN pLineListType b ON a.LineTypeID = b.LineListTypeID
		INNER JOIN uUserGroup c ON c.TeamID = a.TeamId
		INNER JOIN uGroup d ON d.GroupID = c.GroupID
		WHERE d.Active = 1
		AND a.TeamId = @TeamID
		GROUP BY b.LineListType
	...
	*/
	...
	ELSE IF @nDeskFolderId  = 19  
		SELECT @nDeskFolderId AS DeskFolderID, b.SourcingTypeName AS TypeDescription, 
		max(a.AccessRoleId) as AccessRoleId, max(a.AccessView) as AccessView, max(a.AccessModify) as AccessModify, 0 AS AccessRemove, 
		max(a.AccessDelete) as AccessDelete, max(a.AccessPrint) as AccessPrint, max(a.AccessCreate) as AccessCreate
		FROM  sAccessSourcingFolder  a 
		INNER JOIN pSourcingType b ON b.SourcingTypeId = a.sourcingTypeId
		INNER JOIN uUserGroup c ON c.TeamID = a.TeamId
		INNER JOIN uGroup d ON d.GroupID = c.GroupID
		WHERE d.Active = 1 
		AND b.Active = 1
		AND a.TeamId = @TeamID
		GROUP BY b.SourcingTypeName
		
	ELSE IF @nDeskFolderId = 20
		SELECT @nDeskFolderId AS DeskFolderID, b.StyleTypeDescription AS TypeDescription, b.StyleTypeOrder, 
		max(a.AccessRoleId) as AccessRoleId, max(a.AccessView) as AccessView, max(a.AccessModify) as AccessModify, 0 AS AccessRemove, 
		max(a.AccessDelete) as AccessDelete, max(a.AccessPrint) as AccessPrint, max(a.AccessCreate) as AccessCreate
		FROM  sAccessOldSourcing  a INNER JOIN pStyleType b ON a.StyleTypeID = b.StyleTypeID
		INNER JOIN uUserGroup c ON c.TeamID = a.TeamId
		INNER JOIN uGroup d ON d.GroupID = c.GroupID
		WHERE d.Active = 1
		AND a.TeamId = @TeamID
	...
	ELSE IF @nDeskFolderId  = 21  
		SELECT @nDeskFolderId AS DeskFolderID, 'Partner Folder' AS  TypeDescription, 
		max(a.AccessRoleId) as AccessRoleId, max(a.AccessView) as AccessView, max(a.AccessModify) as AccessModify, 0 AS AccessRemove, 
		max(a.AccessDelete) as AccessDelete, max(a.AccessPrint) as AccessPrint, max(a.AccessCreate) as AccessCreate
		FROM  sAccessPartnerFolder  a 
		INNER JOIN uUserGroup c ON c.TeamID = a.TeamId
		INNER JOIN uGroup d ON d.GroupID = c.GroupID
		WHERE d.Active = 1
		AND a.TeamId = @TeamID
		
	ELSE IF @nDeskFolderId  = 22
		SELECT @nDeskFolderId AS DeskFolderID, 'Planning Folder' AS  TypeDescription,
		max(a.AccessRoleId) as AccessRoleId, max(a.AccessView) as AccessView, max(a.AccessModify) as AccessModify, 0 AS AccessRemove, 
		max(a.AccessDelete) as AccessDelete, max(a.AccessPrint) as AccessPrint, max(a.AccessCreate) as AccessCreate
		FROM sAccessPlanningFolder a 
		INNER JOIN uUserGroup c ON c.TeamID = a.TeamId
		INNER JOIN uGroup d ON d.GroupID = c.GroupID
		WHERE d.Active = 1
		AND a.TeamId = @TeamID		
		
	ELSE IF @nDeskFolderId  = 23
		SELECT @nDeskFolderId AS DeskFolderID, 'Calendar Folder' AS  TypeDescription,
		max(a.AccessRoleId) as AccessRoleId, max(a.AccessView) as AccessView, max(a.AccessModify) as AccessModify, 0 AS AccessRemove, 
		max(a.AccessDelete) as AccessDelete, max(a.AccessPrint) as AccessPrint, max(a.AccessCreate) as AccessCreate
		FROM sAccessCalendarFolder a 
		INNER JOIN uUserGroup c ON c.TeamID = a.TeamId
		INNER JOIN uGroup d ON d.GroupID = c.GroupID
		WHERE d.Active = 1
		AND a.TeamId = @TeamID
...
VALUES     ('DB_Version', '0.5.0000', '06658', GetDate())
GO
