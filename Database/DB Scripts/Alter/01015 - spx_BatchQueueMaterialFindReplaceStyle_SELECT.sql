...
/****** Object:  StoredProcedure [dbo].[spx_BatchQueueMaterialFindReplaceStyle_SELECT]    Script Date: 01/16/2011 18:06:47 ******/
IF  EXISTS (SELECT * FROM sysobjects WHERE id = OBJECT_ID(N'[dbo].[spx_BatchQueueMaterialFindReplaceStyle_SELECT]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
...
AS
	...
	SELECT  pStyleHeader.StyleID, pStyleHeader.StyleMasterID, pStyleHeader.StyleWorkflowID, pStyleHeader.StyleType, pStyleHeader.WorkflowType, 
		...
		pStyleHeader.PackagingNo, pStyleHeader.BodyID, pStyleHeader.CustomField31, pStyleHeader.CustomField32, pStyleHeader.CustomField33, 
		pStyleHeader.CustomField34, pStyleHeader.CustomField35, pStyleHeader.CustomField36, pStyleHeader.CustomField37, 
		pStyleHeader.CustomField38, pStyleHeader.CustomField39, pStyleHeader.CustomField40, pStyleSeasonYear.SeasonYearID, 
	...
	FROM         bBatchQueueMaterialReplaceStyle INNER JOIN
		pStyleHeader ON bBatchQueueMaterialReplaceStyle.StyleID = pStyleHeader.StyleID INNER JOIN
...
VALUES ('DB_Version', '4.0.0000', '01015', GetDate())
GO
