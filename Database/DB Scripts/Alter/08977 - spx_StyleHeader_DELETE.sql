/****** Object:  StoredProcedure [dbo].[spx_StyleHeader_DELETE]    Script Date: 08/22/2014 17:53:45 ******/
...
BEGIN
	...
	WHERE ReferenceId = @StyleID AND TACalReferenceTypeId = 2

	OPEN CalendarsCursor
	FETCH NEXT FROM CalendarsCursor INTO @TACalTemplateID
	WHILE @@FETCH_STATUS = 0
	BEGIN
		EXEC spx_TACalReferenceFromStyle_DELETE @TACalTemplateID, @StyleID
		FETCH NEXT FROM CalendarsCursor INTO @TACalTemplateID
	END
	...
	DELETE FROM pPlanningItem			WHERE StyleID = @StyleID
	DELETE FROM pStyleFeature			WHERE StyleID = @StyleID
	...
	DELETE FROM pSourcingStyle			WHERE StyleID = @StyleID
...
VALUES ('DB_Version', '6.0.0000', '08977', GetDate())
GO
