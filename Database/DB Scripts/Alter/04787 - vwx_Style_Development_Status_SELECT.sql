...
SELECT     ISNULL(g.GroupName, '') AS GroupName, dbo.fnx_GetStreamingImagePath(sh.DesignSketchID, sh.DesignSketchVersion) AS ImagePath, sh.StyleNo, sh.Description, 
                      sc.StyleCategory, sh.SizeClass, ISNULL(sh.CustomField3, '') AS StyleStatus, sh.DueDate, CAST(CASE WHEN sh.DueDate < GETDATE() THEN DATEDIFF(d, sh.DueDate,
                       GETDATE()) ELSE '' END AS NVARCHAR(10)) AS DaysLeft, ssy.StyleSeason, ssy.StyleYear, 
                       CASE 
						WHEN sh.OwnerGroup IS NULL THEN '00000000-0000-0000-0000-000000000000'
						WHEN sh.OwnerGroup = '' THEN '00000000-0000-0000-0000-000000000000'
						ELSE sh.OwnerGroup
						END AS OwnerGroup, sc.StyleCategoryId, sh.StyleType, ISNULL(sh.CustomField1, '') AS Division
...
VALUES     ('DB_Version', '5.0.0000', '04787', GetDate())
GO
