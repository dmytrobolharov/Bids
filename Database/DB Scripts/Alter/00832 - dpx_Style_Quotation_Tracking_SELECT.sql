...
--  Report File:	dash_Style_Quotation_Tracking_Body_LLT.rdl
...
ALTER                 PROCEDURE [dbo].[dpx_Style_Quotation_Tracking_SELECT]
(
...
-- Declare variables.
DECLARE @ReportImageFolder	varchar(200)
...
FROM     pStyleQuoteItem q 	WITH (NOLOCK)
		 ...
		 LEFT OUTER JOIN pStyleCostingDuty d WITH (NOLOCK)  ON d.DutyId = q.StyleQuotaDutyID
...
UPDATE s  set imageShowFlag = 1,
			  FilePath =	(SELECT TOP 1 dbo.fnx_GetStreamingImagePath(hImage.ImageID, hImage.ImageVersion)	--Comment #01
...
VALUES     ('DB_Version', '4.0.0000', '832', GetDate())
GO