...
BEGIN
	...
	IF @PlannigColor = 1
		...
		BEGIN
			SET @SqlStr = 'SELECT * FROM (SELECT cp.*, pc.PlanningID FROM vwx_ColorPalette_Grid_SEL cp INNER JOIN pPlanningColor pc ON cp.ColorPaletteID = pc.ColorPaletteID WHERE cp.SeasonYearID = ''' + @SeasonYearID + ''' AND PlanningColorDrop = 0) a WHERE Active = 1 AND PlanningID = ''' + @PlanningID + ''' ' + @WhereExp + ' ORDER BY Sort,L ,H ,S'
		END
		ELSE
		BEGIN
		...
		END
	ELSE
		SET @SqlStr = 'SELECT * FROM vwx_ColorPalette_Grid_SEL WHERE Active = 1 ' + @WhereExp + ' AND ColorIdentity IN (
			SELECT MIN(ColorIdentity) FROM vwx_ColorPalette_Grid_SEL WHERE Active = 1 ' + @WhereExp + '  GROUP BY ColorCode, ColorName) ORDER BY Sort,L ,H ,S'
...
VALUES     ('DB_Version', '6.0.0001', '09142', GetDate())
GO
