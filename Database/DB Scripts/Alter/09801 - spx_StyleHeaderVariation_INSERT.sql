IF OBJECT_ID(N'[dbo].[spx_StyleHeaderVariation_INSERT]') IS NOT NULL
...
BEGIN
	INSERT INTO pStyleHeader
		(StyleID, StyleMasterID, StyleCategory, SizeClassId, SizeClass, StyleType, WorkflowType, StyleNo, TempID, TempNo, CustomerNo, DevelopmentID, 
		DevelopmentNo, SpecNo, Description, SizeRangeId, SizeRange, StyleSet, DueDate, RecDate, Designer, StyleStatus, StyleLocation, WashType, Pitch, MaterialID, 
	...
	SELECT @NewStyleID AS NewStyleId, @StyleMasterID AS StyleMasterId, StyleCategory, SizeClassId, SizeClass, StyleType, WorkflowType, StyleNo, TempID, TempNo, 
		CustomerNo, DevelopmentID, DevelopmentNo, SpecNo, Description, SizeRangeId, SizeRange, StyleSet, DueDate, RecDate, Designer, StyleStatus, StyleLocation, 
...
BEGIN
	...
	-- insert just non-seasonal workflows
	-- seasonal workflows will be inserted along with seasons
	INSERT INTO dbo.pStyleWorkflow (
		...
		WorkComplete, WorkStatus, WorkStatusDate, WorkStatusBy, WorkVersion, WorkComments, WorkSort, CUser, CDate, MUser, MDate, ActualStart, ActualEnd)
	SELECT newid() AS StyleWorkflowID, @NewStyleID AS StyleID, StyleSet, WorkflowID, WorkflowType, WorkflowOrder, WorkDate, WorkStart, 
		...
		WorkSort, @CreatedBy AS CUser, @CreatedDate AS CDate, @CreatedBy AS MUser, @CreatedDate AS MDate, ActualStart, ActualEnd
	FROM dbo.pStyleWorkflow WITH (NOLOCK)
	LEFT JOIN Mapping m ON pStyleWorkflow.WorkflowID = m.Map
	WHERE StyleID = @StyleID
		AND m.IsSeasonal = 0
...
VALUES ('DB_Version', '6.1.0000', '09801', GetDate())
GO
