IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpx_StyleQuote_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[rpx_StyleQuote_SELECT]
GO

CREATE PROCEDURE [dbo].[rpx_StyleQuote_SELECT]
...
BEGIN
	...
	IF @Metric IS NOT NULL AND @Criteria IS NOT NULL AND @Parameter IS NOT NULL AND ISNUMERIC(@Parameter)=1
		BEGIN
			...
			SET @strSQL='SELECT * FROM #QuoteTMP WHERE ' + @Metric + @Criteria + @Parameter
			EXEC (@strSQL)
		END
	ELSE
...
VALUES ('DB_Version', '6.1.0000', '10189', GetUTCDate())
GO