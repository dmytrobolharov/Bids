...
BEGIN --01
	...
	SET @SQL = @SQL + '(SELECT f.sSystemFolder FROM sSystemPage p INNER JOIN sSystemFolder f ON p.sSystemFolderID=f.sSystemFolderID INNER JOIN sSystemHelp h ON h.sSystemPageID=p.sSystemPageID WHERE h.sSystemHelpID=sSystemHelpDetail.sSystemHelpID) AS Folder'
...
GO
	...
	VALUES ('DB_Version', '5.0.0000', '03363', GetDate())
