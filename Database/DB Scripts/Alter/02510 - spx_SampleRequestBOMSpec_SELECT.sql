...
/****** Object:  StoredProcedure [dbo].[spx_SampleRequestBOMSpec_SELECT]    Script Date: 12/23/2011 00:08:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_SampleRequestBOMSpec_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_SampleRequestBOMSpec_SELECT]
GO
...
CREATE  PROCEDURE [dbo].[spx_SampleRequestBOMSpec_SELECT] 
(
...
AS
BEGIN
	DECLARE @samplesize VARCHAR(10), 
			@sql VARCHAR(8000), @selectwhere VARCHAR(8000), 
			@delim VARCHAR(1), @select VARCHAR(8000), 
			@var VARCHAR(100), @ask VARCHAR(100), 
			@spec VARCHAR(100), @rev VARCHAR(100), 
			@fin VARCHAR(100), @pivot VARCHAR(100), 
			@pivotwhere VARCHAR(100), @table VARCHAR(100),
			@Select02 VARCHAR (8000) 
	...
	SELECT Submit AS tb_pivot INTO ##SampleSpec FROM pSampleRequestSpecItemBOM WITH (NOLOCK) WHERE 1=2

	INSERT INTO ##SampleSpec 
		SELECT DISTINCT Submit 
		FROM	pSampleRequestSpecItemBOM WITH (NOLOCK) 
		WHERE 
			Submit Is Not Null AND  StyleSet = @StyleSet AND StyleID = @StyleID 
	...
	SELECT 
		Sort, Submit, SampleRequestSpecID, 
		POM, PointMeasur, Ask, Spec, 
		Var, Rev, Final --=0  
	INTO ##SampleSummary
	FROM 
		pSampleRequestSpecItemBOM WITH (NOLOCK)  
	WHERE 
		StyleID =  @StyleID
	...
	*/		
		...
		BEGIN
			INSERT INTO #tb ( [Column], Alias, 
								Type, datatype, 
								dataformat, cellwidth, 
								GridWidth, GridHeaderCss ) 
			VALUES('Final '+ CAST(@LastSubmit AS NVARCHAR(3)), 'Final '+ CAST(@LastSubmit AS NVARCHAR(3)), 
					'label', 'decimal',
					'', 50, 
					50, 'TableHeader' )
		END 
		...
		SELECT 
			RIGHT( '0000'+ CAST(GridOrder AS NVARCHAR(3)), 4)  AS GridOrder, [Column], Alias, Type, 
...
IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='02510'))
BEGIN
	INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
	VALUES ('DB_Version', '4.1.0000', '02510', GetDate())
END
