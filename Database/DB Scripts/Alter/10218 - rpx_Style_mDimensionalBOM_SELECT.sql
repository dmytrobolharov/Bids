...
BEGIN
	...
	BEGIN
		...
		SET @SQLQUERY = @SQLQUERY + '	,dbo.fnx_GetStreamingImagePath(SB.MaterialImageID,SB.MaterialImageVersion, 2000) AS ImagePath'
		...
		SET @SQLQUERY = @SQLQUERY + '	,ct.ComponentOrder'		
		...
		SET @SQLQUERY = @SQLQUERY + '	LEFT JOIN pComponentType ct ON SB.MaterialType = ct.ComponentTypeID'
	...
	BEGIN
		...
		SET @SQLQUERY1 = @SQLQUERY1 + '	,dbo.fnx_GetStreamingImagePath(SB.MaterialImageID,SB.MaterialImageVersion, 2000) AS ImagePath'
		...
		SET @SQLQUERY1 = @SQLQUERY1 + '	,NULL'
		SET @SQLQUERY1 = @SQLQUERY1 + '	,ct.ComponentOrder'
		...
		SET @SQLQUERY1 = @SQLQUERY1 + '	LEFT JOIN pComponentType ct ON SB.MaterialType = ct.ComponentTypeID'
...
VALUES ('DB_Version', '6.1.0000', '10218', GetUTCDate())
GO
