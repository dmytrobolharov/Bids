IF OBJECT_ID(N'[dbo].[dpx_QuoteToDoStyle_SELECT]') IS NOT NULL
...
BEGIN
	...
	IF @QuoteStatusID IS NULL
		SELECT
			...
			, (CASE WHEN sqi.StyleQuoteItemDueDate<GETDATE() THEN DATEDIFF(d, sqi.StyleQuoteItemDueDate, GETDATE()) END) AS DaysLate
			, sh.StyleID
		FROM
			pStyleHeader sh
			...
			LEFT OUTER JOIN pSourcingCommitmentItem sci ON sh.StyleID = sci.StyleID AND sqi.StyleQuoteItemID = sci.StyleQuoteItemID
	...
	IF @QuoteStatusID = -1
		...
		FROM
			...
			INNER JOIN pSourcingTradePartner stp ON sqi.StyleQuoteTradePartnerID = stp.SourcingTradePartnerID
			INNER JOIN vwx_TradePartnerRelationshipLevels_SEL tprl ON stp.TradePartnerLevelID = tprl.TradePartnerRelationshipLevelID 			
			INNER JOIN pSourcingCommitmentItem sci ON sh.StyleID = sci.StyleID AND sqi.StyleQuoteItemID = sci.StyleQuoteItemID
			LEFT OUTER JOIN dbo.pStyleSeasonYear ssy ON sh.StyleID = ssy.StyleID
			LEFT OUTER JOIN dbo.pSeasonYear sy ON ssy.SeasonYearID = sy.SeasonYearID
			LEFT JOIN pSizeClass scc ON scc.CustomId = sh.SizeClassId
		WHERE
	...
	IF @QuoteStatusID IN (SELECT CustomKey FROM pStyleQuoteItemStatus)
		...
		FROM
			...
			LEFT JOIN pSizeClass scc ON scc.CustomId = sh.SizeClassId
		WHERE
			ssy.SeasonYearID = @SeasonYearID AND sci.SourcingCommitmentItemID IS NULL AND sqis.CustomKey = @QuoteStatusID
		GROUP BY
			sh.StyleNo
			, ISNULL(sdi.StyleDevelopmentName, sdi.Variation)
			, sh.Description
...
VALUES ('DB_Version', '6.1.0000', '09551', GetDate())
GO
