...
BEGIN
	...
	SELECT @LinkedCount = COUNT (*) FROM pWorkFlowItemLinked wfil INNER JOIN dbo.pStyleWorkflow a ON wfil.WorkFlowItemID = a.StyleWorkflowID WHERE wfil.StyleID = @StyleID AND wfil.StyleSet = @StyleSet AND a.WorkflowID = @WorkflowID
	...
	BEGIN
		SELECT b.Map, b.MapDetail, 
				...
				CAST(a.WorkSort as int) WorkSort, ISNULL(d.StyleNo,'') + ISNULL(' (' + d.Description + ')','') AS SourceStyle, 1 AS IsLinked
			FROM
		...
		BEGIN
			SELECT b.Map, b.MapDetail, 
				...
				a.CUser, a.CDate, a.MUser, a.MDate, a.WorkDay,
				d.StyleNo, a.WorkflowAccessTypeID, d.DevelopmentID, d.StyleType,
				CAST(a.WorkSort as int) WorkSort, ISNULL(d.StyleNo,'') + ISNULL(' (' + d.Description + ')','') AS SourceStyle, 0 AS IsLinked
			FROM
				dbo.pStyleWorkflow a
				INNER JOIN dbo.Mapping b ON a.WorkflowID = b.Map 
				INNER JOIN dbo.pWorkflowStatus c ON a.WorkStatus = c.WorkflowStatusID 
				INNER JOIN dbo.pStyleHeader d ON a.StyleID = d.StyleID
				INNER JOIN dbo.sAppSetting app ON LOWER(app.ConfigFileName) = 'server.config' AND LOWER(app.AppSettingKey) = 'HttpServer'
...
VALUES ('DB_Version', '0.5.0000', '05928', GetDate())
GO
