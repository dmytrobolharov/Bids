IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_Body_PatternSheetPiece_UPDATE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_Body_PatternSheetPiece_UPDATE]
GO
...
CREATE PROCEDURE  [dbo].[spx_Body_PatternSheetPiece_UPDATE]
(
...
INTO #tempTblPBody
FROM pBodyPatternSheetPiece where Model_Code = @Model_Code  AND BodyID = @BodyID AND StyleSet = @StyleSet
...
SELECT  @strCADsql = 'INSERT INTO #tempTBLpCAD
          ([Piece_Code]
           ,[Piece_Name]
           ,[Last_Updated]
           ,[IsActive]
...
SET @strSQL  = N'SELECT @CountRows = COUNT(*) FROM ' + REPLACE(REPLACE(@Catalog,'[',''),']','') + '.INFORMATION_SCHEMA.Columns'
...
SET @strCADsql = @strCADsql + '
 ...
 +  @CADDB + '.' + @Catalog + 
 '.dbo.Cad_Piece_Details WHERE  Model_Code = ' + Convert(varchar(50),@Model_Code) +'  and Piece_Code = ' + Convert(varchar(50), @Piece_Code )
...
Where pBodyPatternSheetPiece.Piece_Code = (select Piece_Code from #tempTblPBody where rownum = @RowCnt) AND BodyID = @BodyID AND StyleSet = @StyleSet
...
IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='02538'))
BEGIN
	INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
	VALUES     ('DB_Version', '4.2.0000', '02538', GetDate())
END
GO