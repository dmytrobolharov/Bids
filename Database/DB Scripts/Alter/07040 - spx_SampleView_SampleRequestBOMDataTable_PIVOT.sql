...
;WITH CommentTypes AS (
	SELECT * FROM (VALUES ('F', N'Fit Comments', 0),
						  ('S', N'Shared Comments', 1),
...
WorkFlowItemID, StyleBOMDimensionID, StyleBOMDimensionItemID, AssignedTo, StyleColorID, ItemDim1Id, ItemDim2Id, ItemDim3Id, Dim1Name, Dim2Name, Dim3Name,
...
SELECT 
	...
	+ 'title="' + vsy.SeasonYear + ' '
		+ CASE WHEN gt.DimTypeName like '%color%' THEN g.ItemDim1Name
			...
			WHEN jt.DimTypeName like '%color%' THEN j.ItemDim3Name ELSE '' END + ISNULL(scm.Comments, '') + '" >' 
	+
	CASE 
	...
	, srt.ItemDim2Id
	, srt.ItemDim3Id
	, g.ItemDim1Name AS Dim1Name
	, h.ItemDim2Name AS Dim2Name
	, j.ItemDim3Name AS Dim3Name
...
--	+ '''  border=0 ALT=''Open''><TD class=''font'' align=center valign=center nowrap>&nbsp; 
--	<a href=''#'' onclick="javascript:window.open(''../Sample/SampleRequest_Workflow_Redirect.aspx?SRTID=' + 
--	CAST ( srs.SampleRequestTradeID AS NVARCHAR(50)) + '&SWID=' + srs.SampleWorkflowID + '&SN='+  CAST(srs.StyleSet  as nvarchar(5)) +
--	''',''' +  
--	Replace( CAST ( srs.SampleRequestTradeID AS NVARCHAR(50)) , '-', '') +
--	''',''width=1024, height=700, location=yes, menubar=yes, status=yes, toolbar=yes, scrollbars=yes, resizable=yes'');">' 
--	+
...
--	, NULL as StyleBOMDimensionItemID
--	, srw.AssignedTo
--	, srt.StyleColorID
--	, NULL AS ItemDim1Id
--	, NULL AS ItemDim2Id
...
--	, sh.TechnicalDesigner
--	, sh.ProductionManager
--FROM dbo.pSampleRequestSubmit srs
--	INNER JOIN pSampleRequestTrade srt ON srs.SampleRequestTradeID  = srt.SampleRequestTradeID  
--	INNER JOIN pSampleWorkflow sw ON srs.SampleWorkflowId = sw.SampleWorkflowID 
--	INNER JOIN pSampleRequestSubmitStatus srss ON srs.Status = srss.StatusID	
--	INNER JOIN pSampleRequestWorkflow srw ON srw.SampleRequestWorkflowID = srs.SampleRequestWorkflowID AND srw.Submit = srs.Submit
--	INNER JOIN pStyleHeader sh ON sh.StyleID = srt.StyleID
--	INNER JOIN pStyleCategory sc ON sh.StyleCategory = sc.StyleCategoryId
--	INNER JOIN pStyleType st ON sh.StyleType = st.StyleTypeID
--	LEFT OUTER JOIN pStyleSeasonYear ssy ON srt.StyleSeasonyearID = ssy.StyleSeasonYearID
...
SET @SQL = 'SELECT SampleWorkflow, Link , SampleRequestTradeID, StyleID, DesignSketchID, DesignSketchVersion, TradePartnerID , TradePartnerName, 
StyleNo, Description, SizeClass, Dim1Name, Dim2Name, Dim3Name, TechPack, StyleSet, SetNo, StyleColorID, WorkFlowItemID, StyleBOMDimensionID, StyleBOMDimensionItemID,  
...
VALUES ('DB_Version', '0.5.0000', '07040', GetDate())
GO
