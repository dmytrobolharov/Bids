/****** Object:  StoredProcedure [dbo].[spx_LineFolder_LinePlanningAttribute_Full_SELECT]    Script Date: 07/07/2014 00:03:10 ******/
...
BEGIN
	...
	;WITH dsc as (
		...
		FROM vwx_Division_StyleType_StyleCategory_SELECT vw
			...
			INNER JOIN pLineFolderStyleType lfst ON (lfst.StyleTypeID = vw.StyleTypeID OR vw.StyleTypeID IS NULL) AND lfst.LineFolderID = @LineFolderID
			INNER JOIN pLineFolderStyleCategory lfsc ON (lfsc.StyleCategoryID = vw.StyleCategoryID OR vw.StyleCategoryID IS NULL) AND lfsc.LineFolderID = @LineFolderID
...
VALUES ('DB_Version', '0.5.0000', '08232', GetDate())
GO