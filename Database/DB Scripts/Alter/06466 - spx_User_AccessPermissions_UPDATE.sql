...
AS
	...
	INSERT INTO sAccessBatchQFolderItems (BatchQTypeId, BatchQueueID, AccessRoleId, AccessView, AccessCreate, AccessModify, AccessRemove, AccessDelete, AccessPrint, TeamId, CUser, CDate, MUser, MDate)  
	SELECT af.BatchQueueTypeID, bq.BatchQueueID, MAX(AccessRoleID), MAX(AccessView), MAX(AccessCreate), MAX(AccessModify), MAX(AccessRemove), MAX(AccessDelete), MAX(AccessPrint), @TeamID, @FullName, @CDate, @FullName, @CDate
	FROM sAccessGroupBatchQueueFolder af
	INNER JOIN pBatchQueue bq ON af.BatchQueueTypeID = bq.BatchQueueTypeID
	INNER JOIN uUserGroup ug ON af.GroupID = ug.GroupID 
	INNER JOIN uGroup g ON ug.GroupID = g.GroupID AND g.Active = 1
	INNER JOIN cDeskGroupFolderAccess dg ON ug.GroupID = dg.GroupID AND dg.DeskAccessId = 1
	WHERE ug.TeamID = @TeamID AND dg.DeskFolderID = @DeskFolderID
	GROUP BY af.BatchQueueTypeID, bq.BatchQueueID
...
VALUES ('DB_Version', '0.5.0000', '06466', GetDate())
GO
