...
BEGIN
	...
	'CASE	WHEN ''' + CAST(lfshdt.DimTypeID AS NVARCHAR(50))+ ''' = ItemDim1TypeID THEN ItemDim1Name 
			WHEN ''' + CAST(lfshdt.DimTypeID AS NVARCHAR(50))+ ''' = ItemDim2TypeID THEN ItemDim2Name 
			WHEN ''' + CAST(lfshdt.DimTypeID AS NVARCHAR(50))+ ''' = ItemDim3TypeID THEN ItemDim3Name 
	END AS [' + CAST(lfshdt.DimTypeID AS NVARCHAR(50)) + ']'
	...
	;WITH dims as (
		...
		AND StyleBOMDimensionID IN (SELECT StyleBOMDimensionID FROM pStyleSKUItem WHERE LineFolderSKUHeaderID = @LineFolderSKUHeaderID)
	)
...
VALUES ('DB_Version', '6.1.0000', '10191', GetUTCDate())
GO