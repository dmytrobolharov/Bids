/****** Object:  StoredProcedure [dbo].[spx_SampleRequestHistory_SELECT]    Script Date: 05/22/2013 10:34:17 ******/
...
AND d.StyleID = a.StyleID 
UNION
--NEW Partner MManagement
select  a.SampleRequestTradeID , a.StyleID ,  d.StyleNo ,  d.Description , a.CDate,   c.TradePartnerCode
FROM hSampleHistory  a WITH (NOLOCK) , pSampleRequestTrade b WITH (NOLOCK) ,  uTradePartner c WITH (NOLOCK)  , pStyleHeader d WITH (NOLOCK) 
where a.TeamID =  @TeamId
AND a.SampleRequestTradeID = b.SampleRequestTradeID 
...
UNION
select  a.SampleRequestTradeID , a.StyleID ,  d.StyleNo ,  d.Description , a.CDate,   c.TradePartnerCode
FROM hSampleHistory  a WITH (NOLOCK) , pSampleRequestBOMTrade b WITH (NOLOCK) ,  uTradePartner c WITH (NOLOCK)  , pStyleHeader d WITH (NOLOCK) 
where a.TeamID =  @TeamId
AND a.SampleRequestTradeID = b.SampleRequestTradeID 
AND b.TradepartnerID  =  c.TradePartnerID 
AND c.TradePartnerID = a.TeamID 
...
VALUES     ('DB_Version', '0.5.0000', '05724', GetDate())
GO
