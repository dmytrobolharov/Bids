...
BEGIN
	...
	SELECT
		...
		, ISNULL(REPLACE(CAST(srs.Comment AS NVARCHAR(MAX)), 'span', 'u'), '') AS FitComments
		, CONVERT(NVARCHAR(10),src.MDate, 103) AS MDate, src.MUser, src.SampleRequestComment
	FROM pSampleRequestSubmitBOM srs
		...
		LEFT JOIN pSampleRequestComment src ON srs.SampleRequestTradeID = src.SampleRequestTradeID
			AND srs.SampleRequestWorkflowID = src.SampleRequestWorkflowID
			AND srs.Submit = src.SampleSubmit
			AND srs.StyleSet = src.StyleSet AND src.SampleCommentType = 'S'
	...
	ORDER BY srs.Submit, src.MDate
END
GO

INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
VALUES ('DB_Version', '6.1.0000', '09876', GetDate())
GO
