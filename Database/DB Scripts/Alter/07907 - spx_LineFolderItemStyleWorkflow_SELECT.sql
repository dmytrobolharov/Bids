/****** Object:  StoredProcedure [dbo].[spx_LineFolderItemStyleWorkflow_SELECT]    Script Date: 06/05/2014 15:01:14 ******/
...
DECLARE @EnableOldCosting nvarchar(10)
...
FROM fnx_Permissions_GetIntProductTypePermissions(@TeamID, 12)
WHERE ProductTypeId = @StyleTypeId AND PermissionRoleId <> 0

SELECT @EnableOldCosting = AppSettingValue
FROM sAppSetting
WHERE AppSettingKey = 'ShowOldStyleCosting' AND
	ConfigFileName = @ConfigFile

SELECT @BeginSourcingSeason = AppSettingValue
FROM sAppSetting
WHERE AppSettingKey = 'BeginSourcingSeason' AND
	ConfigFileName = @ConfigFile
...
VALUES ('DB_Version', '0.5.0000', '07907', GetDate())
GO
