IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_MaterialRequestSubmitWorkflowColor_Logic_UPDATE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_MaterialRequestSubmitWorkflowColor_Logic_UPDATE]
GO



CREATE PROCEDURE [dbo].[spx_MaterialRequestSubmitWorkflowColor_Logic_UPDATE] (          
...
WHERE a.MaterialRequestWorkflowTempID = @MaterialRequestWorkflowTempID          
 AND b.MaterialRequestWorkflowColor = 1  
...
BEGIN          
 ...
 INSERT INTO pMaterialRequestSubmitWorkflow (MaterialTradePartnerID, MaterialTradePartnerColorID,           
  ...
  SubmitDays, ResubmitDays, AssignedTo,           
  StartDate, DueDate, EndDate, CDate, CUser, MDate, MUser,          
  MaterialRequestSubmitAllColors --- comment #03           
  ,MaterialRequestPartnerTypeID)
 ...
 SELECT a.MaterialTradePartnerId, b.MaterialTradePartnerColorID,           
  ...
  d.MaterialRequestWorkflowDays, d.MaterialRequestWorkflowRDays, d.MaterialRequestWorkflowAssignedTo,           
  @StartDate, 
  ISNULL(@DueDate, DATEADD(DAY,ISNULL(d.MaterialRequestWorkflowDays, 0), @StartDate)), --WI # 4662 - Check if due date is null then add startdate + MaterialRequestWorkflowDays                      
   @EndDate, @CDate, @CUser, @CDate, @CUser          
  , e.MaterialRequestWorkflowColor --- comment #03           
  , d.MaterialRequestPartnerTypeID
 ...
 FROM pMaterialTradePartner a WITH (NOLOCK)           
  ...
  LEFT OUTER JOIN pMaterialRequestSubmitWorkflow f WITH (NOLOCK)  ON f.MaterialRequestWorkflowID = d.MaterialRequestWorkflowID           
  AND f.MaterialTradePartnerColorID = @MaterialTradePartnerColorID           
 WHERE b.MaterialColorID = @MaterialColorID AND b.MaterialSizeID = @MaterialSizeID           
  AND  a.MaterialTradePartnerID = @MaterialTradePartnerID           
  AND f.MaterialRequestWorkflowID IS NULL           
 ...
 FROM pMaterialRequestSubmitWorkflow a  WITH (NOLOCK)           
  ...
  INNER JOIN pMaterialTradePartnerColor c WITH (NOLOCK)  ON b.MaterialTradePartnerId = c.MaterialTradePartnerID           
  ...
  LEFT OUTER JOIN pMaterialRequestSubmit f WITH (NOLOCK)  ON f.MaterialRequestSubmitWorkflowID = a.MaterialRequestSubmitWorkflowID          
   AND f.MaterialTradePartnerColorID = @MaterialTradePartnerColorID          
 WHERE c.MaterialColorID = @MaterialColorID   
  AND c.MaterialSizeID = @MaterialSizeID           
...
--** Update DueDate in Samplerequest          
UPDATE pMaterialRequestSubmitWorkflow           
SET DueDate = DATEADD( day, ISNULL(pMaterialRequestWorkflowTemplateItem.MaterialRequestWorkflowDays,0), pMaterialTradePartner.MaterialrequestWorkflowStartDate )          
...
VALUES     ('DB_Version', '0.5.0000', '08364', GetDate())
GO
