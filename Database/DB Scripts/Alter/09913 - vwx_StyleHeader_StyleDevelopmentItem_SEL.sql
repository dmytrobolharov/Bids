IF OBJECT_ID(N'[dbo].[vwx_StyleHeader_StyleDevelopmentItem_SEL]') IS NOT NULL
...
SELECT sh.StyleID, sh.StyleMasterID, sh.StyleWorkflowID, sh.StyleType, sh.WorkflowType, sh.RefNo, sh.StyleNo, sh.TempID, 
	   ...
	   sh.StyleSet, sh.TechPackId, sh.TechPackDate, sh.DueDate, sh.StartDate, 
       CASE sh.PlannedDueDate WHEN NULL THEN wti.DueDate ELSE sh.PlannedDueDate END AS PlannedDueDate, 
       CASE sh.PlannedStartDate WHEN NULL THEN wti.StartDate ELSE sh.PlannedStartDate END AS PlannedStartDate, 
	   ...
	   sh.CustomField18, sh.CustomField19, sh.CustomField20, sh.CustomField21, sh.CustomField22, sh.CustomField23, sh.CustomField24, 
	   sh.CustomField25, sh.CustomField26, sh.CustomField27, sh.CustomField28, sh.CustomField29, sh.CustomField30, sh.PackagingNo, 
       sh.StyleCopyID, sh.LinePlanItemID, sh.StyleStatusID, sh.BodyID, sh.TradePartnerID, sh.TradePartnerVendorID, sh.StyleLinkID, 
	   sh.MaterialCoreID, sh.ProdDev, sh.HeaderLocked, sh.CustomField31, sh.CustomField32, sh.CustomField33, sh.CustomField34, 
	   sh.CustomField35, sh.CustomField36, sh.CustomField37, sh.CustomField38, sh.CustomField39, sh.CustomField40, sh.Distribution, 
       sh.IntroSeasonYearID, sh.OwnerGroup, sh.UserCID, sh.UserMID, sh.DivisionID, sh.StyleThemeID, stt.IsSilhouette, 
...
LEFT JOIN pStyleWorkflow sw ON sw.StyleWorkflowID = sh.StyleWorkflowID 
LEFT JOIN pWorkflowTemplateItem wti ON wti.WorkflowID = sw.WorkflowID 
...
VALUES ('DB_Version', '6.1.0000', '09913', GetDate())
GO
