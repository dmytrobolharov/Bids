/****** Object:  StoredProcedure [dbo].[spx_TradePartner_Style_Quote_SELECT]    Script Date: 02/11/2013 13:51:25 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_TradePartner_Style_Quote_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_TradePartner_Style_Quote_SELECT]
GO
...
CREATE PROCEDURE [dbo].[spx_TradePartner_Style_Quote_SELECT]
...
BEGIN
	...
	INNER JOIN pStyleQuoteItemShare qis ON pStyleQuoteItem.StyleQuoteItemID = qis.StyleQouteItemID AND qis.TradePartnerID = @TradePartnerID
	...
	--dbo.pStyleQuoteItemFreightCost.Margin, dbo.pStyleQuoteItemFreightCost.TotalLandCost,
	dbo.pStyleHeader.Description, dbo.pStyleQuoteItem.StyleQuoteItemDueDate, dbo.pStyleQuoteItem.MDate, 
	dbo.pStyleQuoteItem.StyleColorID, NULL, dbo.pStyleQuoteItem.StyleSourcingID,  pStyleQuoteItem.SourcingHeaderID,
	dbo.pStyleQuoteItem.StyleQuoteItemCustomField31, dbo.pStyleQuoteItem.StyleSeasonYearID, dbo.pStyleSeasonYear.SeasonYearID, 
	dbo.pStyleSeasonYear.StyleSeason + ' ' + dbo.pStyleSeasonYear.StyleYear AS StyleSeasonYear, 
	pStyleQuoteItem.TradePartnerID, uTradePartner.TradePartnerName, pStyleQuoteItem.TradePartnerVendorID, uTradePartnerVendor.VendorName, NULL, NULL,
	NULL as PartnerCost,
	NULL as PartnerExtendedCost,
	NULL as AdditionalCost,
	NULL as VariationName
	FROM pStyleQuoteItem 
	LEFT OUTER JOIN dbo.pStyleQuoteItemStatus ON pStyleQuoteItem.StyleQuoteItemStatusId = pStyleQuoteItemStatus.CustomKey
	LEFT JOIN pStyleHeader ON pStyleQuoteItem.StyleID = pStyleHeader.StyleID
	LEFT OUTER JOIN dbo.pStyleSeasonYear ON dbo.pStyleSeasonYear.StyleSeasonYearID = pStyleQuoteItem.StyleSeasonYearID --dbo.pStyleHeader.StyleID = dbo.pStyleSeasonYear.StyleID
	LEFT OUTER JOIN dbo.pStyleCategory ON dbo.pStyleHeader.StyleCategory = dbo.pStyleCategory.StyleCategoryId
	INNER JOIN uTradePartner ON pStyleQuoteItem.TradePartnerID = uTradePartner.TradePartnerID
	LEFT JOIN uTradePartnerVendor ON pStyleQuoteItem.TradePartnerVendorID = uTradePartnerVendor.TradePartnerVendorID
	WHERE pStyleQuoteItem.TradePartnerID = @TradePartnerID
...
VALUES ('DB_Version', '5.0.0000', '05016', GetDate())
GO
