/****** Object:  StoredProcedure [dbo].[spx_Group_DesktopAccessFolder_UPDATE]    Script Date: 02/14/2013 15:21:43 ******/
...
BEGIN       
 ...
 END   
 ELSE IF @DeskFolderId = 13
 BEGIN
	DELETE FROM sAccessGroupLinePlanFolder WHERE GroupID = @GroupID    
	DELETE FROM sAccessLinePlanFolder WHERE TeamId IN ( SELECT DISTINCT TeamID from uUserGroup where GroupID=  @GroupID )      
 END   
...
ELSE IF @DeskFolderId  = 17     
 BEGIN 
  ...
  --DELETE FROM sAccessStyleFolder where TeamID IN  ( SELECT DISTINCT TeamID from uUserGroup where GroupID=  @GroupID ) 
  END
 ELSE IF @DeskFolderId = 18
 BEGIN
	DELETE FROM sAccessBatchQFolder WHERE TeamID IN  ( SELECT DISTINCT TeamID from uUserGroup where GroupID=  @GroupID ) 
	DELETE FROM sAccessBatchQFolderItems WHERE GroupID = @GroupID
	DELETE FROM sAccessBatchQItems WHERE GroupID = @GroupID
 END
 ...
 BEGIN 
  ...
  END
  ELSE IF @DeskFolderId  = 20
 BEGIN 
  DELETE FROM sAccessGroupOldSourcing where GroupID=  @GroupID       
  DELETE FROM sAccessOldSourcing where TeamID IN  ( SELECT DISTINCT TeamID from uUserGroup where GroupID=  @GroupID )          
  END  
END  
...
VALUES ('DB_Version', '5.0.0000', '05046', GetDate())
GO
