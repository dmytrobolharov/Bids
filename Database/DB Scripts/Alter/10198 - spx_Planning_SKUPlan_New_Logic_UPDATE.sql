...
GO
/****** Object:  StoredProcedure [dbo].[spx_Planning_SKUPlan_New_Logic_UPDATE]    Script Date: 03/19/2015 11:55:18 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	DECLARE @Sort INT
	SELECT @Sort = ISNULL(MAX(CAST(psh2.Sort AS INT)), 0) + 1 
	FROM pPlanningSKUHeader psh	
	INNER JOIN pPlanningSKUHeader psh2 ON psh.PlanningID = psh2.PlanningID AND psh.SeasonYearID = psh2.SeasonYearID
	WHERE psh.PlanningSKUHeaderID = @PlanningSKUHeaderID AND psh2.PlanningSKUHeaderID <> @PlanningSKUHeaderID AND ISNUMERIC(psh2.Sort) = 1
	
	UPDATE pPlanningSKUHeader SET WorkStatus = 6, Sort = RIGHT('0000' + CAST(@Sort AS nVARCHAR(5)), 3) WHERE PlanningSKUHeaderID = @PlanningSKUHeaderID	
	...
	BEGIN
		...
		BEGIN		
			...
			UPDATE pWorkFlowItem SET WorkStatus = 6 WHERE WorkflowItemID = @WorkFlowItemId	
...
VALUES ('DB_Version', '6.1.0000', '10198', GetUTCDate())
GO