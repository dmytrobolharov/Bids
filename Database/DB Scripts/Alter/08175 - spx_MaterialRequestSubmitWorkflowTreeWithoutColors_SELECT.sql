/****** Object:  StoredProcedure [dbo].[spx_MaterialRequestSubmitWorkflowTreeWithoutColors_SELECT]    Script Date: 07/01/2014 17:58:57 ******/
...
BEGIN
	...
	BEGIN -- for SrmON
		...
		FROM (
			SELECT DISTINCT a.MaterialTradePartnerColorID, b.MaterialRequestWorkflowID, b.MaterialRequestWorkflowTempItemID
			...
			LEFT JOIN dbo.pMaterialRequestWorkflowTemplateItem f WITH(NOLOCK) ON f.MaterialRequestWorkflowTempItemID = foo.MaterialRequestWorkflowTempItemID
			LEFT JOIN dbo.uTradePartnerType pt WITH(NOLOCK) ON d.MaterialRequestPartnerTypeID = pt.CustomId
		ORDER BY  f.MaterialRequestWorkflowSort
	END
	ELSE
	BEGIN -- for PlmON
		...
		FROM (
			...
			LEFT JOIN dbo.pMaterialRequestWorkflowTemplateItem f WITH(NOLOCK) ON f.MaterialRequestWorkflowTempItemID = foo.MaterialRequestWorkflowTempItemID
			LEFT JOIN dbo.uTradePartnerType pt WITH(NOLOCK) ON d.MaterialRequestPartnerTypeID = pt.CustomId
			INNER JOIN fnx_Permissions_GetIntProductType_WorkflowPermissions(@TeamID, 24, @MaterialTypeID) access ON access.WorkflowId = e.MaterialRequestWorkflowGUIDID 
...
BEGIN
	...
	FROM (
		...
		INNER JOIN dbo.pMaterialRequestSubmitStatus c WITH(NOLOCK)  ON c.StatusID = d.Status
		INNER JOIN dbo.pMaterialRequestWorkflow e WITH(NOLOCK) ON e.MaterialRequestWorkflowID = foo.MaterialRequestWorkflowID
		LEFT JOIN dbo.pMaterialRequestWorkflowTemplateItem f ON f.MaterialRequestWorkflowTempItemID = foo.MaterialRequestWorkflowTempItemID
		LEFT JOIN dbo.uTradePartnerType pt WITH(NOLOCK) ON d.MaterialRequestPartnerTypeID = pt.CustomId
		INNER JOIN dbo.pMaterialRequestSubmitWorkflowShare share ON share.MaterialRequestSubmitWorkflowID = d.MaterialRequestSubmitWorkflowID
...
VALUES ('DB_Version', '0.5.0000', '08175', GetDate())
GO
