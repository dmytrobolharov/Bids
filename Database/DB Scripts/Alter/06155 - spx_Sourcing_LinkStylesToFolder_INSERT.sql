...
BEGIN
    ...
    SELECT @DivisionID = Division, @SeasonYearID = seasonYearId, --@StyleCategoryID = StyleCategoryID, @Season = Season, @Year = [Year],
...
if NOT EXISTS (SELECT * FROM pSourcingHeaderToStyleCategory WHERE SourcingHeaderID = @SourcingHeaderID)
	begin
	...
	AND a.StyleType IN (
		SELECT DISTINCT st.StyleTypeid
		FROM pSourcingHeaderToStyleType shtst
			INNER JOIN pStyleType st ON shtst.StyleTypeID = st.StyleTypeID
			INNER JOIN pStyleTypeTemplate stt ON st.StyleTypeTemplateID = stt.StyleTypeTemplateID
		WHERE StyleTypeTemplateName = 'Style' AND SourcingHeaderID = @SourcingHeaderID)
	OPEN @StyleCursor
	...
	AND a.StyleType IN (
		...
		FROM pSourcingHeaderToStyleType shtst
			...
			INNER JOIN pStyleTypeTemplate stt ON st.StyleTypeTemplateID = stt.StyleTypeTemplateID
		WHERE StyleTypeTemplateName = 'Style' AND SourcingHeaderID = @SourcingHeaderID)
	AND a.StyleCategory IN (SELECT StyleCategoryID FROM pSourcingHeaderToStyleCategory WHERE SourcingHeaderID = @SourcingHeaderID)
	OPEN @StyleCursor
	FETCH NEXT FROM @StyleCursor INTO @CurrentStyleID
...
VALUES ('DB_Version', '0.5.0000', '06155', GetDate())
GO
