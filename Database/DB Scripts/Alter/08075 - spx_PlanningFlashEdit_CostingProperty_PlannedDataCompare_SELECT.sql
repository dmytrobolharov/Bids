IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_PlanningFlashEdit_CostingProperty_PlannedDataCompare_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_PlanningFlashEdit_CostingProperty_PlannedDataCompare_SELECT]
GO

CREATE PROCEDURE [dbo].[spx_PlanningFlashEdit_CostingProperty_PlannedDataCompare_SELECT]
...
;WITH StyleCostSum AS (
	...
	FROM pPlanningItem pli
		...
		WHERE pli.StyleID = @StyleID AND pli.PlanningID = @PlanningID AND ssy.SeasonYearID = @SeasonYearID
), PlannedData AS (
...
SELECT @StyleID as StyleID
	,PlannedData.* 
	,(Planned_WSUnits - WSUnits) as DeltaWSUnits 
	,(Planned_RUnits - RTUnits) as DeltaRUnits 
	,(Planned_EUnits - EUnits) as DeltaEUnits 
FROM PlannedData 
...
VALUES ('DB_Version', '0.5.0000', '08075', GetDate())
GO
