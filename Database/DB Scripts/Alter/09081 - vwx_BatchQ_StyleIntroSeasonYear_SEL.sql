IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vwx_BatchQ_StyleIntroSeasonYear_SEL]'))
...
AS
SELECT DISTINCT 
                      TOP (100) PERCENT dbo.pStyleHeader.StyleID, dbo.pStyleHeader.StyleNo, dbo.pStyleHeader.Description, 
                      ...
                      dbo.pSeasonYear.Year, dbo.pSeasonYear.SeasonYearID,
                      dbo.pStyleType.StyleTypeRepId, dbo.pStyleHeader.DivisionID AS CustomID
                      --, dbo.pLinePlanStyleAttributeItem.AttributeName
FROM         dbo.pStyleHeader(NOLOCK) 
	INNER JOIN dbo.pStyleType ON dbo.pStyleHeader.StyleType = dbo.pStyleType.StyleTypeID
					  INNER JOIN dbo.pSeasonYear(NOLOCK) ON pStyleHeader.IntroSeasonYearID = dbo.pSeasonYear.SeasonYearID
...
VALUES   ('DB_Version', '6.0.0000', '09081', GetDate())
GO