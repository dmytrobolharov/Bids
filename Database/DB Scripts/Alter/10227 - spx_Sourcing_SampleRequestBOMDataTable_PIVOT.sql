IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_Sourcing_SampleRequestBOMDataTable_PIVOT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_Sourcing_SampleRequestBOMDataTable_PIVOT]
GO



CREATE PROCEDURE [dbo].[spx_Sourcing_SampleRequestBOMDataTable_PIVOT] (
...
SELECT @SeasonYearID = seasonYearId FROM pSourcingHeader WHERE SourcingHeaderID = @SourcingHeaderID
...
CASE	
	...
	'<TABLE>
			...
			+ '&SYID=' + CAST(@SeasonYearID AS NVARCHAR(50)) 
			+ '&TP=1'' , ''' 
			+ REPLACE ( CAST ( a.SampleRequestTradeID  AS NVARCHAR(50) ), '-', '' ) 
...
WHERE e.StyleID IN ( SELECT StyleID FROM  pSourcingStyle WHERE SourcingHeaderID  = @SourcingHeaderID ) AND i.SeasonYearID = @SeasonYearID
...
a.Link , d.StyleNo, d.Description, COALESCE(scc.Custom, d.SizeClass), g.ItemDim1Name,h.ItemDim2Name,j.ItemDim3Name, a.ItemDim1Id,a.ItemDim2Id,a.ItemDim3Id, a.TechPack, a.StyleSet, a.SetNo, a.Season, a.Year,a.WorkFlowItemID,a.StyleBOMDimensionID,a.StyleBOMDimensionItemID, Variation,
...
LEFT JOIN pSizeClass scc ON scc.Custom = d.SizeClass
'
...
VALUES ('DB_Version', '6.1.0000', '10227', GetUTCDate())
GO
