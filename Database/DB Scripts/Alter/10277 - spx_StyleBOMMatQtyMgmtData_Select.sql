IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_StyleBOMMatQtyMgmtData_Select]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_StyleBOMMatQtyMgmtData_Select]
GO


CREATE PROCEDURE [dbo].[spx_StyleBOMMatQtyMgmtData_Select]
...
insert into #tmpDimItem (stylematerialId, materialId, materialno, materialname, Placement, UOM)

					select stylematerialId, materialId, materialno, materialname, Placement, UOM from pStyleBOM 					
...
If @Dimlevel = '1'
	begin
		...
		pStyleBOMDimensionitems where StyleBOMDimensionId=@StyleBOMDimensionId
...
WHILE @Rowcnt <= @Matcnt 
	...
	begin
		...
		@materialno = materialno, @materialname = materialname , @Placement = Placement, @UOM = UOM
		...
		If  @Skucnt>0     
			...
			begin
				...
				Placement, UOM, DimID, DimName, DimSort)

				select @Rowcnt, Dtl_ID, @stylematerialId, @materialId, @materialno, @materialname,

				@Placement, @UOM, DimID, DimName, DimSort from #tmpdtlItem 				 			    			
...
SET @query = N'SELECT mat_id, stylematerialId, materialId, materialno, materialname, Placement, UOM, ' +
	@IncrCols +'
	FROM
	(SELECT  t2.mat_id, t2.stylematerialId, t2.materialId, t2.materialno, t2.materialname, t2.Placement, t2.UOM, 
	  t1.dtl_id, t3.MaterialQuantity			  
	...
	inner join #tmpskuItem as t3 on t3.dtlid=t1.dtl_id and t3.dimid=t1.dimid
			...
			t2.materialId = t3.materialId and t2.mat_id=t3.matid 				
			 ) p
	PIVOT
	(
...
--select * from #tmpDtlItem

--select * from #tmpskuItem
...
VALUES ('DB_Version', '6.1.0000', '10277', GetUTCDate())
GO
