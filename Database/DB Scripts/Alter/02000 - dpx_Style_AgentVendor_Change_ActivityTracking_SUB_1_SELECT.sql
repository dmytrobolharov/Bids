...
#01 - Ryan Cabanas - August 25, 2009
	Because 'pStyleHeader.CustomField10' is not used consistently as a date field for
Burberry records, the CAST to 'datetime' is throwing errors.  I've just taken this
SELECT field and am passing a blank to the report to bypass the error.
...
IF  EXISTS (SELECT * from INFORMATION_SCHEMA.ROUTINES WHERE SPECIFIC_NAME = N'dpx_Style_AgentVendor_Change_ActivityTracking_SUB_1_SELECT') 
DROP PROCEDURE dpx_Style_AgentVendor_Change_ActivityTracking_SUB_1_SELECT
GO
...
IF @SortBy = 1
	BEGIN
		...
		SELECT
			...
			,c.StyleID
			,h.StyleNo
			,(SELECT TOP 1 dbo.fnx_GetStreamingImagePath(hImage.ImageID, hImage.[Version])	--Comment #02 
			...
			,/*(CASE WHEN h.CustomField10 IS NULL THEN NULL
		              ...
		              END  )*/ '' as StartShipDate		--Comment #01
			,c.StyleChangeDate
			,c.StyleChangeType
...
VALUES     ('DB_Version', '4.1.0000', '02000', GetDate())
GO