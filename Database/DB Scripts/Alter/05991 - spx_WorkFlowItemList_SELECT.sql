...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			UNION
			SELECT wfi.WorkFlowItemID, wfi.WorkflowID, wfit.MapId, wfi.WorkflowItemTypeId, wfs.WorkflowStatus as Custom, wfit.ItemTypeLabel, wfi.StyleID, wfi.StyleSet, wfi.WorkFlowItemName, wfi.Sort, 
				wfi.CUser, wfi.CDate, wfi.MUser, wfi.MDate,wfi.WorkStatus, 1 AS IsLinked, ISNULL(sh.StyleNo, '') + ISNULL (' - ' + sh.Description, '') AS SourceStyle, '' AS CopySourceStyle, sh.DevelopmentID
			FROM dbo.pWorkFlowItem AS wfi 
...
VALUES ('DB_Version', '0.5.0000', '05991', GetDate())
GO
