...
IF @Status =  1 --If Resubmit Status
	BEGIN
		...
		INSERT INTO pMaterialRequestSubmit (
			...
			RecCarrier, RecTrackNo,  VendorWeight, 
		...
		SELECT @NewMaterialRequestSubmitID, mrs.MaterialRequestSubmitWorkflowID, 
			mrs.MaterialTradePartnerColorID, mrs.MaterialTradePartnerID, 
			@NewSubmit AS Submit, 0 AS [Status], mrs.AssignedTo, mrs.StartDate,
			mrs.RecCarrier, mrs.RecTrackNo,  mrs.VendorWeight,
			mrs.VendorDate, mrs.VendorName, mrs.SubmitDays, mrs.ResubmitDays, 
			DATEADD(DAY, COALESCE(mrs.ResubmitDays, mrwti.MaterialRequestWorkflowRDays, 0), mrs.DueDate), 
			--EndDate, EndBy, 
			mrs.CUser, mrs.CDate, mrs.MUser, mrs.MDate, mrs.TUser, mrs.TDate, 
			mrs.AgentView, mrs.VendorComment, mrs.InternalComment
...
ELSE IF @Status = 2 OR @Status = 3 OR @Status = 4 OR @Status = 0
...
VALUES ('DB_Version', '0.5.0000', '06148', GetDate())
GO
