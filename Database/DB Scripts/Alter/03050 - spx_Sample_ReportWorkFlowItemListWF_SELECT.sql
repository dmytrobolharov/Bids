IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_Sample_ReportWorkFlowItemListWF_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_Sample_ReportWorkFlowItemListWF_SELECT]
GO
...
CREATE PROCEDURE [dbo].[spx_Sample_ReportWorkFlowItemListWF_SELECT]   
...
BEGIN
 SELECT	ReportPageID, ReportMapID, ReportPageTypeID, ReportPageName, 
                ...
                WHERE 
                	ReportMapID = @ReportMapID AND ReportPageActive = 1 
...
IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='03050'))
BEGIN
	
	INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
	VALUES     ('DB_Version', '4.2.0000', '03050', GetDate())     
