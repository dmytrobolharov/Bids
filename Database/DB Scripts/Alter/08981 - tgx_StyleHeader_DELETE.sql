/****** Object:  Trigger [tgx_StyleHeader_DELETE]    Script Date: 08/22/2014 17:46:07 ******/
...
/****** Object:  Trigger [dbo].[tgx_StyleHeader_DELETE]    Script Date: 08/22/2014 17:46:07 ******/
SET ANSI_NULLS ON
GO
...
-- Deleting sample requests in a proper way
DECLARE @SampleRequestTradeID UNIQUEIDENTIFIER
DECLARE @StyleID UNIQUEIDENTIFIER

DECLARE SampleCursor CURSOR SCROLL FOR 
SELECT SampleRequestTradeID, StyleID FROM pSampleRequestTrade WHERE StyleID IN (SELECT StyleID FROM deleted)
OPEN SampleCursor
FETCH NEXT FROM SampleCursor INTO @SampleRequestTradeID, @StyleID
WHILE @@FETCH_STATUS = 0
BEGIN
	EXEC spx_SampleRequestWorkflow_DELETE @SampleRequestTradeID, @StyleID, 1
	FETCH NEXT FROM SampleCursor INTO @SampleRequestTradeID, @StyleID
END
CLOSE SampleCursor
DEALLOCATE SampleCursor

DECLARE BOMSampleCursor CURSOR SCROLL FOR 
SELECT SampleRequestTradeID, StyleID FROM pSampleRequestBOMTrade WHERE StyleID IN (SELECT StyleID FROM deleted)
OPEN BOMSampleCursor
FETCH NEXT FROM BOMSampleCursor INTO @SampleRequestTradeID, @StyleID
WHILE @@FETCH_STATUS = 0
BEGIN
	EXEC spx_SampleRequestBOMWorkflow_DELETE @SampleRequestTradeID, @StyleID, 1
	FETCH NEXT FROM BOMSampleCursor INTO @SampleRequestTradeID, @StyleID
END
CLOSE BOMSampleCursor
DEALLOCATE BOMSampleCursor
...
VALUES ('DB_Version', '6.0.0000', '08981', GetDate())
GO
