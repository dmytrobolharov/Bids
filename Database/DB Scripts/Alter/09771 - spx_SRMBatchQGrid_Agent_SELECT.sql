/****** Object:  StoredProcedure [dbo].[spx_SRMBatchQGrid_Agent_SELECT]    Script Date: 26.02.2015 14:21:54 ******/
...
BEGIN	
	...
	SET @SQLString = 'SELECT DISTINCT SqlBatchQ.* FROM (' + @BatchQueueGridSql + ') AS SqlBatchQ 
		...
		INNER JOIN pTechPack ON pTechPack.StyleID = SqlBatchQ.StyleID 
		INNER JOIN pTechPackTeam ON pTechPack.TechPackID = pTechPackTeam.TechPackID AND pTechPackTeam.TeamID = ''' + CAST(@TradePartnerId AS nVARCHAR(50)) + '''
...
VALUES ('DB_Version', '6.1.0000', '09771', GetDate())
GO
