IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_StyleBOMTemp_INSERT]') AND type in (N'P', N'PC'))
...
DECLARE @Placement varchar(4000)
DECLARE @MaterialCount int
select @MaterialCount = COUNT(*) from pMaterialCoreItem WITH (NOLOCK) WHERE MaterialID = @MaterialID
IF @MaterialCount = 1
	BEGIN
		select @Placement = Placement from pMaterialCoreItem WITH (NOLOCK) WHERE MaterialID = @MaterialID
	END
...
VALUES ('DB_Version', '0.5.0.0000', '05820', GetDate())
GO
