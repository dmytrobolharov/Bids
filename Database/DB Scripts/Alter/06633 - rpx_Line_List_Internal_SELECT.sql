...
BEGIN
	...
	SELECT
		(DENSE_RANK() OVER (ORDER BY lf.LineFolderDescription, sh.StyleNo, m.MaterialNo, sqi.StyleQuoteItemNo, mtp.TradePartnerRelationshipLevelID)-1) / 4 AS Row
		, (DENSE_RANK() OVER (ORDER BY lf.LineFolderDescription, sh.StyleNo, m.MaterialNo, sqi.StyleQuoteItemNo, mtp.TradePartnerRelationshipLevelID)-1) % 4 AS Col
		, lf.LineFolderDescription
		, sh.Description
		, sh.StyleNo
		, sh.SizeRange
		, dbo.fnx_GetStreamingImagePath(sh.DesignSketchID, sh.DesignSketchVersion) AS StyleImage
		...
		, dbo.fnx_GetStreamingColorImagePath(cp.ColorFolderID, cp.ColorPaletteID) AS ColorImage
		, cp.ColorName
		, st.Status
	...
	FROM pLineFolder lf
		...
		LEFT JOIN pMaterialTradePartner mtp ON m.MaterialID = mtp.MaterialId AND m.TradePartnerID = mtp.TradepartnerId AND mtp.SeasonYearID = @SeasonYearID
...
VALUES     ('DB_Version', '0.5.0000', '06633', GetDate())
GO
