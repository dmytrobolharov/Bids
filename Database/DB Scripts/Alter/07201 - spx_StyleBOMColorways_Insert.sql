...
AS 
	...
	Declare @ColorIsAdded bit = 0
	
	begin											
		if @DimLevel=1
			begin
				IF NOT EXISTS(SELECT * FROM pStyleBOMDimensionItem WHERE StyleBOMDimensionID = @StyleBOMDimensionId AND (ItemDim1Id = @ColorPaletteID OR ItemDim1Name = @ColorName))
				BEGIN										
		...
		if @DimLevel=2
			begin
				IF NOT EXISTS(SELECT * FROM pStyleBOMDimensionItem WHERE StyleBOMDimensionID = @StyleBOMDimensionId AND (ItemDim2Id = @ColorPaletteID OR ItemDim2Name = @ColorName))
				BEGIN	
					INSERT INTO	pStyleBOMDimensionItem(StyleBOMDimensionId,	WorkflowID, WorkflowItemID,
		...
		if @DimLevel=3
			begin
				IF NOT EXISTS(SELECT * FROM pStyleBOMDimensionItem WHERE StyleBOMDimensionID = @StyleBOMDimensionId AND (ItemDim3Id = @ColorPaletteID OR ItemDim3Name = @ColorName))
				BEGIN	
					INSERT INTO	pStyleBOMDimensionItem(StyleBOMDimensionId,	WorkflowID, WorkflowItemID,
...
VALUES ('DB_Version', '0.5.0000', '07201', GetDate())
GO
