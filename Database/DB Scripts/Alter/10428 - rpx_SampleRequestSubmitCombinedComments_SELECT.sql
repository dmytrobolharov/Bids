...
BEGIN
	...
	SELECT SampleRequestSubmitID, Comment, 1, ROW_NUMBER() OVER (ORDER BY srs.Submit)
	...
	BEGIN
		...
		FROM fnx_Split(ISNULL(REPLACE(REPLACE(REPLACE(REPLACE(CAST(@FitComment AS NVARCHAR(MAX)), 'span', 'u'), '<br />', '|'), CHAR(10), '|'), CHAR(13),'|'), ''), '|')
		WHERE LEN(value) > 0
...
VALUES ('DB_Version', '6.1.0000', '10428', GetUTCDate())
GO
