...
BEGIN
	...
	BEGIN
		SELECT TOP 1 @Conv = ISNULL(Conv,0) FROM pGRMeasurementsPOMTemplateItem WHERE POMTempID = @NewPOMTempID AND POMCode = @POMCode
		...
		UPDATE pAMLMeasIGC SET Incr = @Conv + @SampleSizeIncr
...
VALUES ('DB_Version', '0.5.0000', '07213', GetDate())
GO
