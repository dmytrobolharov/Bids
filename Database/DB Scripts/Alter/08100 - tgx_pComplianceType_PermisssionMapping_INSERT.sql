/****** Object:  Trigger [tgx_pComplianceType_PermisssionMapping_INSERT]    Script Date: 06/26/2014 11:02:12 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tgx_pComplianceType_PermisssionMapping_INSERT]'))
DROP TRIGGER [dbo].[tgx_pComplianceType_PermisssionMapping_INSERT]
GO

/****** Object:  Trigger [dbo].[tgx_pComplianceType_PermisssionMapping_INSERT]    Script Date: 06/26/2014 11:02:13 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[tgx_pComplianceType_PermisssionMapping_INSERT]
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @ComplianceTypeName NVARCHAR(200)
	    ...
	    FOR SELECT ComplianceTypeID, ComplianceType, 'System', CURRENT_TIMESTAMP, 1
	    FROM inserted
	    
	    OPEN InsertedRows
	    FETCH NEXT FROM InsertedRows INTO @ComplianceTypeID, @ComplianceTypeName, @CUser, @CDate, @Active
	    
		WHILE @@FETCH_STATUS = 0
	    BEGIN
			-- Inserting product type mapping for Image Folder
			IF NOT EXISTS(SELECT * FROM sPermissionProductTypeMap 
							WHERE PermissionFolderTypeId = @ResourceFolderTypeID AND IntProductTypeId = @ComplianceTypeID)
			BEGIN
				INSERT INTO sPermissionProductTypeMap(PermissionFolderTypeId, IntProductTypeId)
				VALUES(@ResourceFolderTypeID, @ComplianceTypeID)
			END
			...
			SELECT PermissionFolderID, @ProductTypeMapID, 1, GroupId,
												@CUser, @CDate, @CUser, @CDate, @Active, @ComplianceTypeName
			FROM sPermissionFolder pf
			WHERE PermissionFolderTypeId = @ResourceFolderTypeID 
...
VALUES ('DB_Version', '0.5.0000', '08100', GetDate())
GO