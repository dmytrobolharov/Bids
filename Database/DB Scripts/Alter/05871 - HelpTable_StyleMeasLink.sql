...
IF NOT EXISTS(SELECT * FROM sSystemPage WHERE MappingPage='Style_MeasLink_Search.aspx')
	BEGIN
		INSERT INTO [sSystemPage] (MappingPage, CDate, CUser, sSystemFolderID)
		VALUES ('Style_MeasLink_Search.aspx', GETDATE(), 'Administrator', (SELECT sSystemFolderID from sSystemFolder where sSystemFolder='Style'))
	END
GO

IF NOT EXISTS(SELECT * FROM sSystemPage WHERE MappingPage='Style_MeasLink_Select.aspx')
	BEGIN
		INSERT INTO [sSystemPage] (MappingPage, CDate, CUser, sSystemFolderID)
		VALUES ('Style_MeasLink_Select.aspx', GETDATE(), 'Administrator', (SELECT sSystemFolderID from sSystemFolder where sSystemFolder='Style'))
	END
GO
...
VALUES     ('DB_Version', '0.5.0000', '05871', GetDate())
GO
