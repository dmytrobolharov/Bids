...
DECLARE @Cnt int
SELECT @Cnt = COUNT(*) from sys.objects where name =  'ColorLibraryCoated' and type = 'U'
IF @Cnt =  0
BEGIN 

	CREATE TABLE [dbo].[ColorLibraryCoated](
	 ...
	 CONSTRAINT [PK_ColorLibraryCoated] PRIMARY KEY CLUSTERED 
	(
		[BookOrder] ASC
	)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
	) ON [PRIMARY]

	ALTER TABLE [dbo].[ColorLibraryCoated] ADD  CONSTRAINT [DF_ColorLibraryCoated_PANTONEType]  DEFAULT (N'PANTONE COATED') FOR [PANTONEType]
 

	ALTER TABLE [dbo].[ColorLibraryCoated] ADD  CONSTRAINT [DF_ColorLibraryCoated_Active]  DEFAULT ((1)) FOR [Active]
	
	ALTER TABLE [dbo].[ColorLibraryCoated] ADD  CONSTRAINT [DF_ColorLibraryCoated_ColorLibraryTypeID]  DEFAULT ('00000000-0000-0000-0000-0000000000e)') FOR [ColorLibraryTypeID]
...
VALUES ('DB_Version', '5.0.0000', '03857', GetDate())
GO
