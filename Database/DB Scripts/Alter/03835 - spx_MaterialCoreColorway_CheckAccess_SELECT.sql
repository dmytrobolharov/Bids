IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_MaterialCoreColorway_CheckAccess_SELECT]') AND type in (N'P', N'PC'))
...
IF  @RowMaterialCoreCount = 0
	BEGIN
		...
		FROM  pMaterial INNER JOIN
			  ...
			  pComponentType ON pMaterial.MaterialType = pComponentType.ComponentTypeID LEFT JOIN
			  sAccessMaterialFolder AS AccessFolder ON AccessFolder.ComponentTypeId = pComponentType.ComponentTypeID
		WHERE pMaterialCoreItem.MaterialCoreID = @MaterialCoreID
				AND AccessFolder.AccessRoleId <> 0
				AND AccessFolder.TeamId = @TeamID
...
ELSE
	...
	--Create a temp table with all available color to pivot the table
		...
		FROM pMaterial INNER JOIN
		  ...
		  pMaterialCoreItem.MaterialCoreItemID = pMaterialCoreColorItem.MaterialCoreItemID LEFT JOIN
		  sAccessMaterialFolder AS AccessFolder ON AccessFolder.ComponentTypeId = pComponentType.ComponentTypeID
		WHERE pMaterialCoreItem.MaterialCoreID = @MaterialCoreID
				AND AccessFolder.AccessRoleId <> 0
				AND AccessFolder.TeamId = @TeamID
...
VALUES     ('DB_Version', '5.0.0000', '03835', GetDate())
GO
