/****** Object:  View [dbo].[vwx_PlanningItem_Import_SEL]    Script Date: 06/04/2014 15:40:35 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vwx_PlanningItem_Import_SEL]'))
...
SELECT psy.PlanningID, psy.SeasonYearID AS DstSeasonYearID, pli.PlanningItemID,
	pli.StyleBOMDimensionID, pli.Sort, pli.SeasonYearID AS SrcSeasonYearID, 
	sh.StyleID, sh.StyleMasterID, sh.StyleWorkflowID, sh.StyleType, sh.WorkflowType, sh.RefNo,
	sh.StyleNo, sh.TempID, sh.TempNo, sh.CustomerNo, sh.DevelopmentID, sh.DevelopmentNo, sh.ConceptID,
	sh.ConceptNo, sh.SpecNo, sh.Description, sh.StyleCategory, sh.SizeClass, sh.SizeRange,
...
INNER JOIN (
		...
		MAX(workflow.PermissionRoleId) AS AccessRoleID, MAX(workflow.PermissionModify) AS AccessModify, MAX(workflow.PermissionView) AS AccessView
...
) asf ON sh.StyleType = asf.IntProductTypeId
...
VALUES ('DB_Version', '0.5.0000', '07889', GetDate())
GO