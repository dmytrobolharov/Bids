...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END)) < 0 THEN CAST(ABS(DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
		WHEN COUNT(CASE WHEN tmmw.IsApproved = 0 OR tmmw.IsApproved IS NULL THEN 1 ELSE NULL END) = 0 THEN 'LightGreen'
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END)) < 0 THEN 'IndianRed'
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END)) BETWEEN 0 AND 7 THEN 'Khaki'		
		ELSE 'None'
	  END AS Color
...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmr.ApprovedStatusCount = 0 THEN tmmr.DueDate ELSE NULL END)) < 0 THEN CAST(ABS(DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmr.ApprovedStatusCount = 0 THEN tmmr.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmr.ApprovedStatusCount = 0 THEN tmmr.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
		...
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmr.ApprovedStatusCount = 0 THEN tmmr.DueDate ELSE NULL END)) < 0 THEN 'IndianRed'
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmmr.ApprovedStatusCount = 0 THEN tmmr.DueDate ELSE NULL END)) BETWEEN 0 AND 7 THEN 'Khaki'		
		ELSE 'None'
	  END AS Color
FROM #tmpCalStatic tcs
CROSS JOIN pCalendarMilestoneItem cmi
...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmsw.ApprovedStatusCount = 0 THEN tmsw.DueDate ELSE NULL END)) < 0 THEN CAST(ABS(DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmsw.ApprovedStatusCount = 0 THEN tmsw.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmsw.ApprovedStatusCount = 0 THEN tmsw.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
		WHEN COUNT(CASE WHEN tmsw.ApprovedStatusCount = 0 AND tmsw.WorkflowID IS NOT NULL THEN 1 ELSE NULL END) = 0 THEN 'LightGreen'
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmsw.ApprovedStatusCount = 0 THEN tmsw.DueDate ELSE NULL END)) < 0 THEN 'IndianRed'
		WHEN DATEDIFF(DAY, cmi.MilestoneDate, MAX(CASE WHEN tmsw.ApprovedStatusCount = 0 THEN tmsw.DueDate ELSE NULL END)) BETWEEN 0 AND 7 THEN 'Khaki'		
		ELSE 'None'
	  END AS Color
FROM #tmpCalStatic tcs
CROSS JOIN pCalendarMilestoneItem cmi
...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, GETDATE(), MAX(tmw.DueDate)) < 0 THEN CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(tmw.DueDate))) AS VARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(tmw.DueDate))) AS VARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
...
VALUES ('DB_Version', '0.5.0000', '05520', GetDate())
GO
