IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_SampleRequest_MeasNewSample_Insert]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_SampleRequest_MeasNewSample_Insert]
GO
...
AS 
	...
	DECLARE @CurrentEvalMeasValue NUMERIC(18, 4)
	...
	BEGIN
		...
		SELECT @CurrentRevisedSpec = RevisedSpec, @CurrentEvalMeasValue = EvalMeasValue
		FROM pMSEvalSampMeas
		...
		-- Updating new Eval Size with old Revised Size	or old Eval Size (if Revised is null)	
		UPDATE pmsevalsampMeas
		SET EvalMeasValue = CASE WHEN @CurrentRevisedSpec IS NULL OR @CurrentRevisedSpec = 0.00 THEN @CurrentEvalMeasValue ELSE @CurrentRevisedSpec END
		WHERE AMLMeasHdrId = @AMLMeasHdrId
		AND SampleRequestHeaderId = @SampleRequestHeaderId
		AND SampleIndex = @NewSubmit
...
VALUES ('DB_Version', '6.0.0000', '08818', GetDate())
GO
