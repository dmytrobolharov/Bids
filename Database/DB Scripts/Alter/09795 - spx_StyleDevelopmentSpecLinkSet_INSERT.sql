IF OBJECT_ID(N'[dbo].[spx_StyleDevelopmentSpecLinkSet_INSERT]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_StyleDevelopmentSpecLinkSet_INSERT]
GO

create  PROCEDURE [dbo].[spx_StyleDevelopmentSpecLinkSet_INSERT]
(
...
-- SELECT POMTempID , * FROM pStyleSpec WHERE StyleID = '00573c22-a262-4c93-8e12-3323f4c348d7'
--select * from pStyleHeader  WHERE StyleID = '00573c22-a262-4c93-8e12-3323f4c348d7'

IF @StyleSet = 1 
	SELECT @POMTempID = POMTempID1 FROM pStyleHeader  WHERE StyleID = @StyleID
ELSE IF @StyleSet = 2
	SELECT @POMTempID = POMTempID2 FROM pStyleHeader  WHERE StyleID = @StyleID
ELSE IF @StyleSet = 3
	SELECT @POMTempID = POMTempID3 FROM pStyleHeader  WHERE StyleID = @StyleID
ELSE IF @StyleSet = 4
	SELECT @POMTempID = POMTempID4 FROM pStyleHeader  WHERE StyleID = @StyleID
...
FROM pPOMTemplate WHERE POMTempGroupID = @POMTempGroupID AND SizeRange = @SizeRange AND SizeClass = @SizeClass


--INSERT pStyleSpecSize

--SELECT * FROM pPOMTemplate WHERE POMTempGroupID = @POMTempGroupID AND SizeRange = @SizeRange AND SizeClass = @SizeClass

--SELECT * FROM #tempStyleSpec

SET @Row_Loop = 1
SET @Row_Count = (SELECT COUNT(*) FROM #tempStyleSpec)
...
IF @Row_Count <> 0
	...
	WHILE @Row_Loop <= @Row_Count 
		BEGIN
			...
			FROM #tempStyleSpec WHERE RecID = @Row_Loop
			...
			IF (((SELECT COUNT(*) FROM pPOMTemplateItem WHERE POM = @POM AND PointMeasur = @PointMeasur AND POMTempID = @POMTempID) = 1) AND
				((SELECT POMTempID FROM #tempStyleSpec WHERE RecID = @Row_Loop) IS NOT NULL))
				BEGIN
					...
					--FROM pPOMTemplateItem WHERE POM = @POM AND PointMeasur = @PointMeasur AND POMTempGroupID = @POMTempGroupID AND  POMTempID = @POMTempID 
					FROM pPOMTemplateItem WHERE POM = @POM AND PointMeasur = @PointMeasur AND POMTempID = @POMTempID 
				
					UPDATE  #tempStyleSpec SET 
			...
			--SELECT * FROM #tempStyleSpec WHERE RecID = @Row_Loop
		...
		IF @StyleSet = 1 
			...
			SET POMTempID1 = @POMTempID, 
			    SizeClass = @SizeClass,
				SizeClassId = (SELECT TOP 1 CustomId FROM pSizeClass WHERE Custom = @SizeClass), 
				Sizerange = @SizeRange,
				SizeRangeId = (SELECT TOP 1 CustomId FROM pSizeRange WHERE SizeRangeCode = @SizeRange)
...
VALUES ('DB_Version', '6.1.0000', '09795', GetDate())
GO
