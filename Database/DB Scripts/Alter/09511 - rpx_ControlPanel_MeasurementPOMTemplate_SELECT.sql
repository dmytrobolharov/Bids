IF OBJECT_ID(N'[dbo].[rpx_ControlPanel_MeasurementPOMTemplate_SELECT]') IS NOT NULL
...
BEGIN
	...
	, Pom AS (
	SELECT sc.Custom + ' (' + sr.SizeRangeCode + ')' AS SizeClassRangeStr, CAST(REPLACE(Size, 'Size', '') AS INT) AS Size, ColumnName
	...
	LEFT JOIN pSizeClass sc ON sc.CustomID = upvt.SizeClassId
	LEFT JOIN pSizeRange sr ON sr.CustomID = upvt.SizeRangeId
	WHERE upvt.POMTempID = @POMTempID)
	SELECT Critical, POMCode, POMDesc
		, CASE WHEN @IsMetric = 0 THEN ISNULL(dbo.fnx_Num2Frac_KeepNegative(TolPlus),0) ELSE CAST(TolPlus AS NVARCHAR(20)) END AS TolPlus
		, CASE WHEN @IsMetric = 0 THEN ISNULL(dbo.fnx_Num2Frac_KeepNegative(TolMinus),0) ELSE CAST(TolMinus AS NVARCHAR(20)) END AS TolMinus
...
VALUES ('DB_Version', '6.1.0000', '09511', GetDate())
GO
