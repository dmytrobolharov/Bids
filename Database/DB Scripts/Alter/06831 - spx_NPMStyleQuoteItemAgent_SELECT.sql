IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_NPMStyleQuoteItemAgent_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_NPMStyleQuoteItemAgent_SELECT]
GO


CREATE PROCEDURE [dbo].[spx_NPMStyleQuoteItemAgent_SELECT] (
...
BEGIN
	...
	SELECT 'N/A' AS ColorName, qi.StyleQuoteItemID, 
...
BEGIN
	...
	INNER JOIN pSourcingTradePartner stp ON qi.StyleQuoteTradePartnerID = stp.SourcingTradePartnerID
	INNER JOIN vwx_TradePartnerRelationshipLevels_SEL tprl ON stp.TradePartnerLevelID = tprl.TradePartnerRelationshipLevelID
	LEFT OUTER JOIN pStyleSeasonYear c WITH (NOLOCK) ON qi.StyleSeasonYearID = c.StyleSeasonYearID
	LEFT OUTER JOIN pSeasonYear b WITH (NOLOCK) ON c.SeasonYearID  = b.SeasonYearID
	WHERE qi.StyleID = @StyleId	AND b.SeasonYearID = @SeasonYearId
	ORDER BY qi.StyleQuoteItemNo DESC
...
VALUES ('DB_Version', '0.5.0000', '06831', GetDate())
GO
