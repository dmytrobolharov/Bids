IF OBJECT_ID(N'[dbo].[spx_TechPackCalendarListTPBQ_INSERT]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_TechPackCalendarListTPBQ_INSERT]
GO

CREATE PROCEDURE [dbo].[spx_TechPackCalendarListTPBQ_INSERT]
...
BEGIN
	...
	WHERE (sh.Active = 1) AND (sh.TechPackDate BETWEEN @StartDate AND @EndDate) AND (sh.TechPackID IS NOT NULL) AND (sh.StyleType = @StyleType)
END 
...
VALUES ('DB_Version', '6.1.0000', '09850', GetDate())
GO
