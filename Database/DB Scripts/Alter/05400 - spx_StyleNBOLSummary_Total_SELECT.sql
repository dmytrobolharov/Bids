/****** Object:  StoredProcedure [dbo].[spx_StyleNBOLSummary_Total_SELECT]    Script Date: 03/25/2013 15:53:58 ******/
...
(
	...
	@ExchangeRateItemId uniqueidentifier = '00000000-0000-0000-0000-000000000000',
...
BEGIN
	...
	SET @strSQL = 'UPDATE #pTotal SET TotalExchangeRate_' + CAST(@jROW as nvarchar(5)) + ' = ' +
		'TotalRate_' + CAST(@jROW as nvarchar(5)) + ' * ' + CAST(@Rate as nvarchar(10))
	EXEC (@strSQL)
...
BEGIN
	SELECT @ExceptionId = ExceptionID FROM #tmpExceptions WHERE ROW = @jROW
	SET @strSQL = @strSQL + ', Total_' + CAST(@jROW as nvarchar(5)) + ', TotalRate_' + CAST(@jROW as nvarchar(5)) + ', TotalExchangeRate_' + CAST(@jROW as nvarchar(5))
	SET @jROW = @jROW + 1
END
...
VALUES ('DB_Version', '0.5.0000', '05400', GetDate())
GO
