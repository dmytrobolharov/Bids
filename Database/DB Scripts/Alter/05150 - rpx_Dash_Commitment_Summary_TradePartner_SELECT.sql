IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpx_Dash_Commitment_Summary_TradePartner_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[rpx_Dash_Commitment_Summary_TradePartner_SELECT]
GO


CREATE PROCEDURE [dbo].[rpx_Dash_Commitment_Summary_TradePartner_SELECT]
...
BEGIN
	insert into #tmpQuoteItems
		select 
			...
			, ISNULL(sci.StyleQuoteItemCustomField18 * sci.StyleQuoteItemCustomField16,0) AS Extended_LDP
			, ISNULL((sci.StyleCostingCustomField15 * sci.StyleQuoteItemCustomField2),0) AS Extended_WHSE
			, (ISNULL((sci.StyleCostingCustomField15 * sci.StyleQuoteItemCustomField2),0) - ISNULL((sci.StyleQuoteItemCustomField18 * sci.StyleQuoteItemCustomField16),0)) AS Extended_Gross_Margin -- Extended_WHSE - Extended_LDP
		FROM
			...
			INNER JOIN vwx_TradePartnerRelationshipLevels_SEL tpm ON stp.TradePartnerLevelID = tpm.TradePartnerRelationshipLevelID
			LEFT JOIN uTradePartner tp ON tp.TradePartnerID = sci.TradePartnerID
...
SET Style_Colors = (
	...
	WHERE
		#tmpQuoteItems.SourcingCommitmentItemID = sqbom.SourcingCommitmentItemID
)
...
VALUES ('DB_Version', '5.0.0000', '05150', GetDate())
GO
