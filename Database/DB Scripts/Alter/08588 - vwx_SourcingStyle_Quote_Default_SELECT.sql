/****** Object:  View [dbo].[vwx_SourcingStyle_Quote_Default_SELECT]    Script Date: 08/06/2014 11:04:34 ******/
...
SELECT DISTINCT 
                      ...
                      dbo.pStyleQuoteItem.StyleCostingCustomField34, dbo.pStyleQuoteItem.StyleCostingCustomField35, 
                      dbo.pStyleQuoteItem.StyleQuoteItem_EcomUnits, dbo.pStyleQuoteItem.StyleQuoteItem_EcomPriceUnit, 
                      dbo.pStyleHeader.StyleNo, dbo.pStyleHeader.SizeClass, 
                      ...
                      + ISNULL(dbo.pStyleQuoteItem.StyleQuoteItemCustomField13, '0.00')) + ISNULL
                          ((SELECT     SUM(ISNULL(Amount, '0.00')) AS Expr1
                              ...
                              WHERE     (QuoteItemID = dbo.pStyleQuoteItem.StyleQuoteItemID)), '0.00') AS PartnerExtendedCost, dbo.pStyleQuoteItem.StyleCostingScenarioItemID, 
                      dbo.pStyleQuoteItem.TechPack, dbo.pStyleQuoteItem.StyleSKUItemID, TACal.TACalTemplateId AS CalendarID
FROM         dbo.pStyleQuoteItem INNER JOIN
                      ...
                      (   SELECT ct.TACalTemplateId, cr.ReferenceId, ct.SeasonYearId FROM 
						  pTACalTemplate ct INNER JOIN
						  pTACalReference cr ON ct.TACalTemplateId = cr.TACalTemplateId AND cr.TACalReferenceTypeId = 19
					  ) AS TaCal ON TaCal.SeasonYearId = pStyleSeasonYear.SeasonYearID AND TACal.ReferenceId = pStyleQuoteItem.StyleQuoteItemID
...
VALUES ('DB_Version', '0.5.0000', '08588', GetDate())
GO
