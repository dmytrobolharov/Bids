...
BEGIN
	...
	SET @WebServiceID = (SELECT WebServiceID FROM sWebService WHERE UPPER(WebServiceName) = UPPER('ControlPanel'))

	-- ControlPanelType
	IF (SELECT COUNT(WebServiceItemName)
		FROM sWebServiceItem
		WHERE UPPER(WebServiceItemName) = UPPER('ControlPanel Type')) < 1
	BEGIN
		INSERT INTO sWebServiceItem(WebServiceID, WebServiceItemID, WebServiceItemName, WebServiceItemDefaultSchema, Active)
		VALUES (@WebServiceID, NEWID(), 'ControlPanel Type', '', 1)
	END	
END
GO
...
VALUES ('DB_Version', '0.5.0000', '05785', GetDate())
GO
