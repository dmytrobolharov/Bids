...
/****** Object:  StoredProcedure [dbo].[spx_SampleRequestBOMSpecRegrade_SELECT]    Script Date: 12/18/2011 18:36:33 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_SampleRequestBOMSpecRegrade_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_SampleRequestBOMSpecRegrade_SELECT]
GO
...
Create PROCEDURE [dbo].[spx_SampleRequestBOMSpecRegrade_SELECT]
(
...
BEGIN
	...
	SELECT @SampleStatus = a.Status,  @ApprovedType = ApprovedType 
...
BEGIN
	SELECT *, @SampleStatus AS SampleStatus, @ApprovedType AS ApprovedType,
	@SampleSizePosition AS SampleSizePosition FROM pSampleRequestSpecItem WITH (NOLOCK)
	WHERE     
	(SampleRequestWorkflowID = @SampleRequestWorkflowID) AND 
	(SampleRequestTradeID = @SampleRequestTradeID) AND 
	(StyleID = @StyleID) AND 
...
IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='02409'))
BEGIN

	INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
	VALUES     ('DB_Version', '4.1.0000', '02409', GetDate())
