/****** Object:  View [dbo].[vwx_TACalTemplateTask_ReferenceFromStyle_SELECT]    Script Date: 08/02/2014 16:27:45 ******/
...
SELECT     crt.TACalRefTaskId, 'Style' AS RefType, sh.StyleID, tw.WorkflowId, ct.SeasonYearId, sh.StyleNo, sh.Description, sh.StyleType, sh.DivisionID, sh.SizeClass, 
                      sh.SizeRange, sh.StyleCategory, sh.IntroSeasonYearID, crt.TaskActualStart, crt.TaskActualEnd, crt.TaskRevisedlStart, 
                      crt.TaskRevisedEnd, crt.TACalTemplateTaskId, sw.WorkAssignedTo, sw.WorkStatus
FROM         dbo.pTACalReferenceTask AS crt INNER JOIN
                      ...
                      dbo.pStyleWorkflow sw ON sw.StyleID = sh.StyleID AND sw.WorkflowID = tw.WorkflowId INNER JOIN
...
VALUES ('DB_Version', '0.5.0000', '08534', GetDate())
GO
