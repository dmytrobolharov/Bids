IF OBJECT_ID(N'[dbo].[vwx_SampleRequest_TimeLineSearchBOM_SELECT]') IS NOT NULL
 DROP VIEW [dbo].[vwx_SampleRequest_TimeLineSearchBOM_SELECT]
GO

CREATE VIEW [dbo].[vwx_SampleRequest_TimeLineSearchBOM_SELECT]
AS
SELECT srwb.TradePartnerID, srwb.TradePartnerVendorID, sh.StyleID, sh.StyleType, sh.WorkflowType, sh.RefNo, 
	   ...
	   sh.MUser, sh.MDate, sh.Active, srwb.SampleRequestTradeID, srwb.Status, ssy.SeasonYearID
FROM dbo.pStyleHeader sh
INNER JOIN pSampleRequestBOMTrade srbt ON sh.StyleID = srbt.StyleID 
INNER JOIN pSampleRequestWorkflowBOM srwb ON srbt.SampleRequestTradeID = srwb.SampleRequestTradeID 
...
VALUES ('DB_Version', '6.1.0000', '09462', GetDate())
GO
