IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpx_Style_ImageSummary_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[rpx_Style_ImageSummary_SELECT]
GO

CREATE   PROCEDURE [dbo].[rpx_Style_ImageSummary_SELECT] 
...
DECLARE @ImageTypeCode nvarchar(200)
DECLARE @ImageTypeName nvarchar(200)
...
BEGIN
	...
	SELECT @ImageTypeName AS ImageName, 
		(dbo.fnx_GetStreamingImagePath(hi.ImageID, hi.[Version], 2000)) AS ImagePath,	--Comment #01
...
VALUES ('DB_Version', '6.1.0000', '10160', GetUTCDate())
GO