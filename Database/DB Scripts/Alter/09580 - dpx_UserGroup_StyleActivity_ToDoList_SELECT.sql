IF OBJECT_ID(N'[dbo].[dpx_UserGroup_StyleActivity_ToDoList_SELECT]') IS NOT NULL
...
IF @SortBy = 1  -- by user name / default
    BEGIN 
		...
		ORDER BY UserName,  h.StyleNo, COALESCE(sc.Custom, h.SizeClass), w.StyleSet,   w.WorkSort, w.WorkflowID
...
BEGIN 
		...
		ORDER BY w.WorkDue, UserName,  h.StyleNo, COALESCE(sc.Custom, h.SizeClass), w.StyleSet,   w.WorkSort, w.WorkflowID
...
IF @SortBy = 3 -- by dayslate
	BEGIN 
		...
		ORDER BY DATEDIFF(d, w.WorkDue, GETDATE()) , UserName,  h.StyleNo, COALESCE(sc.Custom, h.SizeClass), w.StyleSet,   w.WorkSort, w.WorkflowID
...
BEGIN 
		...
		ORDER BY c.StyleCategory, UserName, h.StyleNo, COALESCE(sc.Custom, h.SizeClass), w.StyleSet, w.WorkSort
...
VALUES ('DB_Version', '6.1.0000', '09580', GetDate())
GO
