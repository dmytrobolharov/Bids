IF OBJECT_ID(N'[dbo].[spx_pGRMeasurementsPOMTemplate_Add]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_pGRMeasurementsPOMTemplate_Add]
GO
...
Declare @ClassRange nvarchar(610)
...
select @ClassRangeId = mcr.ClassRangeId , 
       @SizeClass = sc.Custom, 
	   @SizeRange = sr.SizeRangeCode
...
where mcr.SizeClassId = @SizeClassId and mcr.SizeRangeId = @SizeRangeId
...
from 
	(select CustomId as SizeRangeId, SizeRangeCode as SizeRange ,  ' + @IncrCols + ' from pSizeRange
	 where CustomId = ''' + convert(nvarchar(50),@SizeRangeId) + ''' and SizeRangeCode = N''' + REPLACE(convert(nvarchar(50),@SizeRange),'''','''''') + ''') p 
	 unpivot
...
INSERT INTO pGRMeasurementsPOMTemplateItem(
  ...
  POMID, AlternatesId, POMAlternatesId, IsOnQA, IsLinked, FitComId, Critical, POMCode, POMDesc, 
  AlternatesCode, AlternatesDesc, How2MeasText, How2MeasName, POMSort, AlternatesSort, Active, 
  CopyPOMTempItemID, TolPlus, TolMinus, POMTempItemGroupID, Cuser, Cdate, Mdate, MUser)
SELECT @POMTempGroupId, @NewPOMTempID, @SizeClassId, @SizeRangeId, @ClassRangeId, ImageId, 
	   POMID, AlternatesId, POMAlternatesId, IsOnQA, IsLinked, FitComId, Critical, POMCode, POMDesc, 
	   AlternatesCode, AlternatesDesc, How2MeasText, How2MeasName, POMSort, AlternatesSort, Active, 
	   POMTempItemID, TolPlus, TolMinus, POMTempItemGroupID, @CreatedBy, @CreatedDate, @CreatedDate , @CreatedBy   
...
VALUES ('DB_Version', '6.1.0000', '09525', GetDate())
GO
