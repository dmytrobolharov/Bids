IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpx_Quote_SharedCommentSummary_srmOn_SELECT]') AND type in (N'P', N'PC'))
...
BEGIN
	...
	WHERE sqc.CTeamID = @TradePartnerID
		OR sqc.CTeamID IN (SELECT value FROM dbo.fnx_Split(@CUserID, ','))
...
VALUES ('DB_Version', '0.5.0000', '06806', GetDate())
GO
