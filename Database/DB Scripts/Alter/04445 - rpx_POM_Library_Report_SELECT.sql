...
FROM (
	SELECT
		...
		, a.AlternatesCode
		, a.AlternatesDesc
		, (dbo.fnx_GetStreamingImageGRPOMPath(ISNULL(pom.ImageId,'00000000-0000-0000-0000-000000000000'))) AS ImageFile
		, pom.MDate
		, pom.Muser
	FROM (
		SELECT
			ROW_NUMBER() OVER(ORDER BY pomid DESC) AS Row
			, *
		FROM
			pGRMeasurementsPOM
	) pom
...
VALUES     ('DB_Version', '5.0.0000', '04445', GetDate())
GO
