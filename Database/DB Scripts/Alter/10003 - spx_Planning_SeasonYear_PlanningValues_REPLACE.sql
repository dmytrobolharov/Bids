IF OBJECT_ID(N'[dbo].[spx_Planning_SeasonYear_PlanningValues_REPLACE]') IS NOT NULL
...
BEGIN
	...
	WHERE t1.PlanningID = @PlanningID AND t1.SeasonYearID = @srcSeasonYearID	
		AND t2.PlanningStyleCategoryToSubCategoryID IS NULL
	...
	INSERT INTO pPlanningBusiness (PlanningBusinessID, PlanningID, PlanningFinancialID, PlanningFinancialText, PlanningFinancialSort,
		...
		PlanningDivisionSort, PlanningStyleTypeSort, PlanningPlanningCh,
		CUser, CDate, MUser, MDate, PlanningBrandID, PlanningBrand, SeasonYearID, PlanningSubCategoryID, PlanningSubCategory, Active)
	SELECT NEWID(), t1.PlanningID, t1.PlanningFinancialID, t1.PlanningFinancialText, t1.PlanningFinancialSort,
		...
		t1.PlanningDivisionSort, t1.PlanningStyleTypeSort, t1.PlanningPlanningCh,
		t1.CUser, t1.CDate, t1.MUser, t1.MDate, t1.PlanningBrandID, t1.PlanningBrand, @dstSeasonYearID, t1.planningSubCategoryID, t1.PlanningSubCategory, t1.Active
	...
	/* import Development Tracker configs (WI#22944) */
	INSERT INTO pPlanningConfig (PlanningConfigID, PlanningID, SeasonYearID, DivisionID, StyleTypeID, StyleCategoryID, [Description], WorkflowTemplateID,
		 SizeClassId, SizeRangeId, [Status], StyleThemeID, SubCategoryID, CUser, CDate, MUser, MDate)
	SELECT NEWID(), @PlanningID, @dstSeasonYearID, DivisionID, StyleTypeID, StyleCategoryID, [Description], WorkflowTemplateID,
		 SizeClassId, SizeRangeId, [Status], StyleThemeID, SubCategoryID, @CUser, @CDate, @CUser, @CDate
	FROM pPlanningConfig
	WHERE PlanningID = @PlanningID AND SeasonYearID = @srcSeasonYearID
	/*************************************************/
...
VALUES ('DB_Version', '6.1.0000', '10003', GetUTCDate())
GO
