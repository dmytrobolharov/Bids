/****** Object:  StoredProcedure [dbo].[spx_SampleRequest_DesignSample_Meas_POMTemplateLibrary_Add]    Script Date: 07/02/2013 15:44:37 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_SampleRequest_DesignSample_Meas_POMTemplateLibrary_Add]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_SampleRequest_DesignSample_Meas_POMTemplateLibrary_Add]
GO
...
CREATE PROCEDURE [dbo].[spx_SampleRequest_DesignSample_Meas_POMTemplateLibrary_Add]
(
...
SELECT @MaxPomRow = isnull(MAX(pom_row),0) FROM pSampleRequestamlmeaspom WITH (NOLOCK) WHERE SampleRequestAMLMeasHdrId = @SampleRequestAMLMeasHdrId
Set @MaxPomRow=@MaxPomRow+1

set @SampleRequestAMLMeasPomId= NEWID()

INSERT INTO pSampleRequestamlmeaspom(SampleRequestAMLMeasHdrId, SampleRequestAMLMeasPomId, GRPOMCompanyId, POMGuidId, POMAlternatesId,  Refcode, Description, 
...
SELECT @SampleRequestAMLMeasHdrId , @SampleRequestAMLMeasPomId, POMTempItemID , POMId, POMAlternatesId, POMcode, 	
			...
			begin								
				if @IsNegative = 1
					begin
						...
						and p.SampleRequestamlmeashdrid=@SampleRequestAMLMeasHdrId and p.SampleRequestamlmeaspomid=@SampleRequestAMLMeasPomId and p.POM_Row=@MaxPomRow
...
if @IsRelative=1 
	begin
		EXEC spx_SampleRequest_DesignSample_MeasTemplate_IMRelative_UPDATE	
	...
	begin
		EXEC spx_SampleRequest_DesignSample_MeasTemplate_IMIncrement_UPDATE	
		@SampleRequestAMLMeasHdrId,
		@sTemplatecol,
		@MaxPomRow		
	end	
...
if @samplecol<> @sTemplatecol
	begin
		if @IsRelative=1 
			begin
				EXEC spx_SampleRequest_DesignSampleMeas_TempSmpSizeRelative_Update
				@SampleRequestAMLMeasHdrId,
				@samplecol,				
				@MaxPomRow	
			end
			...
			begin
				EXEC spx_SampleRequest_DesignSample_Meas_TempSmpSizeIncrement_Update
				@SampleRequestAMLMeasHdrId,
				@samplecol,				
				@MaxPomRow
			end
...
VALUES ('DB_Version', '0.5.0000', '05970', GetDate())
GO