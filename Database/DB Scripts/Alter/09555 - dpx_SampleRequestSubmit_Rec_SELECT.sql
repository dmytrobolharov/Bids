IF OBJECT_ID(N'[dbo].[dpx_SampleRequestSubmit_Rec_SELECT]') IS NOT NULL
 DROP PROCEDURE [dbo].[dpx_SampleRequestSubmit_Rec_SELECT]
GO

CREATE PROCEDURE [dbo].[dpx_SampleRequestSubmit_Rec_SELECT]
(
	@Flag int
)
AS
...
IF(@Flag = 1)	--Submits Received Today.
	BEGIN
		...
		SELECT Users.FirstName + ' ' + Users.LastName AS AssignedTo, sh.StyleNo, 
			  ...
			  pSampleRequestSubmit.RecDate, pSampleRequestSubmit.RecBy, pSampleRequestSubmit.RecCarrier, 
			  pSampleRequestSubmit.RecTrackNo, pSampleRequestSubmit.RecWeight
		FROM  pSampleRequestWorkflow INNER JOIN
			  ...
			  uTradePartner ON uTradePartnerVendor.TradePartnerID = uTradePartner.TradePartnerID INNER JOIN
			  pSampleRequestSubmit ON 
			  pSampleRequestWorkflow.SampleRequestWorkflowID = pSampleRequestSubmit.SampleRequestWorkflowID AND 
			  pSampleRequestWorkflow.Submit = pSampleRequestSubmit.Submit LEFT OUTER JOIN
...
VALUES ('DB_Version', '6.1.0000', '09555', GetDate())
GO
