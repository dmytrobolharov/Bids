...
SELECT @NewStyleMaterialID, '00000000-0000-0000-0000-000000000000' as StyleMaterialMasterID, MainMaterial,MaterialLining,Development, MiscColor, @StyleSet, @StyleID, UOM, Qty, VolumePrice, Ext, 
	...
	L, M, N, O, P, Q, R, S, T, U, V, W, X, Y, Z, 0, 0, TradePartnerID, TradePartnerVendorID, TradePartnerRelationshipLevelID, MaterialBOM , @MaterialSizeID AS MaterialSizeID, StyleMaterialLinkID, 			
	MaterialCoreItemID, @Dim1TypeSel, @Dim2TypeSel, @Dim3TypeSel
FROM  pStyleBOMTemp WITH (NOLOCK)
WHERE StyleMaterialID = @StyleMaterialID
	AND StyleID = @StyleID
...
VALUES     ('DB_Version', '0.5.0000', '06784', GetDate())
GO
