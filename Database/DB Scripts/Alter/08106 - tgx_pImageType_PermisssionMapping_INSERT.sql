/****** Object:  Trigger [tgx_pImageType_PermisssionMapping_INSERT]    Script Date: 06/26/2014 10:44:49 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tgx_pImageType_PermisssionMapping_INSERT]'))
DROP TRIGGER [dbo].[tgx_pImageType_PermisssionMapping_INSERT]
GO

/****** Object:  Trigger [dbo].[tgx_pImageType_PermisssionMapping_INSERT]    Script Date: 06/26/2014 10:44:50 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[tgx_pImageType_PermisssionMapping_INSERT]
   ON  [dbo].[pImageType]
   AFTER INSERT
AS 
BEGIN
	...
	BEGIN TRY
		DECLARE @ImageFolderTypeID INT = 6
		DECLARE @ImageTypeID UNIQUEIDENTIFIER
		DECLARE @ImageType NVARCHAR(200)
	    ...
	    FOR SELECT ImageTypeID, ImageType, ISNULL(CUser, 'System'), ISNULL(CDate, CURRENT_TIMESTAMP), Active
	    FROM inserted
	    
	    OPEN InsertedRows
	    FETCH NEXT FROM InsertedRows INTO @ImageTypeID, @ImageType, @CUser, @CDate, @Active
	    
		WHILE @@FETCH_STATUS = 0
	    BEGIN
			-- Inserting product type mapping for Image Folder
			IF NOT EXISTS(SELECT * FROM sPermissionProductTypeMap 
							WHERE PermissionFolderTypeId = @ImageFolderTypeID AND GuidProductTypeId = @ImageTypeID)
			BEGIN
				INSERT INTO sPermissionProductTypeMap(PermissionFolderTypeId, GuidProductTypeId)
				VALUES(@ImageFolderTypeID, @ImageTypeID)
			END
			...
			SELECT PermissionFolderID, @ProductTypeMapID, 1, GroupId,
												@CUser, @CDate, @CUser, @CDate, @Active, @ImageType
			FROM sPermissionFolder pf
			WHERE PermissionFolderTypeId = @ImageFolderTypeID 
...
VALUES ('DB_Version', '0.5.0000', '08106', GetDate())
GO