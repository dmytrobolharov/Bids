IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_Style_Report_Change_Log_Summary]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_Style_Report_Change_Log_Summary]
GO
...
AS
SELECT     sh.StyleID, sh.StyleCategory, sh.StyleNo, ISNULL(dbo.pStyleDevelopmentItem.StyleDevelopmentName, 
                      'Variation ' + CAST(dbo.pStyleDevelopmentItem.Variation AS VARCHAR(5))) AS StyleDevelopmentName, ISNULL(sh.Description, '') AS Description, sh.SizeClass,
                          (SELECT     TOP (1) dbo.fnx_GetStreamingImagePathChangeLogReport(dbo.hImage.ImageID, dbo.hImage.Version, N'100') AS Expr1) AS FilePath, 
                      (CASE sh.StyleSet WHEN 1 THEN ISNULL(sh.Pc1, CAST(sh.StyleSet AS VARCHAR(1)) + 'st Set') WHEN 2 THEN ISNULL(sh.Pc2, CAST(sh.StyleSet AS VARCHAR(1)) 
                      + 'nd Set') WHEN 3 THEN ISNULL(sh.Pc3, CAST(sh.StyleSet AS VARCHAR(1)) + 'rd Set') WHEN 4 THEN ISNULL(sh.Pc4, CAST(sh.StyleSet AS VARCHAR(1)) + 'th Set') 
                      END) AS StyleSet, ISNULL(Change.Workflow, '') AS Workflow, Change.ChangeDate
FROM         (SELECT     ChangeTableID, ChangeTablePKID, ChangeUserName, ChangeDate, ChangeLogItemID, ChangeTransID, ChangeLogID, ChangeFieldName, 
                       ...
                       WHERE     (l.ChangeTableID = 'D616FAAE-7ACD-DF11-AF06-005056C00008')) AS Change RIGHT OUTER JOIN
...
VALUES     ('DB_Version', '5.0.0000', '04160', GetDate())
GO