...
BEGIN
	...
	DECLARE @OldSystemScenarioID UNIQUEIDENTIFIER
	SELECT @OldSystemScenarioID = StyleCostingScenarioItemsId FROM pStyleCosting
	WHERE StyleCostingID = @StyleCostingID
	
	-- If we're setting new system costsheet
	IF (@NewSystemScenarioItemID <> @OldSystemScenarioID)
	BEGIN
		...
		SET costing.StyleCostingDate = item.StyleCostingDate,
			...
			costing.StyleCostingCurrency = item.StyleCostingCurrency,
			...
			costing.ExchangeRateItemID = item.ExchangeRateItemID
		...
		AND item.StytleCostingScenarioItemsID = @NewSystemScenarioItemID
	END
	ELSE
	BEGIN -- If we're updating existing costsheet
		UPDATE costing
		SET costing.StyleCostingDate = item.StyleCostingDate,
			costing.StyleCostingComments = item.StyleCostingComments,
			costing.StyleCostingHTS = item.StyleCostingHTS,
...
VALUES ('DB_Version', '0.5.0000', '07059', GetDate())
GO
