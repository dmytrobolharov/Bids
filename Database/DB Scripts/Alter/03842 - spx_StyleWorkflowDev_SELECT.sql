...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_StyleWorkflowDev_SELECT]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[spx_StyleWorkflowDev_SELECT]
GO

CREATE  PROCEDURE [dbo].[spx_StyleWorkflowDev_SELECT]
(
...
AS
	...
	BEGIN
		...
		FROM dbo.pStyleWorkflow a WITH (NOLOCK) 
			...
			INNER JOIN pWorkflow w ON a.WorkflowID = w.WorkflowID
...
VALUES              ('DB_Version', '5.0.0000', '03842', GetDate())
GO
