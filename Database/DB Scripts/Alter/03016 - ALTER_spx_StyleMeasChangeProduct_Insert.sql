...
IF EXISTS (select * from sysobjects where id = object_id('spx_StyleMeasChangeProduct_Insert') and sysstat & 0xf = 4)
    drop procedure spx_StyleMeasChangeProduct_Insert
GO


CREATE PROCEDURE spx_StyleMeasChangeProduct_Insert
(
...
SELECT @OldSampleCol = SizeCol FROM pamlmeaslbl WITH (NOLOCK) WHERE AMLMeasHdrId = @AMLMeasHdrId and IsSample=1
...
Update pamlmeashdr set ProductTypeID=@ProductTypeID, ProductType=@ProductType, MeasRowCnt=(select max(pom_row) from pamlmeaspom where AMLMeasHdrId = @AMLMeasHdrId) 
...
VALUES ('DB_Version', '4.2.0000', '03016', GetDate())
GO
