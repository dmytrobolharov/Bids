...
AS 
	...
	/** PSD -- This causes problems when the color is in the pStyleColorway but not pLineListItemColor. **/
	...
	**/
	...
	IF NOT EXISTS (SELECT LineFolderItemColorID
				   FROM dbo.pLineFolderItemColor
		...
		**/
		...
		BEGIN
			...
			BEGIN
				...
				EXEC spx_StyleSeasonYearTrans_INSERT @StyleID = @StyleID2, @SeasonYearID = @SeasonYearID, 
					@CUser = @CUser, @CDate = @CDate, @ChangeTransID = NULL, @ChangeTransUserID = NULL, @ChangeTransPageName = NULL, @Sort = NULL
				
				SELECT @StyleSeasonYearID2 = StyleSeasonYearID FROM pStyleSeasonYear WHERE StyleID = @StyleID2 AND SeasonYearID = @SeasonYearID
...
VALUES ('DB_Version', '0.5.0000', '07229', GetDate())
GO
