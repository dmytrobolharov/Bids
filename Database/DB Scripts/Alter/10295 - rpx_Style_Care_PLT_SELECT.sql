...
BEGIN
	...
	INSERT INTO #tmpXmlDocTable (Name, Alias, Visible, lookupquery)
	SELECT Name, alias, Visible, lookupquery
	...
	WHERE Visible='true' AND lookupquery IS NOT NULL AND lookupquery <> ''
	...
	WHILE ( @RowCount <= @TotalRowCount )
		BEGIN
			...
			SET @QUERY	=	@QUERY+ '	SET ImagePath=dbo.fnx_GetStreamingCareImagePath(RTRIM('+ @TableName+'.CustomKey), '''+ RTRIM(LTRIM(REPLACE(REPLACE(@Alias,' ',''),'ING',''))) +''', ''150'')'
			...
			SET @QUERY	=	@QUERY+	'	WHERE pStyleCare.styleid='''+CONVERT(NVARCHAR(50),@StyleId)+''''
	...
	INSERT INTO #tempFinalSelect (XmlDocId, Name, Alias, Visible, lookupquery, Imagepath, Expression, CareInstruction, SpecialInstruction)
	SELECT XmlDocId, Name, Alias, Visible, lookupquery, Imagepath, Expression, CareInstruction, SpecialInstruction 
	FROM #tmpXmlDocTable
...
VALUES ('DB_Version', '6.1.0000', '10295', GetUTCDate())
GO
