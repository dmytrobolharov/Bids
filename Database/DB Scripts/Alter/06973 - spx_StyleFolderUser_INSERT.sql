...
IF (SELECT COUNT(*)  FROM pStyleTeam WITH (NOLOCK) WHERE  StyleID = @StyleID AND TeamID = @TeamID AND (@TradePartner = 0 OR @TradePartner = 1 AND SeasonYearID = @SeasonYearID)) = 0
BEGIN
...
VALUES ('DB_Version', '0.5.0000', '06973', GetDate())
GO
