IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vwx_StyleSampleRequest_Search_SEL]'))
...
FROM         dbo.pStyleSeasonYear AS a RIGHT OUTER JOIN    
                      dbo.pStyleHeader AS b ON a.StyleID = b.StyleID INNER JOIN  
                      dbo.pSeasonYear AS c ON c.SeasonYearID = a.SeasonYearID  INNER JOIN  
                      dbo.pStyleDevelopmentItem ON a.StyleID = dbo.pStyleDevelopmentItem.StyleID  
GO

IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='01942'))
BEGIN

	INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
	VALUES     ('DB_Version', '4.1.0000', '01942', GetDate())
