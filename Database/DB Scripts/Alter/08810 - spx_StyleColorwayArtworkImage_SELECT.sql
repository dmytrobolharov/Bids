...
CREATE PROCEDURE [dbo].[spx_StyleColorwayArtworkImage_SELECT]  
(    
...
;WITH StyleMaterials AS (
	SELECT StyleMaterialID, Artwork FROM pStyleMaterials 
	WHERE StyleID = @StyleID AND StyleSet = @StyleSet
	UNION
	SELECT StyleMaterialID, Artwork FROM pStyleBOM
		INNER JOIN pWorkFlowItem ON pStyleBOM.WorkflowItemId = pWorkFlowItem.WorkFlowItemID
	WHERE pStyleBOM.StyleID = @StyleID AND pStyleBOM.StyleSet = @StyleSet AND pWorkFlowItem.StyleSeasonYearID = @StyleSeasonYearID
)
...
SELECT  NEWID() AS [StyleDetailFormID] ,  
 ...
 INNER JOIN StyleMaterials B ON A.StyleMaterialID = B.StyleMaterialID     
...
VALUES ('DB_Version', '6.0.0000', '08810', GetDate())
GO
