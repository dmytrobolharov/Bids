...
SELECT     dbo.pLineFolderItem.LineFolderID, dbo.pLineFolderItem.LineFolderItemID, dbo.pLineFolderItem.LineItemFolder1, dbo.pLineFolderItem.LineItemFolder2, 
                      ...
                      '<img src="' + REPLACE(dbo.fnx_GetStreamingImageSmallPath(dbo.pStyleHeader.DesignSketchID, dbo.pStyleHeader.DesignSketchVersion), 'S=500', 'S=50') + '" alt="" />' AS [Image], dbo.pStyleHeader.DevelopmentID, 
                      dbo.iCustom1.Custom AS DivisionName, dbo.pStyleType.StyleTypeDescription, dbo.pSubCategory.Custom AS SubCategoryName,
                      '<a href="Line_List_Folder_Style.aspx?SID=' + CAST(dbo.pStyleHeader.StyleID AS NVARCHAR(50)) 
                      + '&LFID=' + CAST(dbo.pLineFolder.LineFolderID AS NVARCHAR(50)) + '&LFIID=' + CAST(dbo.pLineFolderItem.LineFolderItemID AS NVARCHAR(50)) 
                      + '">' + dbo.pStyleHeader.StyleNo + '</a>' AS StylePropertyLink                      
FROM         dbo.pLineFolderItem INNER JOIN
                      ...
                      dbo.pWorkFlowItem ON dbo.pStyleBOMDimension.WorkFlowItemID = dbo.pWorkFlowItem.WorkFlowItemID LEFT JOIN
                      ...
                      dbo.pSubCategory ON dbo.pStyleHeader.SubCategoryId = CAST(dbo.pSubCategory.CustomID AS NVARCHAR(200))

GO


INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
VALUES ('DB_Version', '0.5.0000', '08216', GetDate())
GO
