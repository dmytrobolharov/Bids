...
IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE SPECIFIC_NAME = N'spx_pGRMeasurementsPOMAlternates_UpdateLinked')
   DROP PROCEDURE spx_pGRMeasurementsPOMAlternates_UpdateLinked
GO
...
Create PROCEDURE [dbo].[spx_pGRMeasurementsPOMAlternates_UpdateLinked]
...
UPDATE pGRMeasurementsPOMAlternates
  SET	POMCode = @POMCode,
		...
		MUser = @MUser, 
		MDate = @MDate	
WHERE POMId=@POMId AND IsLinked =1

 GO

IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='02091'))
	BEGIN
		INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
		VALUES ('DB_Version', '4.1.0000', '02091', GetDate())
	END
GO