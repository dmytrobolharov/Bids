IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpx_StyleCommitment_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[rpx_StyleCommitment_SELECT]
GO

CREATE PROCEDURE [dbo].[rpx_StyleCommitment_SELECT]
...
BEGIN
	...
	IF @Metric IS NOT NULL AND @Criteria IS NOT NULL AND @Parameter IS NOT NULL AND ISNUMERIC(@Parameter)=1
		BEGIN
			IF @Metric LIKE '%Percent%' SET @Parameter = CAST(CAST(@Parameter AS DECIMAL(18,4))/100 AS nVARCHAR(MAX))
			DECLARE @strSQL nvarchar(max)
			SET @strSQL='SELECT * FROM #CommitmentTMP WHERE ' + @Metric + @Criteria + @Parameter
			EXEC (@strSQL)
		END
	ELSE
...
VALUES ('DB_Version', '6.1.0000', '10188', GetUTCDate())
GO