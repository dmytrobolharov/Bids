IF OBJECT_ID(N'[dbo].[spx_MaterialRequestSubmitColor_PIVOT]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_MaterialRequestSubmitColor_PIVOT]
GO

CREATE PROCEDURE [dbo].[spx_MaterialRequestSubmitColor_PIVOT](
...
SELECT f.MaterialTradePartnerColorID, g.MaterialTradePartnerId, j.MaterialID, 
	...
	CASE  
		WHEN f.MaterialColorImageID IS NULL  
		 THEN 
			'<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0"><TR><TD width="18">'+
			dbo.fnx_GetStreamingColorImageHTML(f.ColorFolderID, f.ColorPaletteID, 16, 'border="0" alt="Open"') +
			'</TD></TR></TABLE>'
		 ELSE  
			'<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0"><TR><TD width="18">'+
			dbo.fnx_GetStreamingImageHTML(f.MaterialColorImageID, f.MaterialColorImageVersion, 16, 'border="0" alt="Open"') +
			'</TD></TR></TABLE>'
...
VALUES ('DB_Version', '6.1.0000', '10371', GetUTCDate())
GO
