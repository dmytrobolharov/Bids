...
CREATE PROCEDURE [dbo].[spx_Planning_SKUHeader_SELECT]
	...
	@PlanningItemID UNIQUEIDENTIFIER = NULL
AS
BEGIN
	WITH SKUDims AS (
	...
	LEFT JOIN pStyleSKUItem ssh 
		...
		INNER JOIN pPlanningItem pli ON wfi.StyleID = pli.StyleID 
			AND pli.PlanningID = @PlanningID AND pli.StyleSeasonYearID = wfi.StyleSeasonYearID 
			ON pli.PlanningItemID = @PlanningItemID 
				AND ssh.PlanningSKUHeaderID = psh.PlanningSKUHeaderID
	WHERE psh.PlanningID = @PlanningID AND psh.SeasonYearID = @SeasonYearID
	ORDER BY psh.Sort
END
...
VALUES     ('DB_Version', '0.5.0000', '08339', GetDate())
GO