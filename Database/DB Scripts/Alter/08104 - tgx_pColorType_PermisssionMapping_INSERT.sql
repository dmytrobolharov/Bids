/****** Object:  Trigger [tgx_pColorType_PermisssionMapping_INSERT]    Script Date: 06/26/2014 10:50:17 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tgx_pColorType_PermisssionMapping_INSERT]'))
DROP TRIGGER [dbo].[tgx_pColorType_PermisssionMapping_INSERT]
GO

/****** Object:  Trigger [dbo].[tgx_pColorType_PermisssionMapping_INSERT]    Script Date: 06/26/2014 10:50:17 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[tgx_pColorType_PermisssionMapping_INSERT]
   ON  [dbo].[pColorType]
   AFTER INSERT
AS 
BEGIN
	...
	BEGIN TRY
		DECLARE @ColorFolderTypeID INT = 7
		
		DECLARE @ColorTypeID INT
		DECLARE @ColorTypeName NVARCHAR(200)
	    ...
	    FOR SELECT ColorTypeID, ColorTypeDescription, ISNULL(CUser, 'System'), ISNULL(CDate, CURRENT_TIMESTAMP), Active
	    FROM inserted
	    
	    OPEN InsertedRows
	    FETCH NEXT FROM InsertedRows INTO @ColorTypeID, @ColorTypeName, @CUser, @CDate, @Active
		WHILE @@FETCH_STATUS = 0
	    BEGIN
			-- Inserting product type mapping for Image Folder
			IF NOT EXISTS(SELECT * FROM sPermissionProductTypeMap 
							WHERE PermissionFolderTypeId = @ColorFolderTypeID AND IntProductTypeId = @ColorTypeID)
			BEGIN
				INSERT INTO sPermissionProductTypeMap(PermissionFolderTypeId, IntProductTypeId)
				VALUES(@ColorFolderTypeID, @ColorTypeID)
			END
			...
			SELECT PermissionFolderID, @ProductTypeMapID, 1, GroupId,
												@CUser, @CDate, @CUser, @CDate, @Active, @ColorTypeName
			FROM sPermissionFolder pf
			WHERE PermissionFolderTypeId = @ColorFolderTypeID 
...
VALUES ('DB_Version', '0.5.0000', '08104', GetDate())
GO