IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_LineFolder_MerchandiseBusiness_PlanningLogic_UPDATE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_LineFolder_MerchandiseBusiness_PlanningLogic_UPDATE]
GO

CREATE PROCEDURE [dbo].[spx_LineFolder_MerchandiseBusiness_PlanningLogic_UPDATE]
...
BEGIN
	...
	FROM pLineFolderMerchandiseBusiness pb WHERE pb.PlanningID = @PlanningID AND pb.SeasonYearID = @SeasonYearID AND pb.LineFolderID = @LineFolderID
	...
	*/
	...
	BEGIN
		...
		FROM (
			SELECT PlanningPlanningCh, PlanningFinancialID FROM pLineFolderMerchandiseBusiness
				WHERE PlanningID = @PlanningID AND PlanningDivisionID = @CurrentDivisionID AND PlanningStyleTypeID = @CurrentStyleTypeID
					...
					AND SeasonYearID = @SeasonYearID AND LineFolderID = @LineFolderID
	...
	FROM (
		SELECT PlanningPlanningCh, PlanningFinancialID FROM pLineFolderMerchandiseBusiness
			WHERE LineFolderID = @LineFolderID AND PlanningID = @PlanningID AND PlanningBrandID IS NOT NULL
				AND PlanningStyleCategoryID IS NOT NULL AND PlanningSubCategoryID IS NULL		
	...
	UPDATE pLineFolderMerchandiseBusiness SET 
		PlanningPlanningCh = CASE PlanningFinancialID 
			...
			WHEN '10000000-0000-0000-0000-000000000028' THEN @GrossMargin
			ELSE PlanningPlanningCh END
	WHERE LineFolderID = @LineFolderID AND PlanningID = @PlanningID AND PlanningBrandID IS NULL
		AND PlanningDivisionID IS NULL AND PlanningStyleTypeID IS NULL
		AND PlanningStyleCategoryID IS NULL AND PlanningSubCategoryID IS NULL			
		AND SeasonYearID = @SeasonYearID
		AND PlanningFinancialID IN (
...
VALUES     ('DB_Version', '0.5.0000', '08279', GetDate())
GO
