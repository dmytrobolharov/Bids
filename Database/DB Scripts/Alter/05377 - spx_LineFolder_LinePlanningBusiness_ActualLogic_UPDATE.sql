/****** Object:  StoredProcedure [dbo].[spx_LineFolder_LinePlanningBusiness_ActualLogic_UPDATE]    Script Date: 03/21/2013 17:42:13 ******/
...
BEGIN
	...
	*/
	...
	BEGIN
		...
		SELECT @WHSUnits = SUM(sci.StyleQuoteItemCustomField2) FROM pSourcingCommitmentItem  sci INNER JOIN @StyleList sl ON sl.StyleID = sci.StyleID
		WHERE sci.StyleSeasonYearID = @StyleSeasonYearID
		
		-- Retail Units
		SELECT @RetailUnits = SUM(sci.StyleQuoteItemCustomField6) FROM pSourcingCommitmentItem  sci INNER JOIN @StyleList sl ON sl.StyleID = sci.StyleID
		WHERE sci.StyleSeasonYearID = @StyleSeasonYearID
		
		-- Avg Cost / Unit
		SELECT @AvgCostUnit = (SUM(sci.StyleQuoteItemCustomField24)/ (SUM(sci.StyleQuoteItemCustomField16))) FROM pSourcingCommitmentItem  sci INNER JOIN @StyleList sl ON sl.StyleID = sci.StyleID
		WHERE sci.StyleSeasonYearID = @StyleSeasonYearID
		
		-- Avg Wholesale Price / Unit
		SELECT @AvgWHSPriceUnit = (SUM(sci.StyleCostingCustomField15 * sci.StyleQuoteItemCustomField2)/ @WHSUnits) FROM pSourcingCommitmentItem  sci INNER JOIN @StyleList sl ON sl.StyleID = sci.StyleID
		WHERE sci.StyleSeasonYearID = @StyleSeasonYearID
		
		-- Avg Retail Price / Unit
		SELECT @AvgRetPriceUnit = (SUM(sci.StyleCostingCustomField16 * sci.StyleQuoteItemCustomField6) / @RetailUnits) FROM pSourcingCommitmentItem  sci INNER JOIN @StyleList sl ON sl.StyleID = sci.StyleID
		WHERE sci.StyleSeasonYearID = @StyleSeasonYearID
		...
		-- Extended Cost
		SELECT @ExtendedCost = SUM(sci.StyleQuoteItemCustomField24) FROM pSourcingCommitmentItem  sci INNER JOIN @StyleList sl ON sl.StyleID = sci.StyleID
		WHERE sci.StyleSeasonYearID = @StyleSeasonYearID
...
VALUES ('DB_Version', '0.5.0000', '05377', GetDate())
GO
