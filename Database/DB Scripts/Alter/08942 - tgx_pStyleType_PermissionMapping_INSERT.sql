/****** Object:  Trigger [tgx_pStyleType_PermissionMapping_INSERT]    Script Date: 08/21/2014 15:22:17 ******/
...
/****** Object:  Trigger [dbo].[tgx_pStyleType_PermissionMapping_INSERT]    Script Date: 08/21/2014 15:22:17 ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN TRY
		...
		BEGIN
			...
			INSERT INTO sPermissionWorkflow (PermissionFolderId, PermissionProductTypeId, PermissionProductTypeMapId, GroupId, WorkflowId,
				PermissionRoleId, PermissionView, PermissionCreate, PermissionRemove, PermissionModify, PermissionDelete, PermissionPrint,
				CUser, CDate, MUser, MDate, PermissionWorkflowName)
			SELECT pf.PermissionFolderId, PermissionProductTypeId, ptm.PermissionProductTypeMapId, pf.GroupId, '70000000-0000-0000-0000-000000000001',
				0, 0, 0, 0, 0, 0, 0,
				@CUser, @CDate, @CUser, @CDate, 'Style Header'
			FROM sPermissionProductType ppt 
			INNER JOIN sPermissionFolder pf ON ppt.PermissionFolderId = pf.PermissionFolderId AND pf.PermissionFolderTypeId = @StyleFolderTypeID
			INNER JOIN sPermissionProductTypeMap ptm ON ppt.PermissionProductTypeMapId = ptm.PermissionProductTypeMapId
			WHERE ptm.PermissionFolderTypeId = @StyleFolderTypeID AND ptm.IntProductTypeId = @StyleTypeID
			AND NOT EXISTS(SELECT * FROM sPermissionWorkflow WHERE PermissionProductTypeId = ppt.PermissionProductTypeId AND WorkflowId = '70000000-0000-0000-0000-000000000001')
		...
		-- Setting parents for bubbles
		EXEC spx_Permissions_WorkflowsGrouping_UPDATE @StyleFolderTypeID
...
VALUES ('DB_Version', '6.0.0000', '08942', GetDate())
GO
