IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_PlanningFlashEdit_CostingProperty_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_PlanningFlashEdit_CostingProperty_SELECT]
GO


CREATE PROCEDURE [dbo].[spx_PlanningFlashEdit_CostingProperty_SELECT]
...
BEGIN
	...
	INTO #tmpStyles	
	FROM vwx_PlanningFlashEdit_CostingProperty_SELECT WHERE PlanningID = @PlanningID AND SeasonYearID = @SeasonYearID
	
	-- calculate NegativeDelta Column
	;WITH StyleCostSum AS (
	SELECT 
		pli.StyleID, sh.DivisionID, sh.StyleType as StyleTypeID, sh.StyleCategory as StyleCategoryID, sh.SubCategoryId
	...
	FROM pPlanningItem pli
		...
		INNER JOIN pPlanningItem pli2 
			INNER JOIN pStyleHeader sh2 ON pli2.StyleID = sh2.StyleID
			INNER JOIN pStyleSeasonYear ssy2 ON pli2.StyleSeasonYearID = ssy2.StyleSeasonYearID
		ON pli.PlanningID = pli2.PlanningID AND ssy.SeasonYearID = ssy2.SeasonYearID
		...
		LEFT JOIN pStyleCostingHeader sch ON pli2.StyleSeasonYearID = sch.StyleSeasonYearID
		WHERE pli.PlanningID = @PlanningID AND ssy.SeasonYearID = @SeasonYearID
	GROUP BY pli.StyleID, sh.DivisionID, sh.StyleType, sh.StyleCategory, sh.SubCategoryId
	), PlannedData AS (
	...
	EXEC('SELECT * FROM #tmpStyles ' + @SearchCond + ' order by PlanningItemSort, StyleNo ' )
...
VALUES ('DB_Version', '0.5.0000', '08074', GetDate())
GO
