IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpx_Style_Sourcing_BOL_TotalCost_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[rpx_Style_Sourcing_BOL_TotalCost_SELECT]
GO
...
CREATE PROCEDURE [dbo].[rpx_Style_Sourcing_BOL_TotalCost_SELECT]
...
-- insert into temp table BOL list for Standard page for different StyleSets and Colorways
...
SELECT s.*, @ExceptionId_Standard, 'Standard', c.StyleColorID, b.SourceBOLId, b.TradePartnerId, b.TradeVendorId, b.Adj AS SourcingAdj
		...
		INNER JOIN pStyleSourcingBOLColor c ON s.StyleID = c.StyleID AND s.StyleSet = c.StyleSet AND @ExceptionId_Standard = c.ExceptionId
...
-- insert data into temp total table and calc sum using group by base rate for different Stylesets, Exceptions, Coloways
INSERT #tmp_bol_list_Total (StyleSet, SetName, ExceptionId, ExceptionName, StyleColorID, Sort, BaseRateId, BaseRateCode, Totaltime, ConvCost, ConvCurr)
	SELECT t.StyleSet, t.SetName, t.ExceptionId, t.ExceptionName, t.StyleColorID, 0 as Sort, t.BaseRateId, 'Type ' + pBOLBaseRateType.RateCode, SUM (Totaltime), 
		CASE
			WHEN (SELECT COUNT(*) FROM #tmp_bol_list as t1
					WHERE t1.BaseRateId = t.BaseRateId
					AND t1.ConvCost IS NULL 
					AND t1.StyleSet = t.StyleSet 
					AND t1.ExceptionId = t.ExceptionId 
					AND t1.StyleColorID = t.StyleColorID) > 0 THEN
				NULL
			ELSE
				SUM(ConvCost)
		END, @ConvCurr
	FROM #tmp_bol_list t LEFT JOIN pBOLBaseRateType
		ON t.BaseRateId = pBOLBaseRateType.BaseRateTypeId
		WHERE MacroId IS NULL
	GROUP BY StyleSet, SetName, ExceptionId, ExceptionName, StyleColorID, BaseRateId, pBOLBaseRateType.RateCode
UNION
	SELECT StyleSet, SetName, ExceptionId, ExceptionName, StyleColorID, 2 as Sort, '00000000-0000-0000-0000-000000000001' as BaseRateId, 'Total' as BaseRateCode, SUM(Totaltime), SUM(ConvCost), @ConvCurr
	FROM #tmp_bol_list WHERE MacroId IS NULL
	GROUP BY StyleSet, SetName, ExceptionId, ExceptionName, StyleColorID
	
UPDATE #tmp_bol_list_Total SET Sort = 1, BaseRateCode = 'Other' 
		WHERE BaseRateId = '00000000-0000-0000-0000-000000000000'
	
UPDATE #tmp_bol_list_Total SET sTotaltime = CAST (Totaltime as nvarchar(50))	
UPDATE #tmp_bol_list_Total SET sConvCost = 
	CASE
		WHEN ConvCost IS NULL THEN 'Missing rate!'
		ELSE CAST (ConvCost as nvarchar(50))
	END

-- select from temp total table
SELECT tt.*, sc.StyleColorName, e.CDate as ExceptionCDate, sc.Sort as StyleColorSort 
FROM #tmp_bol_list_Total tt
	INNER JOIN pStyleBOLException e ON tt.ExceptionId = e.BOLExceptionId
	INNER JOIN pStyleColorway sc ON tt.StyleColorID = sc.StyleColorID
ORDER BY tt.StyleSet ASC, ExceptionCDate, StyleColorSort, sc.StyleColorName, tt.Sort ASC
...
IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='02226'))
	BEGIN
		INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
		VALUES ('DB_Version', '4.1.0000', '02226', GetDate())
	END
GO