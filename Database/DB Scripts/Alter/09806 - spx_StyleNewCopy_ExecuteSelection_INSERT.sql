IF OBJECT_ID(N'[dbo].[spx_StyleNewCopy_ExecuteSelection_INSERT]') IS NOT NULL
...
SELECT @StyleSet = StyleSet, @SizeClassId = SizeClassId, @SizeClass = SizeClass, 
       @SizeRangeId = SizeRangeId, @SizeRange = SizeRange
...
FROM pStyleCopyColorTmp a
	INNER JOIN dbo.pStyleColorwaySeasonYear b ON a.StyleColorwaySeasonYearID =  b.StyleColorwaySeasonYearID
...
BEGIN 
	...
	SELECT @SizeClassId = SizeClassId, @SizeClass = SizeClass, 
	       @SizeRangeId = SizeRangeId, @SizeRange = SizeRange
	FROM dbo.pStyleHeader WITH(NOLOCK)
	...
	INSERT INTO pStyleWorkflow(
		...
		WorkComments, WorkSort, CUser, CDate, MUser, MDate, WorkDay, StyleSeasonYearID, ActualStart)
	SELECT NEWID() AS StyleWorkflowID, @NewStyleID AS StyleID, b.StyleSet, b.WorkflowID, b.WorkflowType, b.WorkflowOrder, b.WorkDate, b.WorkStart, 
		b.WorkDue, b.WorkAssignedTo, 0 AS WorkComplete, 
		--CASE WHEN WorkStatus = 1 THEN WorkStatus ELSE 6 END, 
		CASE WHEN b.WorkflowID = '40000000-0000-0000-0000-000000000006' THEN 
			CASE WHEN (SELECT COUNT(*) FROM pStyleCopyTmp WHERE StyleID = @StyleID AND Map = b.WorkflowID) = 0 THEN 
				1
			ELSE
				6
			END 
		ELSE CASE WHEN WorkStatus = 1 THEN 
			WorkStatus 
		ELSE 6 END END,
		NULL AS WorkStatusDate, NULL  AS WorkStatusBy, 0 AS WorkVersion, 
		b.WorkComments, b.WorkSort, @CUser, @CDate, @CUser, @CDate , b.WorkDay, 
		CASE WHEN m.IsSeasonal = 0 THEN NULL ELSE @StyleSeasonYearID END, CASE WHEN WorkStatus = 1 THEN NULL ELSE GETDATE() END
	FROM dbo.pStyleWorkflow b 
	LEFT JOIN pStyleSeasonYear ssy ON b.StyleSeasonYearID = ssy.StyleSeasonYearID
	LEFT JOIN Mapping m ON b.WorkflowID = m.Map
	WHERE b.StyleID = @StyleID
		AND (m.IsSeasonal = 1 AND ssy.SeasonYearID = @SeasonYearID_Param OR m.IsSeasonal = 0)
	...
	SELECT newid() AS StyleTechPackStatusID, @NewStyleID AS StyleID, @StyleSeasonYearID, WorkStatus, WorkAssignedTo, WorkStart, WorkDue, ActualStart, ActualEnd,
		@CUser AS CUser, @CDate AS CDate, @CUser AS MUser, @CDate AS MDate
	FROM pStyleTechPackStatus stps
		INNER JOIN pStyleSeasonYear ssy ON stps.StyleSeasonYearID = ssy.StyleSeasonYearID
	WHERE stps.StyleID = @StyleID and ssy.SeasonYearID = @SeasonYearID_Param
	...
	BEGIN 
		...
		WHERE StyleID = @StyleID 
			AND StyleSet <= @StyleSet AND (SELECT COUNT(*) FROM pStyleCopyTmp WHERE StyleID = @StyleID AND Map = '40000000-0000-0000-0000-000000000006') <> 0
	END 	
...
BEGIN 
	...
	SELECT tmp.Map, tmp.WorkflowItemID 
	FROM pStyleCopyTmp tmp
	LEFT JOIN pWorkFlowItem wfi ON wfi.WorkFlowItemID = tmp.WorkFlowItemID
	WHERE tmp.TransactionID = @TransactionID 
		AND tmp.StyleID = @StyleID
		AND tmp.Map NOT IN (
    ...
    ORDER BY wfi.WorkflowID, wfi.Sort
	...
	BEGIN
		--** Copy Costing
		EXEC spx_StyleCostingCopy_INSERT @StyleID=@StyleID, @NewStyleID=@NewStyleID, @CreatedBy = @CUser, @CreatedDate = @CDate, @SeasonYearID = @SeasonYearID_Param
	END
...
BEGIN
	...
	SELECT @StyleSeasonYearID = StyleSeasonYearID, @LinePlanItemID = LinePlanItemID FROM pStyleSeasonYear
	...
	BEGIN
		INSERT INTO dbo.pStyleColorwaySeasonYear (
			StyleColorwaySeasonYearID, StyleSeasonYearID ,StyleColorwayID ,StyleID, StyleColorDelivery1, StyleColorDelivery2,
			StyleColorDelivery3, StyleColorDelivery4, StyleColorDelivery5, StyleColorStatus, SampleStatus )
		SELECT NEWID(), @StyleSeasonYearID, a.StyleColorID, @NewStyleID, 1, 1, 1, 1, 1, @ColorStatus, @ColorStatus
...
VALUES ('DB_Version', '6.1.0000', '09806', GetDate())
GO
