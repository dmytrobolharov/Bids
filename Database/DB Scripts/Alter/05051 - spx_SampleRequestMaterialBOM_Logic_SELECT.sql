...
@WorkFlowItemID AS uniqueidentifier
)
AS
...
IF (@SampleWorkflowID = '25A') --License
    BEGIN
        ...
        WHERE pStyleMaterials.StyleId = @StyleID AND pStyleMaterials.StyleSet = @StyleSet AND pStyleMaterials.MaterialTrack = 1 AND
              ...
              AND pStyleMaterials.WorkflowItemId=@WorkFlowItemID AND pStyleMaterials.StyleBOMDimensionId=@StyleBOMDimensionID
...
ELSE IF (@SampleWorkflowID = '20A') --GCC
	BEGIN
		...
		WHERE pStyleMaterials.StyleId = @StyleID AND pStyleMaterials.StyleSet = @StyleSet AND pStyleMaterials.MaterialTrack = 1 AND
			  pSampleRequestWorkflow.SampleRequestWorkflowID = @SampleRequestWorkflowID AND
			  (pStyleMaterials.StyleMaterialID NOT IN(SELECT StyleMaterialID FROM pSampleRequestMaterialBOM WITH (NOLOCK) WHERE SampleRequestWorkflowID = @SampleRequestWorkflowID)) AND
			  (pStyleMaterials.MaterialType NOT IN (SELECT ComponentTypeID FROM pComponentType WITH (NOLOCK) WHERE ComponentType = @ComponentType)) AND
			  (pSampleRequestWorkflow.SampleRequestWorkflowID = @SampleRequestWorkflowID) AND 
...
ELSE
    BEGIN
        ...
        SELECT SampleRequestMaterialID ,  StyleMaterialID  , ItemDim1Id,ItemDim2Id,ItemDim3Id  FROM  pSampleRequestMaterialBOM
        WITH (NOLOCK)          WHERE SampleRequestTradeID = @SampleRequestTradeID
...
SELECT @select = ' SELECT A.MainMaterial, A.StyleID, A.StyleSet, A.MaterialNo, A.MaterialName,
A.MaterialSize FROM pSampleRequestMaterialBOM A WITH (NOLOCK)
LEFT OUTER JOIN pStyleBOM B ON A.StyleMaterialID =B.StyleMaterialID
GROUP BY A.MainMaterial, A.SampleRequestWorkflowID, A.SampleWorkflowID,
A.TradePartnerVendorID, A.StyleID, A.StyleSet, A.StyleMaterialID, A.MaterialNo, A.MaterialName, A.MaterialSize, B.WorkflowItemId, B.StyleBOMDimensionId '

SELECT @subdate = 'MAX(EndDate)'
SELECT @revdate = 'MAX(EndDate)'
SELECT @pivot = 'Submit'
SELECt @table = 'pSampleRequestMaterialBOM '
...
--+ @delim + convert(varchar(100), tb_pivot) + @delim + ' THEN  CONVERT(NVARCHAR(40),A.CDate,101) ELSE ''-----''  '  ) + ', '
+ @delim + convert(varchar(100), tb_pivot) + @delim + ' THEN  A.CDate ELSE NULL  '  ) + ', '

+ '''Apr ' + convert(varchar(100), tb_pivot) + ''' = ' +
stuff(@revdate,charindex( '(', @revdate )+1, 0, ' CASE ' + @pivot + ' WHEN '
--+ @delim + convert(varchar(100), tb_pivot) + @delim + ' THEN  Case Status  when 2 then CONVERT(NVARCHAR(40),EndDate,101)  when 3 then CONVERT(NVARCHAR(40),EndDate,101) when 4  then CONVERT(NVARCHAR(40),EndDate,101) else  ''-----''  END   '        ) + ', '
+ @delim + convert(varchar(100), tb_pivot) + @delim + ' THEN  Case Status  when 2 then EndDate  when 3 then EndDate  when 4 then EndDate else  NULL  END   '        ) + ', '
FROM ##SampleSpec
...
SELECT @selectwhere = ' HAVING  (A.StyleSet =' + '''' + convert(varchar(5),@StyleSet) + ''') 
AND (A.StyleID =' + '''' + convert(varchar(50),@StyleID) + ''')
AND (A.SampleRequestWorkflowID =' + '''' + convert(varchar(50),@SampleRequestWorkflowID) + ''')
AND (A.SampleWorkflowID =' + '''' + convert(varchar(50),@SampleWorkflowID) + ''')
AND B.WorkflowItemId='''+convert(varchar(50),@WorkflowItemId)+''' AND B.StyleBOMDimensionId='''+convert(varchar(50),@StyleBOMDimensionID)+'''
...
BEGIN
    ...
    INSERT INTO #tb ( [Column], Alias, Type, datatype, cellwidth, GridWidth, GridHeaderCss )
    VALUES('MaterialName', 'Material Name', 'label','string',  50, 150, 'TableHeaderBlue' )
    ...
    BEGIN
        ...
        INSERT INTO #tb ( [Column], Alias, Type, datatype, cellwidth, GridWidth, GridHeaderCss )
        VALUES('Sub ' + CAST(@CONT AS NVARCHAR(10)), 'Sub ' + CAST(@CONT AS NVARCHAR(10)), 'label', 'datetime', 50, 70, 'TableHeaderBlue')

        INSERT INTO #tb ( [Column], Alias, Type, datatype, cellwidth, GridWidth, GridHeaderCss )
        VALUES('Apr ' + CAST(@CONT AS NVARCHAR(10)), 'Apr ' + CAST(@CONT AS NVARCHAR(10)), 'label', 'datetime', 50, 70, 'TableHeaderGreen')
...
VALUES ('DB_Version', '5.0.0000', '05051', GetDate())
GO
