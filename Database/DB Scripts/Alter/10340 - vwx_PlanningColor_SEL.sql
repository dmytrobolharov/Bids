IF OBJECT_ID(N'[dbo].[vwx_PlanningColor_SEL]') IS NOT NULL
...
WITH BOMColorsUsed AS (SELECT     cp.ColorPaletteID, pli.PlanningID, ssy.SeasonYearID, CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END AS UsedInBOM
    ...
    SELECT     TOP (100) PERCENT dbo.pPlanningColor.PlanningColorID, dbo.pPlanningColor.PlanningID, dbo.pPlanningColor.CUser, dbo.pPlanningColor.CDate, 
							...
							dbo.fnx_GetStreamingColorImageHTML(dbo.pPlanningColor.ColorFolderID, dbo.pPlanningColor.ColorPaletteID, 25, '') AS ColorImageUrl, 
							dbo.fnx_GetStreamingColorImagePath(dbo.pPlanningColor.ColorFolderID, dbo.pPlanningColor.ColorPaletteID, 25) AS ColorImagePath, 
...
VALUES ('DB_Version', '6.1.0000', '10340', GetUTCDate())
GO
