...
SET @query = N'SELECT SAPCode, PLMCode, StyleColorNo, StyleColorName, ColorFolderID, ColorPaletteID' + ISNULL(', '+@cols,'') + '
		FROM
		(SELECT SAPCode, PLMCode, SeasonYear, Color, StyleColorNo, StyleColorName, ColorFolderID, ColorPaletteID, Sort FROM #tmpStyleColorSeasonYear) p
		PIVOT
		(
...
VALUES ('DB_Version', '0.5.0000', '07500', GetDate())
GO