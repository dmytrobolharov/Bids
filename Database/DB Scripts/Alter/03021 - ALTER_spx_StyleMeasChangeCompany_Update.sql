...
IF EXISTS (select * from sysobjects where id = object_id('spx_StyleMeasChangeCompany_Update') and sysstat & 0xf = 4)
    drop procedure spx_StyleMeasChangeCompany_Update
GO


CREATE PROCEDURE spx_StyleMeasChangeCompany_Update
(
...
select @IsRelative = IsRelative, @IsNegative = IsNegative, @IsMetric = IsMetric from pGRCompany WITH (NOLOCK) where CompanyGuidId=@CompanyID
...
SELECT  @OldProductTypeID = ProductTypeID ,@OldSizeRangeID = SizeRangeID  , @OldSizeClassID = SizeClassID ,
 @OldCompanyID = CompanyID FROM pamlmeashdr WITH (NOLOCK) WHERE AMLMeasHdrId = @AMLMeasHdrId
...
if @MaxPOMRow>0 
	...
	if @RemoveRules=1
		...
		--SELECT @RemoveRules AS RemoveRules
	if @Keepsamplesizemeas=1
		begin
			if @oldSampleCol<>@SampleCol
				begin																				
					--select * FROM #tempsampleData order by pom_row , sizecol
					update pamlmeasigc set Incr = cast(t.Incr	as decimal(18,4))								
	...
	--SELECT @KeepExistingPOM AS KeepExistingPOM
	If @KeepExistingPOM = 0
		begin        
			...
			--select  * FROM #tempIGCData  order by Refcode, POM_Row, sizecol

	  
			SELECT @RowCounter = COUNT(*) FROM #tempIGCData
			
			--SELECT @RowCounter AS RowCounter
			...
			if @RowCounter>0	
				begin				
					...
					--select Refcode, POM_Row, Sizecol, (substring(cast(Sizecol as nvarchar(10)), 5, len(Sizecol)-4)-1) as Sizeno
					...
					--				end						
					--  FROM #tempIGCData order by Refcode, POM_Row, sizecol
					...
					and p.pomalternatesid=t.pomalternatesid and p.refcode=t.refcode and p.POM_Row=t.POM_Row and p.POM_Row <= @MaxPOMRow and p.amlmeashdrid=@AMLMeasHdrId ) 
...
VALUES ('DB_Version', '4.2.0000', '03021', GetDate())
GO
