...
-- modified DueDate selection based on sharing so it won't mess up sorting
DECLARE @selStr NVARCHAR(MAX)
SET @selStr = '
SELECT * FROM (
	SELECT pvt.*
	, CASE WHEN mrsws.MaterialRequestSubmitWorkflowID IS NULL THEN NULL ELSE ISNULL(mrsw.DueDate, ''1900-01-01 00:00:00'') END AS DueDate
	, MaterialRequestWorkflowID + ''_DueDate'' as Date 
	FROM (SELECT MaterialTradePartnerColorID, MaterialTradePartnerId, MaterialID, TradePartnerID, 
	...
	LEFT JOIN pMaterialRequestSubmitWorkflowShare mrsws ON mrsw.MaterialRequestSubmitWorkflowID = mrsws.MaterialRequestSubmitWorkflowID 
		AND mrsws.TradePartnerID = ''' + CAST(@TradePartnerID AS nVARCHAR(50)) + '''
) piv
...
VALUES ('DB_Version', '0.5.0000', '07458', GetDate())
GO
