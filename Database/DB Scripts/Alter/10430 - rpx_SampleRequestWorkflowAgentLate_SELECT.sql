IF OBJECT_ID(N'[dbo].[rpx_SampleRequestWorkflowAgentLate_SELECT]') IS NOT NULL
...
BEGIN
	...
	SELECT
		...
		, sdi.Variation
		, COALESCE(src.SizeRangeCode, sh.SizeRange) AS SizeRange
		...
		, sw.SampleWorkflow
		, srw.DueDate
		, dbo.fnx_DatetimeToString(srw.DueDate, @PrefLanguage) AS DueDateTxt
		, srw.Status
		, CASE
	...
	FROM
		...
		LEFT JOIN pSizeRange src ON src.CustomId = sh.SizeRangeId
	WHERE
		srw.DueDate <= GETDATE()
...
VALUES ('DB_Version', '6.1.0000', '10430', GetUTCDate())
GO
