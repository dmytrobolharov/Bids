...
BEGIN
	if Len(@Imperial)= 0
		begin
			...
			Piece_Buffering,
			Piece_nSplits					 
			from pBodyPatternSheetPiece WHERE Model_Code =@ModelCode AND BodyID = @BodyID AND StyleSet = @StyleSet ORDER BY Piece_Name			
		end		
	else
		begin
			select ID, 
...
VALUES     ('DB_Version', '4.0.0000', '01306', GetDate())
