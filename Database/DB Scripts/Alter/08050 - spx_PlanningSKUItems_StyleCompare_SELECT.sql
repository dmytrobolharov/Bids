IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_PlanningSKUItems_StyleCompare_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_PlanningSKUItems_StyleCompare_SELECT]
GO


CREATE PROCEDURE [dbo].[spx_PlanningSKUItems_StyleCompare_SELECT]
...
BEGIN
	...
	WHERE ((wi.StyleID = @StyleID AND ssi.PlanningSKUHeaderID = @PlanningSKUHeaderID)
		OR (@StyleID IS NULL))
...
VALUES ('DB_Version', '0.5.0000', '08050', GetDate())
GO
