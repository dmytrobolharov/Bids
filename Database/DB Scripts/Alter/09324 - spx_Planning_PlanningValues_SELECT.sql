...
/****** Object:  StoredProcedure [dbo].[spx_Planning_PlanningValues_SELECT]    Script Date: 02/05/2015 16:23:31 ******/
...
BEGIN
	...
	SET @sql = 
		...
		FROM (
			...
			SELECT PlanningPlanningCh, PlanningDivisionID, PlanningStyleTypeID, PlanningStyleCategoryID, 
				...
				(SELECT Active FROM pPlanningBusiness  ppbO WHERE 			
					SeasonYearID = '''  + CAST(@SeasonYearID as NVARCHAR(40)) + '''
						AND ppb.PlanningDivisionID = ppbO.PlanningDivisionID
						AND ppb.PlanningStyleTypeID = ppbO.PlanningStyleTypeID
						AND ppb.PlanningStyleCategoryID = ppbO.PlanningStyleCategoryID
						AND ppb.PlanningBrandID = ppbO.PlanningBrandID
						AND ((ppb.PlanningSubCategoryID is not null and ppb.PlanningSubCategoryID = ppbO.PlanningSubCategoryID) or 
							(ppb.PlanningSubCategoryID is null and ppbO.PlanningSubCategoryID is null))
						AND ppb.PlanningID = ppbO.PlanningID
						AND ppb.PlanningFinancialID = ppbO.PlanningFinancialID
				) AS Active,
				(
					SELECT COUNT(*) FROM pStyleCategoryStyleType scst
					WHERE scst.StyleTypeID = ppb.PlanningStyleTypeID
				) AS CategoriesCount
			FROM pPlanningBusiness ppb WHERE 
			PlanningID = ''' + CAST(@PlanningID as NVARCHAR(40)) + ''' 				
	...
	EXEC('
		INSERT INTO ##tmpPlanningValues (Name, ' + @PlanningFinancialIDList + ', SubCategoriesCount)
		SELECT NULL, ' + @SumColumns + ', 0
	...
	SELECT * FROM ##tmpPlanningValues ORDER BY ChangeLogName
...
VALUES ('DB_Version', '6.1.0000', '09324', GetDate())
GO
