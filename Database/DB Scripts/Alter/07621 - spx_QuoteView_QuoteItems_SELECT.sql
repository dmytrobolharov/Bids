...
BEGIN
	...
	BEGIN
		...
		INNER JOIN sAccessQuotationFolder aqf ON dbo.pStyleHeader.StyleType = aqf.StyleTypeID AND aqf.TeamID = @TeamID
		WHERE (@TradePartnerID IS NULL OR @TradePartnerID IN (p.AgentID, p.VendorID, p.FactoryID))
			AND (aqf.AccessRoleID = 3 OR aqf.AccessView = 1)'
	...
	BEGIN
		...
		WHERE StyleID IN (
			...
			INNER JOIN sAccessQuotationFolder aqf ON sh.StyleType = aqf.StyleTypeID AND aqf.TeamID = @TeamID
...
VALUES ('DB_Version', '0.5.0000', '07621', GetDate())
GO
