/****** Object:  StoredProcedure [dbo].[spx_LinePlanStyleNewLogic_UPDATE]    Script Date: 10/26/2012 17:49:21 ******/
...
BEGIN
	...
	*/
	-- Reworked for enhanced costing 2012
	DECLARE @CostingTypeCursor CURSOR
	DECLARE @CurrentCostingTypeID INT
	DECLARE @NewCostingID UNIQUEIDENTIFIER
	
	SET @CostingTypeCursor = CURSOR SCROLL
	FOR SELECT DISTINCT StyleCostingTypeID FROM pStyleCostingType WHERE Active = 1
	OPEN @CostingTypeCursor
	FETCH NEXT FROM @CostingTypeCursor INTO @CurrentCostingTypeID
	
	WHILE @@FETCH_STATUS = 0
	BEGIN
		
		SELECT @NewCostingID = NEWID()
		
		EXEC spx_StyleCosting_INSERT2
			@StyleCostingID = @NewCostingID,
			@StyleID = @StyleID,
			@StyleCostingTypeID = @CurrentCostingTypeID,
			@StyleCostingDate = @MDate,
			@StyleCostingStatus = 0,
			@CreatedBy = @MUser,
			@CreatedDate = @MDate,
			@StyleSeasonYearID = @StyleSeasonYearID,
			@BOMWorkflowItemID = NULL,
			@BOLWorkflowItemID = NULL
		
		FETCH NEXT FROM @CostingTypeCursor INTO @CurrentCostingTypeID
	
	END
	
	CLOSE @CostingTypeCursor
	DEALLOCATE @CostingTypeCursor
...
BEGIN
	...
	BEGIN
		IF @StyleTypeID = 34 OR @StyleTypeID = 10 OR @StyleTypeID = 36 OR @StyleTypeID = 38 OR @StyleTypeID = 40 OR @StyleTypeID = 42 OR @StyleTypeID = 44
		BEGIN 
			--***
			--** Mens / Womens
			--***	
	...
	BEGIN 
		...
		INSERT INTO #pWorkflowTemplateItem  (WorkflowID, WorkflowTemplateItemID, WorkflowSort, WorkflowDays,WorkflowAssignedTo) 
		SELECT WorkflowID, WorkflowTemplateItemID, WorkflowSort, WorkflowDays,WorkflowAssignedTo
		...
		DECLARE @WorkflowTemplateItemID UNIQUEIDENTIFIER
		...
		BEGIN 
			...
			SELECT @WorkflowID = WorkflowID, @WorkflowTemplateItemID = WorkflowTemplateItemID, @WorkflowSort = WorkflowSort, 
			...
			@WorkflowTemplateItemID = @WorkflowTemplateItemID,
			@PlannedWorkDays = @WorkflowDays,
			@PlannedStartDate = @CurrentDate,
			@PlannedDueDate = @CurrentDate,
			@WorkflowAccessTypeDesc = ''
...
VALUES     ('DB_Version', '5.0.0000', '04263', GetDate())
GO
