/****** Object:  StoredProcedure [dbo].[spx_TACalendar_StyleWorkflowReferences_Progress_UPDATE]    Script Date: 08/21/2014 19:14:40 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_TACalendar_StyleWorkflowReferences_Progress_UPDATE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_TACalendar_StyleWorkflowReferences_Progress_UPDATE]
GO
...
CREATE PROCEDURE [dbo].[spx_TACalendar_StyleWorkflowReferences_Progress_UPDATE]
...
BEGIN
	...
	INNER JOIN pTATaskWorkflow tw ON tw.TATaskWorkflowId = tt.TaskWorkflowId AND UPPER(tw.WorkflowFolder) = 'STYLE'
	...
	AND (tw.WorkflowId = @WorkflowID OR @WorkflowID IS NULL)
	...
	BEGIN
		SELECT @CountAll = COUNT(*) FROM vwx_TACalTemplateTask_ReferenceFromStyle_SELECT WHERE TACalTemplateTaskId = @TACalTemplateTaskID
		SELECT @CountFinished = COUNT(*) FROM vwx_TACalTemplateTask_ReferenceFromStyle_SELECT WHERE TACalTemplateTaskId = @TACalTemplateTaskID AND Ended = 1
...
VALUES ('DB_Version', '6.0.0000', '08940', GetDate())
GO
