/****** Object:  Trigger [tgx_pColorType_PermisssionMapping_DELETE]    Script Date: 06/26/2014 10:50:23 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tgx_pColorType_PermisssionMapping_DELETE]'))
DROP TRIGGER [dbo].[tgx_pColorType_PermisssionMapping_DELETE]
GO

/****** Object:  Trigger [dbo].[tgx_pColorType_PermisssionMapping_DELETE]    Script Date: 06/26/2014 10:50:24 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[tgx_pColorType_PermisssionMapping_DELETE]
   ON  [dbo].[pColorType]
   AFTER DELETE
AS 
BEGIN
	...
	BEGIN TRY
		...
		SELECT ColorTypeID FROM deleted
		...
		BEGIN
			...
			WHERE IntProductTypeId = @DeletedProductTypeID AND PermissionFolderTypeId = @ColorFolderTypeID
...
VALUES ('DB_Version', '0.5.0000', '08103', GetDate())
GO