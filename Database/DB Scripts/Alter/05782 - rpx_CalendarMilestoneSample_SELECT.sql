...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, GETDATE(), cmi.MilestoneDate) < 0 THEN CAST(ABS(DATEDIFF(DAY, GETDATE(), cmi.MilestoneDate)) AS VARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, GETDATE(), cmi.MilestoneDate)) AS VARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
...
INNER JOIN (
	SELECT SampleRequestSubmitID, SampleRequestTradeID, SampleWorkflowID, MAX(Submit) AS Submit FROM pSampleRequestSubmit
	GROUP BY SampleRequestSubmitID, SampleRequestTradeID, SampleWorkflowID UNION
	SELECT SampleRequestSubmitID, SampleRequestTradeID, SampleWorkflowID, MAX(Submit) AS Submit FROM pSampleRequestSubmitBOM
	GROUP BY SampleRequestSubmitID, SampleRequestTradeID, SampleWorkflowID
) srs ON sw.SampleWorkflowID = srs.SampleWorkflowID AND tsw.SampleRequestTradeID = srs.SampleRequestTradeID
WHERE cmi.DependentWorkflowTypeID = '00000000-0000-0000-0000-000000000004' AND cmi.CalendarHeaderID = @CalendarHeaderID
GROUP BY
	  tcs.StyleID
	, tcs.StyleNo
...
VALUES ('DB_Version', '0.5.0.0000', '05782', GetDate())
GO
