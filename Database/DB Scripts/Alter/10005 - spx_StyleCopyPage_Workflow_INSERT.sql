IF OBJECT_ID(N'[dbo].[spx_StyleCopyPage_Workflow_INSERT]') IS NOT NULL
...
END
ELSE IF @Map = '70000000-0000-0000-0000-000000000999' -- Desing Image (copy back image)
BEGIN
	UPDATE pStyleHeader
	SET DesignSketchBackID = (SELECT DesignSketchBackID FROM pStyleHeader WHERE StyleID = @StyleID), 
		DesignSketchBackVersion = (SELECT DesignSketchBackVersion FROM pStyleHeader WHERE StyleID = @StyleID)
...
BEGIN 
	...
	EXECUTE spx_WorkflowItem_Seasonal_COPY 
		@WorkflowID = @Map,
...
VALUES ('DB_Version', '6.1.0000', '10005', GetUTCDate())
GO
