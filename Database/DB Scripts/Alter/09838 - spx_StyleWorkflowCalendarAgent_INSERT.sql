IF OBJECT_ID(N'[dbo].[spx_StyleWorkflowCalendarAgent_INSERT]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_StyleWorkflowCalendarAgent_INSERT]
GO

CREATE PROCEDURE [dbo].[spx_StyleWorkflowCalendarAgent_INSERT]
...
AS 
	...
	SELECT  @CalendarId AS CalendarId, pStyleWorkflow.StyleID, pStyleWorkflow.WorkflowID, Mapping.MapDetail, 
		    pStyleWorkflow.WorkDue, 'Style' AS CalendarType, 
			pStyleWorkflow.WorkStatus, sh.StyleNo + ' (' + COALESCE(sc.Custom, sh.SizeClass) + ') ' + sh.Description AS CalendarDescription
...
VALUES ('DB_Version', '6.1.0000', '09838', GetDate())
GO
