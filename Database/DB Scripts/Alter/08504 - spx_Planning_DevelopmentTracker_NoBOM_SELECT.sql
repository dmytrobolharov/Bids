...
BEGIN
	...
	WHERE
		...
		AND pli.StyleID NOT IN (SELECT StyleID FROM pStyleHeader sh
							INNER JOIN pWorkflowTemplate wt ON sh.WorkflowTemplateID = wt.WorkflowTemplateID
							INNER JOIN pWorkflowTemplateItem wti ON wt.WorkflowTemplateID = wti.WorkflowTemplateID
							WHERE WorkflowID = '40000000-0000-0000-0000-000000000080' AND (wti.WorkflowTemplateID IS NOT NULL
							AND sh.StyleWorkflowID IS NOT NULL OR sh.StyleWorkflowID IS NULL ))
...
VALUES ('DB_Version', '0.5.0000', '08504', GetDate())
GO
