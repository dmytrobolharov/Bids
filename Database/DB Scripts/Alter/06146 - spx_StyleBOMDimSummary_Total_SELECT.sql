/****** Object:  StoredProcedure [dbo].[spx_StyleBOMDimSummary_Total_SELECT]    Script Date: 08/12/2013 11:54:08 ******/
...
BEGIN
	...
	SELECT '   ' AS cellSpace, 'BOM Costing  ' AS [Cost Type], '$' + CAST(CAST(SUM(ISNULL(Qty,0) * ISNULL(MaterialPrice,0)) AS DECIMAL(18,2)) AS VARCHAR(10)) + '' AS [Total Cost], '   ' AS cellSpace1
	FROM         pStyleBOM WHERE StyleBOMDimensionId = @StyleBOMDimensionId AND MaterialBOM = 1
	UNION ALL
	SELECT '   ' AS cellSpace, 'Total Costing  ' AS [Cost Type], '$' +  CAST(CAST(SUM(ISNULL(Qty,0) * ISNULL(MaterialPrice,0)) AS DECIMAL(18,2)) AS VARCHAR(10)) + '' AS [Total Cost], '   ' AS cellSpace1
...
VALUES ('DB_Version', '0.5.0000', '06146', GetDate())
GO
