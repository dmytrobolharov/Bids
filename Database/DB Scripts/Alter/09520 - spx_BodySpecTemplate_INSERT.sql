IF OBJECT_ID(N'[dbo].[spx_BodySpecTemplate_INSERT]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_BodySpecTemplate_INSERT]
GO

CREATE PROCEDURE [dbo].[spx_BodySpecTemplate_INSERT](
...
DECLARE @newSizeRange		nvarchar(200)
DECLARE @newSizeClass		nvarchar(400)
...
BEGIN
	IF @BodySet = 1 UPDATE pBody SET POMTempID1 = @POMTempID, SizeRange = @newSizeRange WHERE BodyID=@BodyID
	IF @BodySet = 2 UPDATE pBody SET POMTempID2 = @POMTempID, SizeRange = @newSizeRange WHERE BodyID=@BodyID
	IF @BodySet = 3 UPDATE pBody SET POMTempID3 = @POMTempID, SizeRange = @newSizeRange WHERE BodyID=@BodyID
	IF @BodySet = 4 UPDATE pBody SET POMTempID4 = @POMTempID, SizeRange = @newSizeRange WHERE BodyID=@BodyID
END
...
BEGIN
	...
	BEGIN
		INSERT INTO pBodySpec
			(BodyID, BodySet, POMTempItemID, POMLibraryID, POMTempID, Critical, POM, PointMeasur, TOLN, TOL, Spec, Grade0, Grade1, Grade2, Grade3, Grade4, Grade5, 
		...
		SELECT @tmpBodyID AS BodyID, @BodySet AS BodySet, POMTempItemID, POMLibraryID, POMTempID, Critical, POM, PointMeasur, TOLN, TOL, Spec, Grade0, Grade1, Grade2, 
			...
			0 AS Size19, 0 AS Size18, 0 AS Size17, HowToMeasurText, HowToMeasurImage, Sort, CDate, CUser, @MDate AS MDate, 
			@MUser AS MUser
		FROM  pPOMTemplateItem
		WHERE (POMTempID = @tmpPOMTempID) AND POM NOT IN (SELECT POM FROM pBodySpec WHERE BodyID = @tmpBodyID AND BodySet = @BodySet)
	END

	BEGIN
		IF @BodySet = 1 UPDATE pBody SET POMTempID1 = @tmpPOMTempID,  SizeRange = @tmpSizeRange WHERE BodyID = @tmpBodyID  
		IF @BodySet = 2 UPDATE pBody SET POMTempID2 = @tmpPOMTempID WHERE BodyID = @tmpBodyID
		IF @BodySet = 3 UPDATE pBody SET POMTempID3 = @tmpPOMTempID WHERE BodyID = @tmpBodyID
		IF @BodySet = 4 UPDATE pBody SET POMTempID4 = @tmpPOMTempID WHERE BodyID = @tmpBodyID
	END
...
VALUES ('DB_Version', '6.1.0000', '09520', GetDate())
GO
