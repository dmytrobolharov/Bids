IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[rpx_SampleRequestWorkflowAgentLate_OLD_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[rpx_SampleRequestWorkflowAgentLate_OLD_SELECT]
GO

CREATE PROCEDURE [dbo].[rpx_SampleRequestWorkflowAgentLate_OLD_SELECT]
...
SELECT  srw.SampleRequestWorkflowID, srw.SampleWorkflowID,
        ...
        dbo.pStyleHeader.SizeClass + ') ' + dbo.pStyleHeader.Description AS LateDescription, REPLACE(srs.Comment, 'span', 'u') AS Comment,
...
FROM    srw INNER JOIN 
        ...
        vwx_TradePartnerRelationshipLevels_SEL tprl ON srt.TradePartnerRelationshipLevelID = tprl.TradePartnerRelationshipLevelID LEFT JOIN 
...
VALUES ('DB_Version', '0.5.0000', '08581', GetDate())
GO
