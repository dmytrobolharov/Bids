IF OBJECT_ID(N'[dbo].[vwx_PlanningImportColor_SEL]') IS NOT NULL
 DROP VIEW [dbo].[vwx_PlanningImportColor_SEL]
GO

CREATE VIEW [dbo].[vwx_PlanningImportColor_SEL]
AS
SELECT
	pssiy.PlanningID
	, pssy.SeasonYearID
	...
	, cp.Sort
	, cf.ColorTypeID
	, dbo.fnx_GetStreamingColorImagePath(pci.ColorFolderID, pci.ColorPaletteID, 25) AS ColorImageUrl
	, dbo.fnx_GetStreamingColorImageHTML(pci.ColorFolderID, pci.ColorPaletteID, 25, '') AS ColorImagePath
...
VALUES ('DB_Version', '6.1.0000', '10341', GetUTCDate())
GO
