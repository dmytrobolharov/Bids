...
/*Select*/
SELECT
	REPLACE(pWorkflowItem.WorkflowItemName,'Specification','Spec') + (SELECT ISNULL(' - ' + REPLACE(REPLACE(ReportPageName, 'Measurement ', ''),'- ',''), '') FROM rReportStylePageItem WHERE ReportPageID = @RPID) AS WorkflowItemName
...
VALUES ('DB_Version', '5.0.0000', '04906', GetDate())
GO
