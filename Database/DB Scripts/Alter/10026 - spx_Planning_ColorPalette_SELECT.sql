...
CREATE PROCEDURE [dbo].[spx_Planning_ColorPalette_SELECT]
	...
	@ColorSeasonYearID UNIQUEIDENTIFIER = NULL
AS
BEGIN
	DECLARE @SqlStr NVARCHAR(MAX)
	
	IF @ColorSeasonYearID IS NOT NULL
	BEGIN
		SET @WhereExp = @WhereExp + ' AND ColorPaletteID IN (SELECT ColorPaletteID FROM pColorPaletteSeasonYear WHERE ColorFolderID = ColorFolderID AND SeasonYearID = ''' + CAST(@ColorSeasonYearID AS NVARCHAR(50)) + ''')'
	END 
	
	IF @PlannigColor = 1
		SET @SqlStr = 'SELECT * INTO #tmp0 FROM (SELECT cp.*, pc.PlanningID FROM pColorPalette AS cp INNER JOIN pPlanningColor pc ON cp.ColorPaletteID = pc.ColorPaletteID WHERE SeasonYearID = ''' + CAST(@SeasonYearID AS NVARCHAR(40)) + ''' AND PlanningColorDrop = 0) a WHERE Active = 1 AND PlanningID = ''' + CAST(@PlanningID AS NVARCHAR(40)) + ''' ' + @WhereExp + ' ORDER BY Sort,L ,H ,S;'
	ELSE
...
VALUES ('DB_Version', '6.1.0000', '10026', GetUTCDate())
GO