...
SELECT * INTO #BOMDetails FROM pSourcingQuotationBOMDetails WHERE StyleQuoteItemID = @StyleQuoteItemID
...
begin
	INSERT INTO [dbo].[pSourcingQuotationBOMDetails]
     ...
     select
           ...
           ,@MDate 
           from #tmpDimItem di
           LEFT JOIN #BOMDetails bd 
			ON  (di.Dim1ID = bd.ItemDim1Id OR bd.ItemDim1Id IS NULL)
			AND (di.Dim2ID = bd.ItemDim2Id OR bd.ItemDim2Id IS NULL)
			AND (di.Dim3ID = bd.ItemDim3Id OR bd.ItemDim3Id IS NULL)
...
VALUES ('DB_Version', '6.1.0000', '10231', GetUTCDate())
GO
