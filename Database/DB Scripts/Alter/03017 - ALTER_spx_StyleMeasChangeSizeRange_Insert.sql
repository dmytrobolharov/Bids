...
IF EXISTS (select * from sysobjects where id = object_id('spx_StyleMeasChangeSizeRange_Insert') and sysstat & 0xf = 4)
    drop procedure spx_StyleMeasChangeSizeRange_Insert
GO


CREATE PROCEDURE spx_StyleMeasChangeSizeRange_Insert
(
...
if @PrimaryKeysOnly=1
	begin
		...
		(ClassRangeId = @SizeRangeID) AND (ProductCompanyId = @ProductTypeID) AND                       
		(IsPrimaryKeyMeas=1))
	end
else
	begin
...
SELECT @ProductType = Product FROM  pGRProductCompany WITH (NOLOCK) where ProductCompanyId = @ProductTypeID

SELECT @SizeRangeName = SizeRange FROM  pGRClassRange WITH (NOLOCK) where ClassRangeId = @SizeRangeID

Update pamlmeashdr set ProductTypeID=@ProductTypeID, SizeRangeID=@SizeRangeID , ProductType=@ProductType,
SizeRange = @SizeRangeName, MeasRowCnt=(select max(pom_row) from pamlmeaspom where AMLMeasHdrId = @AMLMeasHdrId) 
...
VALUES ('DB_Version', '4.2.0000', '03017', GetDate())
GO
