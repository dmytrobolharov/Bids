IF OBJECT_ID(N'[dbo].[spx_StyleDimBOMSummary_SELECT]') IS NOT NULL
...
BEGIN
	...
	BEGIN
		...
		SELECT pStyleBOM.StyleMaterialID, pStyleBOM.AutoColor, pComponentType.ComponentTypeID, pComponentType.ComponentDescription, 
			...
			dbo.fnx_GetStreamingColorImageHTML(pColorPalette.ColorFolderID, pColorPalette.ColorPaletteID, 20, '') AS ColorImage,
			...
			, 0 AS MaterialDel, 
			dbo.fnx_GetStreamingImageHTML(pStyleBOM.MaterialImageID, pStyleBOM.MaterialImageVersion, 25, 
			 'onclick="javascript:var w=window.open(''' + dbo.fnx_GetStreamingImagePath(pStyleBOM.MaterialImageID, pStyleBOM.MaterialImageVersion, 600) + 
			 ''', ''ImageZoom'', ''menubar=0,toolbar=0,status=0,scrollbars=1,location=0,resizable=1,top=1,left=1,height=800,width=800''); w.focus(); return false;"') AS MaterialImage,
	...
	BEGIN
		...
		BEGIN
			...
			SELECT pStyleBOM.StyleMaterialID, pStyleBOM.AutoColor, pComponentType.ComponentTypeID, pComponentType.ComponentDescription, 
				...
				pMaterialsize.MaterialPrice as MPrice,
				pColorPalette.ColorPaletteID, pColorPalette.ColorFolderID,  pColorPalette.ColorCode, pColorPalette.ColorName,
				dbo.fnx_GetStreamingColorImagePath(pColorPalette.ColorFolderID, pColorPalette.ColorPaletteID, '20') AS ColorImage,
				GroupName = case pStyleBOM.MainMaterial when 1 then
								'Main Material'
							else
								pComponentType.ComponentDescription
				...
				, 0 AS MaterialDel, 
				dbo.fnx_GetStreamingImagePath(pStyleBOM.MaterialImageID, pStyleBOM.MaterialImageVersion, 600) AS MaterialImage,
				pStyleHeader.HeaderLocked,
				pStyleBOM.TradePartnerRelationshipLevelID,
...
BEGIN
	...
	BEGIN
		...
		SELECT pStyleBOM.StyleMaterialID, pStyleBOM.AutoColor, pComponentType.ComponentTypeID, pComponentType.ComponentDescription, 
			...
			, 0 AS MaterialDel, 
		    dbo.fnx_GetStreamingImageHTML(pMaterial.MaterialImageID, pMaterial.MaterialImageVersion, 25, 
             'onclick="javascript:var w=window.open(''' + dbo.fnx_GetStreamingImagePath(pMaterial.MaterialImageID, pMaterial.MaterialImageVersion, 600) + 
             ''', ''ImageZoom'', ''menubar=0,toolbar=0,status=0,scrollbars=1,location=0,resizable=1,top=1,left=1,height=800,width=800''); w.focus(); return false;"') AS MaterialImage,
			pStyleHeader.HeaderLocked,
			pStyleBOM.TradePartnerRelationshipLevelID
		FROM pStyleBOM 		
	...
	BEGIN
		...
		BEGIN
			...
			SELECT pStyleBOM.StyleMaterialID, pStyleBOM.AutoColor, pComponentType.ComponentTypeID, pComponentType.ComponentDescription, 
				...
				, 0 AS MaterialDel, 
				dbo.fnx_GetStreamingImagePath(pMaterial.MaterialImageID, pMaterial.MaterialImageVersion, 600) AS MaterialImage,
				pStyleHeader.HeaderLocked,
				pStyleBOM.TradePartnerRelationshipLevelID
			FROM pStyleBOM 
				--INNER JOIN pMaterial ON pStyleBOM.MaterialID = pMaterial.MaterialID
...
VALUES ('DB_Version', '6.1.0000', '10386', GetUTCDate())
GO
