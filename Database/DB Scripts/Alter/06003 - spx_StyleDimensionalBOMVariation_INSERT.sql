...
BEGIN
	...
	BEGIN
		...
		INSERT INTO pWorkFlowItem (WorkFlowItemID, WorkflowID, WorkflowItemTypeId, StyleID, WorkFlowItemName,
									...
									StyleSeasonYearID, WorkflowItemMasterID)
		SELECT @NewWorkflowItemID, WorkflowID, WorkflowItemTypeId, @NewStyleID, WorkFlowItemName,
									wfi.Sort, wfi.CUser, wfi.CDate, wfi.MUser, wfi.MDate, StyleSet, RepRowGuidColID, StatusID, 
									WorkStart, WorkDue, WorkAssignedTo, WorkStatus, WorkEscalateTo, 
									ssy2.StyleSeasonYearID, (SELECT MasterID FROM @MasterIDs m WHERE m.OldWorkFlowItemMasterID = wfi.WorkflowItemMasterID)
		FROM pWorkFlowItem wfi
		LEFT JOIN pStyleSeasonYear ssy ON wfi.StyleSeasonYearID = ssy.StyleSeasonYearID
		LEFT JOIN pStyleSeasonYear ssy2 ON ssy.SeasonYearID = ssy2.SeasonYearID AND ssy2.StyleID = @NewStyleID
...
VALUES ('DB_Version', '0.5.0000', '06003', GetDate())
GO
