...
DECLARE @Cnt int
SELECT @Cnt = COUNT(*) from sys.objects where name =  'ColorLibraryPantonePNCoated' and type = 'U'
IF @Cnt =  0
BEGIN 

	CREATE TABLE [dbo].[ColorLibraryPantonePNCoated](
		[BookOrder] [int] NOT NULL,
		[Page] [float] NULL,
		[PageRow] [float] NULL,
		[PageColumn] [float] NULL,
	 ...
	 CONSTRAINT [PK_ColorLibraryPantonePNCoated] PRIMARY KEY CLUSTERED 
	(
		[BookOrder] ASC
	)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
	) ON [PRIMARY]

	ALTER TABLE [dbo].[ColorLibraryPantonePNCoated] ADD  CONSTRAINT [DF_ColorLibraryPantonePNCoated_PANTONEType]  DEFAULT (N'PANTONE Pastels & Neons COATED') FOR [PANTONEType]
	ALTER TABLE [dbo].[ColorLibraryPantonePNCoated] ADD  CONSTRAINT [DF_ColorLibraryPantonePNCoated_Active]  DEFAULT ((1)) FOR [Active]
	ALTER TABLE [dbo].[ColorLibraryPantonePNCoated] ADD  CONSTRAINT [DF_ColorLibraryPantonePNCoated_ColorLibraryTypeID]  DEFAULT ('00000000-0000-0000-0000-0000000000g)') FOR [ColorLibraryTypeID]
...
VALUES ('DB_Version', '5.0.0000', '05008', GetDate())
GO
