...
WITH BOMColorsUsed AS (SELECT     cp.ColorPaletteID, pli.PlanningID, ssy.SeasonYearID, CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END AS UsedInBOM
    ...
    SELECT     TOP (100) PERCENT dbo.pPlanningColor.PlanningColorID, dbo.pPlanningColor.PlanningID, dbo.pPlanningColor.CUser, dbo.pPlanningColor.CDate, 
                            ...
                            + '&CPID=' + CAST(ISNULL(dbo.pPlanningColor.ColorPaletteID, '00000000-0000-0000-0000-000000000000') AS NVARCHAR(50)) AS ColorImageUrl, '<img src=''' +
                                (SELECT     TOP (1) AppSettingValue
                                  FROM          dbo.sAppSetting
                                  WHERE      (AppSettingKey = 'ImageServer')) + '/ColorStream.ashx?S=25&CFID=' + CAST(ISNULL(dbo.pPlanningColor.ColorFolderID, 
                            '00000000-0000-0000-0000-000000000000') AS NVARCHAR(50)) + '&CPID=' + CAST(ISNULL(dbo.pPlanningColor.ColorPaletteID, '00000000-0000-0000-0000-000000000000') 
                            AS NVARCHAR(50)) + '''  />' AS ColorImagePath, dbo.pPlanningColor.SeasonYearID, dbo.pColorFolder.ColorFolderDescription, dbo.pColorType.ColorTypeDescription, 
...
VALUES ('DB_Version', '6.1.0000', '09408', GetDate())
GO