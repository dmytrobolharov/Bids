...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF (SELECT UPPER(AppSettingValue) FROM sAppSetting WHERE AppSettingKey = 'AllowSizeClassColorways' AND ConfigFileName = @ConfigFile) = 'TRUE'
				SET @AllSizeClasses = 1
				
			SELECT @SeasonYearID = pStyleSeasonYear.SeasonYearID FROM pPlanningItem
				INNER JOIN pStyleSeasonYear ON pPlanningItem.StyleID = pStyleSeasonYear.StyleSeasonYearID
...
VALUES     ('DB_Version', '0.5.0000', '06717', GetDate())
GO
