/****** Object:  View [dbo].[vwx_TACalTemplateTask_ReferenceFromMaterialRequest_SELECT]    Script Date: 08/12/2014 15:58:41 ******/
...
AS
	...
	FROM
	(
		SELECT crt.TACalRefTaskId, 'Material Request' as RefType, pm.MaterialID, tw.WorkflowId, ct.SeasonYearId, 
		pm.MaterialNo, pm.MaterialName, pm.MaterialType, mtp.MaterialTradePartnerID, 
		CASE WHEN MAX(CASE WHEN mrsw.StartDate IS NULL THEN 1 ELSE 0 END) = 0
			THEN MAX(mrsw.StartDate) 
		END AS ActualStart, 
		CASE WHEN MAX(CASE WHEN mrsw.EndDate IS NULL THEN 1 ELSE 0 END) = 0
			THEN MAX(mrsw.EndDate) 
		END AS ActualEnd, 
		crt.TaskRevisedlStart, crt.TaskRevisedEnd,
		crt.TACalTemplateTaskId, ctt.TaskPlanStart, ctt.TaskPlanEnd, 
		...
		INNER JOIN pMaterialRequestSubmitWorkflow mrsw ON mrsw.MaterialTradePartnerID = mtp.MaterialTradePartnerId AND mrw.MaterialRequestWorkflowID = mrsw.MaterialRequestWorkflowID
		...
		GROUP BY crt.TACalRefTaskId, pm.MaterialID, tw.WorkflowId, ct.SeasonYearId, pm.MaterialNo, pm.MaterialName, pm.MaterialType, mtp.MaterialTradePartnerID,  crt.TaskRevisedlStart, crt.TaskRevisedEnd,
		crt.TACalTemplateTaskId, ctt.TaskPlanStart, ctt.TaskPlanEnd, tpm.AgentID, tpm.AgentName, tpm.VendorID, tpm.VendorName, tpm.FactoryID, tpm.FactoryName
	) AS DataSource
...
VALUES ('DB_Version', '0.5.0000', '08751', GetDate())
GO
