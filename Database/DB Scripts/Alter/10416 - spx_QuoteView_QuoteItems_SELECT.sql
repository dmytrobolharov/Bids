...
BEGIN
	...
	BEGIN
		...
		'SELECT DISTINCT ''<img src="../System/Icons/'' + dbo.pStyleQuoteItemStatus.CustomIcon + ''" />'' AS ImagePath
			...
			, COALESCE(sc.Custom, dbo.pStyleHeader.SizeClass) AS SizeClass
			, COALESCE(sr.SizeRangeCode, dbo.pStyleHeader.SizeRange) AS SizeRange
		...
		INNER JOIN fnx_Permissions_GetIntProductTypePermissions(@TeamID, 4) access ON access.ProductTypeID = dbo.pStyleHeader.StyleType
	    LEFT JOIN pSizeClass sc ON sc.CustomId = dbo.pStyleHeader.SizeClassId
	    LEFT JOIN pSizeRange sr ON sr.CustomId = dbo.pStyleHeader.SizeRangeId
	...
	BEGIN
		...
		WHERE StyleID IN (
			...
			INNER JOIN pStyleHeader ON vsqt.StyleID = dbo.pStyleHeader.StyleID
			INNER JOIN fnx_Permissions_GetIntProductTypePermissions(@TeamID, 4) access ON access.ProductTypeID = dbo.pStyleHeader.StyleType
...
VALUES ('DB_Version', '6.1.0000', '10416', GetUTCDate())
GO