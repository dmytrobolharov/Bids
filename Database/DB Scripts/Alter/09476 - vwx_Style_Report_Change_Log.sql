IF OBJECT_ID(N'[dbo].[vwx_Style_Report_Change_Log]') IS NOT NULL
 DROP VIEW [dbo].[vwx_Style_Report_Change_Log]
GO

CREATE VIEW [dbo].[vwx_Style_Report_Change_Log]
AS
SELECT sh.StyleNo, ISNULL(sh.Description, '') AS Description, COALESCE(sc.Custom, sh.SizeClass) AS SizeClass, sh.StyleCategory, (CASE sh.StyleSet WHEN 1 THEN ISNULL(sh.Pc1, 
                      CAST(sh.StyleSet AS nVARCHAR(1)) + 'st Set') WHEN 2 THEN ISNULL(sh.Pc2, CAST(sh.StyleSet AS nVARCHAR(1)) + 'nd Set') WHEN 3 THEN ISNULL(sh.Pc3, 
                      CAST(sh.StyleSet AS nVARCHAR(1)) + 'rd Set') WHEN 4 THEN ISNULL(sh.Pc4, CAST(sh.StyleSet AS nVARCHAR(1)) + 'th Set') END) AS StyleSet, Change.Workflow, 
                      ...
                      'Variation ' + CAST(dbo.pStyleDevelopmentItem.Variation AS nVARCHAR(5))) AS StyleDevelopmentName,
                      ...
                      Change.ChangeFieldAlias, Change.ChangeFieldName, ChangeTransTypeID, sh.DivisionID, sh.StyleType
FROM         (SELECT     ChangeTableID, ChangeTablePKID, ChangeUserName, ChangeDate, ChangeLogItemID, ChangeTransID, ChangeLogID, ChangeFieldName, 
                                              ...
                                              '' AS WorkStatus, '' AS WorkflowItemID, ChangeTablePKID AS StyleID, ChangeTransTypeID
                       ...
                       SELECT     cli.ChangeTableID, cli.ChangeTablePKID, cli.ChangeUserName, cli.ChangeDate, cli.ChangeLogItemID, cli.ChangeTransID, cli.ChangeLogID, 
                                             ...
                                             cli.ChangeSessionNo, cli.ChangeTransNo, cli.WorkflowID, cli.Workflow, cli.ChangeUserID,
                                                 (SELECT     WorkflowStatus
                                                   ...
                                                   WHERE      (WorkflowStatusID = wi.WorkStatus)) AS WorkStatus, ISNULL(CAST(wi.WorkFlowItemID AS nvarchar(255)), '') AS WorkflowItemID, 
                                             wi.StyleID, ChangeTransTypeID
                       ...
                       SELECT     l.ChangeTableID, l.ChangeTablePKID, l.ChangeUserName, l.ChangeDate, l.ChangeLogItemID, l.ChangeTransID, l.ChangeLogID, l.ChangeFieldName, 
                                             ...
                                             l.Workflow, l.ChangeUserID, '' AS WorkStatus, '' AS WorkflowItemID, c.StyleId AS StyleID, ChangeTransTypeID
                       ...
                       SELECT     l.ChangeTableID, l.ChangeTablePKID, l.ChangeUserName, l.ChangeDate, l.ChangeLogItemID, l.ChangeTransID, l.ChangeLogID, l.ChangeFieldName, 
                                             l.ChangeFieldAlias, l.ChangeBeforeValue, l.ChangeAfterValue, l.ChangeBeforeText, l.ChangeAfterText, l.ChangeSort, l.Custom_SMat_Name, 
                                             l.Custom_SMat_Type, l.Custom_SMat_No, l.ChangeTransTablePKID, l.ActionType, l.ChangeSessionID, l.ChangeSessionNo, l.ChangeTransNo, l.WorkflowID, 
                                             l.Workflow, l.ChangeUserID, '' AS WorkStatus, '' AS WorkflowItemID, sm.StyleID, ChangeTransTypeID
                       FROM         dbo.vwx_ChangeLogItem_SEL AS l INNER JOIN
                      ...
                      dbo.hImage ON sh.DesignSketchID = dbo.hImage.ImageID AND sh.DesignSketchVersion = dbo.hImage.Version LEFT JOIN 
					  pSizeClass sc ON sc.CustomID = sh.SizeClassId
...
VALUES ('DB_Version', '6.1.0000', '09476', GetDate())
GO
