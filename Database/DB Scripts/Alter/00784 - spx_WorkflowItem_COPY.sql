...
BEGIN
	...
	IF @@Error <> 0 OR @@RowCount <> 1 GOTO Error
		...
		END
			...
			select @oldMeashdrId = AMLMeasHdrId from pamlmeashdr where StyleID = @CopyStyleID
			AND WorkflowID = @WorkflowID AND WorkflowItemID = @WorkflowItemID AND StyleSet = @CopyStyleSet			
...
VALUES     ('DB_Version', '4.0.0000', '784', GetDate())
