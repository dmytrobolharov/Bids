IF OBJECT_ID(N'[dbo].[spx_SampleRequestActivityCalendarAgent_INSERT]') IS NOT NULL
...
SELECT @CalendarId AS CalendarId,
	...
	END + '<BR>' + sh.StyleNo + ' (' + COALESCE(scc.Custom, sh.SizeClass) + ') ' + sh.Description + '</span>' AS CalendarDescription,
    ...
    ssy.SeasonYearID, sh.CustomField3, sh.CustomField10, COALESCE(scc.Custom, sh.SizeClass) AS SizeClass, 
	COALESCE(sr.SizeRangeCode, sh.SizeRange) AS SizeRange, srw.AssignedTo
INTO #tmpCal
FROM srt
...
INNER JOIN fnx_Permissions_GetIntProductType_WorkflowPermissions(@TeamId, 5, NULL) access ON sw.SampleWorkflowGUID = access.WorkflowId AND sh.StyleType = access.ProductTypeID 
...
VALUES ('DB_Version', '6.1.0000', '09679', GetDate())
GO
