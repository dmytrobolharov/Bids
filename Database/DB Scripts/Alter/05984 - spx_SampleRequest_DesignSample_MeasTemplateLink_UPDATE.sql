/****** Object:  StoredProcedure [dbo].[spx_SampleRequest_DesignSample_MeasTemplateLink_UPDATE]    Script Date: 07/02/2013 15:47:51 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_SampleRequest_DesignSample_MeasTemplateLink_UPDATE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_SampleRequest_DesignSample_MeasTemplateLink_UPDATE]
GO
...
CREATE PROCEDURE [dbo].[spx_SampleRequest_DesignSample_MeasTemplateLink_UPDATE]
...
Begin
	select @POMTempItemID=GRPOMCompanyId from pSampleRequestAMLMeasPOM where SampleRequestAmlmeaspomid=@Amlmeaspomid and SampleRequestAmlmeasHdrid=@AmlmeasHdrid 
	
	IF @Linked = 1
		BEGIN			
			update pSampleRequestamlmeaspom set IsLinked=1, refCode=a.POMCode, 
			...
			critical=a.Critical
			from pSampleRequestamlmeaspom p INNER JOIN pGRMeasurementsPOMTemplateItem a ON p.POMAlternatesID = a.POMAlternatesId
			and p.GRPOMCompanyId = a.POMTempItemID and a.POMTempItemID=@POMTempItemID
			where SampleRequestAmlmeaspomid=@Amlmeaspomid and SampleRequestAmlmeasHdrid=@AmlmeasHdrid
			...
			SELECT @MaxSizeCol = count(SizeCol) FROM pSampleRequestamlmeaslbl WITH (NOLOCK) WHERE SampleRequestAMLMeasHdrId = @AMLMeasHdrId
			select @samplecol = sizecol from pSampleRequestamlmeaslbl WITH (NOLOCK) WHERE SampleRequestAMLMeasHdrId = @AMLMeasHdrId and IsSample=1
			SELECT @IsRelative = IsRelative, @IsNegative = IsNegative, @IsMetric = IsMetric FROM pSampleRequestamlmeashdr WITH (NOLOCK) WHERE SampleRequestAMLMeasHdrId = @AMLMeasHdrId
			set @SizeCnt = 0
			set @IncrCols = ''
			set @IncrNullCols = '' 
			set @IncrCol = ''
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						AND SampleRequestAMLMeasPOMId = @Amlmeaspomid
						FETCH NEXT FROM @TempTableCursor INTO @CurrentSizeCol
					END
				END
				ELSE
...
VALUES ('DB_Version', '0.5.0000', '05984', GetDate())
GO
