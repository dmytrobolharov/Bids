IF OBJECT_ID(N'[dbo].[spx_StyleBOMPending_INSERT]') IS NOT NULL
...
BEGIN TRY
	BEGIN TRANSACTION
		...
		IF @@ROWCOUNT != 0
		BEGIN
			-- update style workflow item stautus
			UPDATE pWorkFlowItem 
			SET WorkStatus = 6, ActualStart = @CreatedDate
			WHERE WorkFlowItemID = @WorkflowItemID AND WorkStatus = 1
			-- update style workflow stautus
			UPDATE pStyleWorkflow
			SET WorkStatus = 6, ActualStart = @CreatedDate
			WHERE StyleId = @StyleID AND WorkFlowID = @WorkflowID AND StyleSet = @StyleSet AND 
				  StyleSeasonYearID = (SELECT StyleSeasonYearID FROM pWorkFlowItem WHERE WorkFlowItemID = @WorkflowItemID) AND 
				  WorkStatus = 1
		END	    
		...
		BEGIN
			...
			WHERE StyleID = @StyleID AND StyleSet = @StyleSet AND 
				  StyleMaterialID <> @NewStyleMaterialID AND 
				  @WorkflowItemId = WorkflowItemId AND WorkflowId = @WorkflowId
		END 
...
VALUES ('DB_Version', '6.1.0000', '09303', GetDate())
GO
