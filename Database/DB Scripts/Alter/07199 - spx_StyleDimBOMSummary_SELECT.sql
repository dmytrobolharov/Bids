...
BEGIN
	...
	BEGIN
		...
		SELECT pStyleBOM.StyleMaterialID, pStyleBOM.AutoColor, pComponentType.ComponentTypeID, pComponentType.ComponentDescription, 
			...
			MaterialPrice = CASE WHEN pMaterialTradePartnerColor.MaterialTradePartnerColorID IS NOT NULL THEN 
								CASE WHEN ISNUMERIC(pMaterialTradePartnerColor.MaterialTradeColor8) = 1 THEN CAST(pMaterialTradePartnerColor.MaterialTradeColor8 AS DECIMAL(18,2)) END 
								ELSE CASE WHEN 1 IN (pStyleBOM.Dim1TypeSel, pStyleBOM.Dim2TypeSel, pStyleBOM.Dim3TypeSel) THEN
										ISNULL(pStyleBOMItem.MaterialPrice, '0.00')
										ELSE pStyleBOM.MaterialPrice 
										END
								END, 
			pStyleBOM.Ext, 
	...
	BEGIN
		...
		BEGIN
			...
			SELECT pStyleBOM.StyleMaterialID, pStyleBOM.AutoColor,pComponentType.ComponentTypeID, pComponentType.ComponentDescription, 
				...
				pStyleBOM.StyleID, pStyleBOM.WorkFlowId,pStyleBOM.WorkflowItemID,pStyleBOM.StyleBOMDimensionId,
				pStyleBOM.Dim1TypeSel, pStyleBOM.Dim2TypeSel, pStyleBOM.Dim3TypeSel,
				pStyleBOM.UOM,UOM.UOM as "UOMValue", MaterialColorId=COALESCE(pStyleBOM.MaterialColorId,'00000000-0000-0000-0000-000000000000'), CAST(ISNULL(pStyleBOM.Qty,0) AS DECIMAL(18,5)) AS Qty, 
				MaterialPrice = CASE WHEN pMaterialTradePartnerColor.MaterialTradePartnerColorID IS NOT NULL 
									THEN CAST(pMaterialTradePartnerColor.MaterialTradeColor8 AS DECIMAL(18,2)) 
									ELSE pStyleBOM.MaterialPrice 
								END, 
				pStyleBOM.Ext, 
				pStyleBOM.MaterialSize, pStyleBOM.MaterialID, pStyleBOM.MaterialPlacement, 
...
VALUES ('DB_Version', '0.5.0000', '07199', GetDate())
GO
