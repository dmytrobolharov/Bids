...
IF NOT EXISTS (select * from pControlPanel WHERE UPPER(ControlPanelName) = 'DATA DICTIONARY')
BEGIN
    ...
    SELECT COALESCE(MAX(ControlPanelID),0) + 1, 
           ...
           'Control_DataDictionary_Default.xml', 
           'Control_DataDictionary_Search.xml', 
           'dDataDictionary', 
           'Control_DataDictionary_List.aspx?CPID=' + convert(varchar(12), COALESCE(MAX(ControlPanelID),0) + 1), 
           0,
...
VALUES              ('DB_Version', '4.1.0000', '01681', GetDate())
GO
