/****** Object:  StoredProcedure [dbo].[spx_StyleBOMDimSummary_Total_SELECT]    Script Date: 11/15/2013 11:29:44 ******/
...
BEGIN
	...
	SELECT '   ' AS cellSpace, 'BOM Costing  ' AS [Cost Type], '$' + CAST(CAST(SUM(ISNULL(Dim_Qty,0) * isnull(MaterialPrice,VolumePrice)) AS DECIMAL(18,2)) AS nVARCHAR(10)) + '' AS [Total Cost], '   ' AS cellSpace1
	FROM         #temp_BOM WHERE MaterialBOM = 1
	UNION ALL
	SELECT '   ' AS cellSpace, 'Total Costing  ' AS [Cost Type], '$' +  CAST(CAST(SUM(ISNULL(Dim_Qty,0) * isnull(MaterialPrice,VolumePrice)) AS DECIMAL(18,2)) AS nVARCHAR(10)) + '' AS [Total Cost], '   ' AS cellSpace1
	FROM         #temp_BOM
...
VALUES     ('DB_Version', '0.5.0000', '06599', GetDate())
GO
