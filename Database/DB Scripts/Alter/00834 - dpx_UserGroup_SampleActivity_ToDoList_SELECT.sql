...
-- =============================================
ALTER                           PROCEDURE [dbo].[dpx_UserGroup_SampleActivity_ToDoList_SELECT]
...
-- To have dummy image file to fill the report cells that replaces the duplcated style image
-- Without this dummy image the border lines of the cells will be cut
...
IF @SortBy = 1  -- by user name / default
    BEGIN 
		...
		SELECT
			...
			,f.SampleWorkflow
			,w.DueDate
			,0  as DaysLate -- temporary. Will be modified at the next step
		...
		GROUP BY w.AssignedTo, u.FirstName, u.LastName, h.StyleNo, h.StyleID,  h.[Description], h.SizeClass, w.StyleSet, 
		...
		UPDATE #TempSampleActivity SET FilePath=@ImageFolder + '/blank_jpeg.JPG' 
		WHERE imageShowFlag <> 1 or LTRIM(RTRIM(FilePath)) IS NULL or FilePath = ''
...
IF @SortBy = 2 -- by date
    BEGIN 
		...
		GROUP BY w.DueDate, w.AssignedTo, u.FirstName, u.LastName, h.StyleNo, h.StyleID,  h.[Description], h.SizeClass, w.StyleSet, 
			w.TradePartnerID, ut.TradePartnerName, w.TradePartnerVendorID, uv.VendorName 
        			, ps.StatusID
			, ps.Status, f.SampleWorkflowID, f.SampleWorkflow
        		,w.DueDate, w.Status, h.CustomField4, 
...
IF @SortBy = 3 -- by dayslate
	...
	BEGIN 
		...
		GROUP BY DATEDIFF(day, GETDATE(), w.DueDate), w.AssignedTo, u.FirstName, u.LastName, h.StyleNo, h.StyleID,  h.[Description], h.SizeClass, w.StyleSet, 
			w.TradePartnerID, ut.TradePartnerName, w.TradePartnerVendorID, uv.VendorName 
        			, ps.StatusID
			, ps.Status, f.SampleWorkflowID, f.SampleWorkflow
        		,w.DueDate, w.Status, h.CustomField4, 
...
IF @SortBy = 4 -- by category
	...
	BEGIN 
		...
		GROUP BY 
			...
			u.FirstName, u.LastName,  h.StyleNo, h.StyleID,  h.[Description], h.SizeClass, w.StyleSet, 
			w.TradePartnerID, ut.TradePartnerName, w.TradePartnerVendorID, uv.VendorName
        			,ps.StatusID
			, ps.Status, f.SampleWorkflowID, f.SampleWorkflow
        		,w.DueDate, w.Status, h.CustomField4, 
...
VALUES     ('DB_Version', '4.0.0000', '834', GetDate())
GO