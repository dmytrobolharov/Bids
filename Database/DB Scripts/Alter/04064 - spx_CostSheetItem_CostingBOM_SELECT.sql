...
BEGIN
    ...
    SELECT sysCol.StyleMaterialId, sysCol.StytleCostingScenarioItemsID, sysCol.MaterialName, sysCol.MaterialNo, 
			...
			secondcol.MaterialCost AS Cost2, thirdcol.MaterialCost AS Cost3, 0 AS IsGroup, (SELECT COUNT(*) FROM pStyleBOM bom WHERE bom.StyleMaterialID = sysCol.StyleMaterialId) As IsCopied
	...
	AND sysCol.StyleMaterialId IS NOT NULL
	UNION SELECT DISTINCT -- Selecting all the material types with it descriptions
			...
			1 AS IsGroup,
			(SELECT COUNT(*) FROM pComponentType component WHERE component.ComponentTypeID = outSel.MaterialType) As IsCopied
...
VALUES     ('DB_Version', '5.0.0000', '04064', GetDate())
