...
CREATE PROCEDURE [dbo].[spx_CostSheetItem_CostingUnits_Multi_SELECT]
	@CostingComparisonId VARCHAR(50),
	@CostingXMLData XML
AS
BEGIN
	...
	DECLARE @ListOfCostingScenarios VARCHAR(MAX)
	
	SELECT @ListOfCostingScenarios = COALESCE(@ListOfCostingScenarios + ',', '') + 
	   CAST(StyleCostingScenarioID AS VARCHAR(50))
	FROM pStyleCostingComparisonTemp
	WHERE StyleCostingComparisonID = @CostingComparisonId
	ORDER BY ID
	...
	SELECT ColumnName FROM #tempUnitStyle WHERE UPPER(ColumnVisible) = 'TRUE' ORDER BY ColumnOrder
...
VALUES     ('DB_Version', '5.0.0000', '04102', GetDate())
GO
