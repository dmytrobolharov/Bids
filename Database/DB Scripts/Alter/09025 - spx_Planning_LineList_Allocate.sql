...
BEGIN
	...
	/***  check if all required line list configs exist  ***/	
	DECLARE @CountOfStyleWithoutConfig INT 
	IF @ConfigLevel = 1
	BEGIN
		SELECT @CountOfStyleWithoutConfig = COUNT(*) FROM #tmpAvailableStyles
			LEFT JOIN #tmpExistingPlanningConfigurations 
			ON #tmpAvailableStyles.DivisionID = #tmpExistingPlanningConfigurations.DivisionID
			WHERE #tmpExistingPlanningConfigurations.PlanningConfigID IS NULL
	END
	ELSE IF @ConfigLevel = 2
	BEGIN
		SELECT @CountOfStyleWithoutConfig = COUNT(*) FROM #tmpAvailableStyles
			LEFT JOIN #tmpExistingPlanningConfigurations 
			ON #tmpAvailableStyles.DivisionID = #tmpExistingPlanningConfigurations.DivisionID
				AND #tmpAvailableStyles.StyleType = #tmpExistingPlanningConfigurations.StyleTypeID
			WHERE #tmpExistingPlanningConfigurations.PlanningConfigID IS NULL
	END
	ELSE IF @ConfigLevel = 3
	BEGIN
		SELECT @CountOfStyleWithoutConfig = COUNT(*) FROM #tmpAvailableStyles
			...
			ON #tmpAvailableStyles.DivisionID = #tmpExistingPlanningConfigurations.DivisionID
				AND #tmpAvailableStyles.StyleType = #tmpExistingPlanningConfigurations.StyleTypeID
				AND #tmpAvailableStyles.StyleCategory = #tmpExistingPlanningConfigurations.StyleCategoryID
			WHERE #tmpExistingPlanningConfigurations.PlanningConfigID IS NULL
	END
	...
	SELECT ISNULL(@CountOfStyleWithoutConfig,0) AS CountOfStyleWithoutConfig
...
VALUES ('DB_Version', '6.0.0000', '09025', GetDate())
GO
