...
AS Link, 
e.AssignedTo, NULL as StyleColorID, b.ItemDim1Id, b.ItemDim2Id,  b.ItemDim3Id,  
...
AND (a.Submit >= @NoOfSubmits OR @NoOfSubmits IS NULL OR @NoOfSubmits = '')
AND (e.AssignedTo = @UserID OR @UserID IS NULL)
AND (e.AssignedTo IN (SELECT value FROM dbo.fnx_Split(@AssignedTo, ',')) OR @AssignedTo IS NULL)
AND (g.StyleType IN (SELECT value FROM dbo.fnx_Split(@StyleType, ',')) OR @StyleType IS NULL)
AND (g.StyleCategory IN (SELECT value FROM dbo.fnx_Split(@StyleCategory, ',')) OR @StyleCategory IS NULL)
AND (h.SeasonYearID IN (SELECT value FROM dbo.fnx_Split(@SeasonYear, ',')) OR @SeasonYear IS NULL)
...
+  CAST(a.Submit AS NVARCHAR(5)) + ')</b></a></TD></TR></TABLE>'
AS Link, 
e.AssignedTo, b.StyleColorID, NULL as ItemDim1Id, NULL as ItemDim2Id, NULL as ItemDim3Id,  
...
AgentID, AgentName, VendorID, VendorName, FactoryID, FactoryName, Variation, StyleType, StyleCategory, SeasonYearID, CDate
FROM #tmpSample3',
NULL,
NULL,
'SampleWorkflow',
...
VALUES ('DB_Version', '0.5.0000', '07030', GetDate())
GO
