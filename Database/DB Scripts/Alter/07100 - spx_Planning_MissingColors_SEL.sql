...
BEGIN
	...
	(
		...
		FROM pPlanningItem pli
			...
			LEFT JOIN pPlanningColor pc ON cp.ColorPaletteID = pc.ColorPaletteID
				AND pc.PlanningID = @PlanningID AND pc.SeasonYearID = @SeasonYearID
		WHERE pc.ColorPaletteID IS NULL AND cp.ColorPaletteID = @ColorPaletteID
		UNION
...
VALUES ('DB_Version', '0.5.0000', '07100', GetDate())
GO
