...
SELECT     ISNULL(g.GroupName, '') AS GroupName, dbo.fnx_GetStreamingImagePath(sh.DesignSketchID, sh.DesignSketchVersion) AS ImagePath, sh.StyleNo, sh.Description, 
                      sc.StyleCategory, sh.SizeClass, ISNULL(sh.CustomField3, '') AS StyleStatus, sh.DueDate, CASE WHEN sh.DueDate < GETDATE() THEN CAST(DATEDIFF(d, sh.DueDate,
...
VALUES ('DB_Version', '0.5.0000', '06004', GetDate())
GO
