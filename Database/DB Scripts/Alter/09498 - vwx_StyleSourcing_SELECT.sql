IF OBJECT_ID(N'[dbo].[vwx_StyleSourcing_SELECT]') IS NOT NULL
 DROP VIEW [dbo].[vwx_StyleSourcing_SELECT]
GO

CREATE VIEW [dbo].[vwx_StyleSourcing_SELECT]
AS
SELECT sh.StyleID, sh.StyleMasterID, sh.StyleWorkflowID, sh.StyleType, sh.WorkflowType, sh.RefNo, sh.StyleNo, 
	   ...
	   sh.CustomField30, sh.PackagingNo, sh.StyleCopyID, sh.LinePlanItemID, sh.StyleStatusID, sh.BodyID, ss.MostLikelyVendor, 
	   ss.TradePartnerID, ss.TradePartnerVendorID, ss.StyleSourcingID, ss.SourcingName
...
VALUES ('DB_Version', '6.1.0000', '09498', GetDate())
GO
