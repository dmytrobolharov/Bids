...
SELECT	
	  CASE WHEN RTRIM(LTRIM(tms.S)) <> '10' THEN 1 ELSE 0 END AS IsApproved
	, tms.StaticDueDate
...
SELECT 
	  CASE WHEN COUNT(CASE WHEN cmiws.WorkflowStatusID = tms.S THEN 1 ELSE NULL END) = 0 THEN 1 ELSE 0 END AS IsApproved
...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, tmmw.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END)) < 0 THEN CAST(ABS(DATEDIFF(DAY, tmmw.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, tmmw.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
		WHEN COUNT(CASE WHEN tmmw.IsApproved = 0 OR tmmw.IsApproved IS NULL THEN 1 ELSE NULL END) = 0 THEN 'LightGreen'
		WHEN DATEDIFF(DAY, tmmw.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END)) < 0 THEN 'IndianRed'
		WHEN DATEDIFF(DAY, tmmw.MilestoneDate, MAX(CASE WHEN tmmw.IsApproved = 0 THEN tmmw.DueDate ELSE NULL END)) BETWEEN 0 AND 7 THEN 'Khaki'		
		ELSE 'None'
	  END AS Color
FROM #tmpMatStatic tms
INNER JOIN #tmpMilestoneMaterialWorkflow tmmw ON tms.MaterialID = tmmw.MaterialID
GROUP BY
	  tms.MaterialID
	, tms.MaterialNo
	, tms.MaterialName
...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, tmmr.MilestoneDate, MAX(CASE WHEN tmmr.IsApproved = 0 THEN tmmr.DueDate ELSE NULL END)) < 0 THEN CAST(ABS(DATEDIFF(DAY, tmmr.MilestoneDate, MAX(CASE WHEN tmmr.IsApproved = 0 THEN tmmr.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, tmmr.MilestoneDate, MAX(CASE WHEN tmmr.IsApproved = 0 THEN tmmr.DueDate ELSE NULL END))) AS VARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
		WHEN COUNT(CASE WHEN tmmr.IsApproved = 0 OR tmmr.IsApproved IS NULL THEN 1 ELSE NULL END) = 0 THEN 'LightGreen'
		WHEN DATEDIFF(DAY, tmmr.MilestoneDate, MAX(CASE WHEN tmmr.IsApproved = 0 THEN tmmr.DueDate ELSE NULL END)) < 0 THEN 'IndianRed'
		WHEN DATEDIFF(DAY, tmmr.MilestoneDate, MAX(CASE WHEN tmmr.IsApproved = 0 THEN tmmr.DueDate ELSE NULL END)) BETWEEN 0 AND 7 THEN 'Khaki'		
		ELSE 'None'
	  END AS Color
FROM #tmpMatStatic tms
...
SELECT 
	...
	, CASE 
		WHEN COUNT(CASE WHEN tmr.MaterialRequestWorkflowSort = 0 OR tmr.MaterialRequestWorkflowSort IS NULL THEN 1 ELSE NULL END) = 0 THEN 'Approved'
	...
	, MAX(tmr.DueDate) AS DueDate
	, CASE 
		WHEN COUNT(CASE WHEN tmr.MaterialRequestWorkflowSort = 0 OR tmr.MaterialRequestWorkflowSort IS NULL THEN 1 ELSE NULL END) = 0 THEN 'LightGreen'
...
VALUES     ('DB_Version', '0.5.0000', '05517', GetDate())
GO
