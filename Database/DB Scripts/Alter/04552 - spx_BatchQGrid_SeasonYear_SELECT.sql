...
BEGIN	
	...
	SET @SQLString = 'SELECT * FROM (' + REPLACE(@BatchQueueGridSql,'SELECT * FROM pSeasonYear','SELECT * FROM pSeasonYear WHERE Active = 1') + ') AS SqlBatchQ WHERE NOT EXISTS (SELECT * FROM ' +  @BatchQueueJobTable + ' WHERE ' + @BatchQueuePagePkName + ' = SqlBatchQ.' + @BatchQueuePagePkName + ')'
...
VALUES     ('DB_Version', '5.0.0000', '04552', GetDate())
GO
