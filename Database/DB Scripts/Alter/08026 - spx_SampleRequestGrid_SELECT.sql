/****** Object:  StoredProcedure [dbo].[spx_SampleRequestGrid_SELECT]    Script Date: 06/17/2014 15:25:13 ******/
...
END AS [Style Set], pStyleColorway.MainColor AS Color FROM pSampleRequestWorkflow  a INNER JOIN
	pStyleHeader ON a.StyleID = pStyleHeader.StyleID LEFT OUTER JOIN
	pStyleColorway ON a.StyleColorID = pStyleColorway.StyleColorID INNER JOIN
	pSampleRequestTrade ON a.SampleRequestTradeID = pSampleRequestTrade.SampleRequestTradeID '
...
ELSE 
	EXEC (' INSERT INTO ##tmpSampleWorkflow  
			SELECT DISTINCT a.SampleWorkflowID  FROM pSampleRequestWorkflow  a WITH (NOLOCK)  
			INNER JOIN pSampleWorkflow sw ON sw.SampleWorkflowID = a.SampleWorkflowID
			INNER JOIN pStyleHeader sh ON sh.StyleID = a.StyleID 
			INNER JOIN fnx_Permissions_GetIntProductType_WorkflowPermissions(''' + @TeamID + ''', 5, NULL) acc ON acc.WorkflowID = sw.SampleWorkflowGUID AND acc.ProductTypeID = sh.StyleType 
			WHERE SampleRequestTradeID IN (' + @sqlSelect + ') AND  a.SampleWorkflowID   IN  ( 
			SELECT DISTINCT SampleWorkflowID   FROM  pSampleWorkflowViewSubmit WITH (NOLOCK) WHERE TeamId = ''' +  @TeamID + ''' )  
			AND acc.PermissionRoleID > 0')
...
VALUES ('DB_Version', '0.5.0000', '08026', GetDate())
GO
