/****** Object:  StoredProcedure [dbo].[spx_BodyPatternSheetModel_GET_UPDATE]    Script Date: 08/22/2012 14:36:06 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_BodyPatternSheetModel_GET_UPDATE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_BodyPatternSheetModel_GET_UPDATE]
GO
...
CREATE PROCEDURE  [dbo].[spx_BodyPatternSheetModel_GET_UPDATE]
(
	@CADDB varchar(50),
	@Catalog varchar(50),
...
FROM pBodyPatternSheetModel where BodyID = @BodyID AND StyleSet = @StyleSet and IsLinked = 1
...
SET	  
    ...
    --,[Model_Comments] = (SELECT Model_Comments from  #tempTBLCAD ) -- do not replace model comments by original ones from the CAD
...
Where pBodyPatternSheetModel.Model_Code = (select Model_Code from #tempTblBody where rownum = @RowCnt)
       AND pBodyPatternSheetModel.BodyID = @BodyID AND StyleSet = @StyleSet and IsLinked = 1
...
--EXEC [spx_Body_PatternSheetPiece_UPDATE] @BodyID, @StyleSet, @Model_Code,@CADDB,@Catalog
...
select @Sortsql = 'SELECT 	[ID] ,
	...
	[Model_CrOpId],
	[Model_Create],
	ISNULL([Model_RevOp1],[MUser]) as Model_RevOp1,
	ISNULL([Model_Rev1],[MDate]) as Model_Rev1,
	[Model_RevOp2],
	[Model_Rev2],
	[Model_GRTable],
	[Model_MTMFile],
	...
	[NotchFile],
	[AnnotationFile],
	[StorageType] FROM pBodyPatternSheetModel   WHERE (BodyID = ' + CHAR(39) + Convert(varchar(36),@BodyID) + CHAR(39) + ' AND StyleSet = ' + Convert(varchar(10),@StyleSet) + ' and Active = 1 )' + ' order by ' + @strSortOrder 
...
VALUES ('DB_Version', '5.0.0000', '03794', GetDate())
GO
