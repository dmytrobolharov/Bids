...
/****** Object:  StoredProcedure [dbo].[spx_StyleModel_GET_UPDATE]    Script Date: 10/06/2010 13:21:51 ******/
SET ANSI_NULLS ON
GO
...
select @Sortsql = 'SELECT 	[ID] ,
	...
	[NotchFile],
	[AnnotationFile],
	[StorageType] FROM pBodyPatternSheetModel  WHERE (StyleID = ' + CHAR(39) + Convert(varchar(36),@StyleID) + CHAR(39) + ' and Active = 1)' +'order by ' + @strSortOrder
EXEC(@Sortsql)
END
GO
----------------------------------------------------------------
...
VALUES     ('DB_Version', '4.0.0000', '595', GetDate())
GO
