...
BEGIN
	...
	DELETE FROM pPlanningBusiness WHERE 
		...
		AND NOT (PlanningBrandID IS NULL
	...
	BEGIN
		INSERT INTO pPlanningBusiness(
			PlanningID, SeasonYearID, 
			PlanningFinancialID, PlanningFinancialText, PlanningFinancialSort)
		SELECT @PlanningID, @SeasonYearID, 
			PlanningFinancialID, PlanningFinancialText, PlanningFinancialSort
	...
	WHERE PlanningID = @PlanningID AND SeasonYearID = @SeasonYearID 
		AND PlanningBrandID IS NULL 
		AND (PlanningDivisionID IS NOT NULL 
			OR PlanningStyleTypeID IS NOT NULL
			OR PlanningStyleCategoryID IS NOT NULL 
			OR PlanningSubCategoryID IS NOT NULL)
...
VALUES     ('DB_Version', '0.5.0000', '08266', GetDate())
GO
