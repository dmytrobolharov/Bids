IF OBJECT_ID(N'[dbo].[dpx_SampleRequestSubmit_Approved_SELECT]') IS NOT NULL
 DROP PROCEDURE [dbo].[dpx_SampleRequestSubmit_Approved_SELECT]
GO

CREATE PROCEDURE [dbo].[dpx_SampleRequestSubmit_Approved_SELECT]
(
	@Flag int
)
AS
...
IF(@Flag = 1)	--Submits approved today.
	BEGIN
		...
		SELECT Users.FirstName + ' ' + Users.LastName AS AssignedTo, sh.StyleNo, 
			...
			pSampleWorkflow.GroupName, pSampleRequestSubmitStatus.Status, pStyleSeasonYear.StyleSeason, 
			pStyleSeasonYear.StyleYear, uTradePartner.TradePartnerName, uTradePartnerVendor.VendorName, pStyleColorway.StyleColorNo, 
			pStyleColorway.StyleColorName, pStyleColorway.StyleColorStandardID, sh.StyleID, 
...
VALUES ('DB_Version', '6.1.0000', '09553', GetDate())
GO
