...
BEGIN
	...
	; WITH HasBOMWorkflow AS (
		SELECT sw.StyleSeasonYearID, 1 AS HasBOM
		FROM pStyleHeader sh
		INNER JOIN pStyleWorkflow sw ON sh.StyleID = sw.StyleID AND sw.StyleSet = 1
		LEFT JOIN pWorkflowTemplateItem wti ON 
				sh.StyleWorkflowID = wti.WorkflowTemplateID AND sw.WorkflowID = wti.WorkflowID
		WHERE sw.WorkflowID = '40000000-0000-0000-0000-000000000080'
			AND (wti.WorkflowTemplateItemID IS NOT NULL
				AND sh.StyleWorkflowID IS NOT NULL
				OR sh.StyleWorkflowID IS NULL)
	), MismatchingBOM AS (
		SELECT DISTINCT pli.PlanningItemID, 1 AS MismatchingBOM FROM pLineFolderItem lfi
		...
		, ws.WorkflowStatusID
		, 1 - ISNULL(HasBOMWorkflow.HasBOM, 0) AS MissingBOMWorkflow
		...
		, ISNULL(mb.MismatchingBOM, 0) AS MismatchingBOM
	INTO #tmpItems
	...
	LEFT JOIN HasBOMWorkflow ON pli.StyleSeasonYearID = HasBOMWorkflow.StyleSeasonYearID
	...
	LEFT JOIN MismatchingBOM mb ON pli.PlanningItemID = mb.PlanningItemID	
	WHERE pli.PlanningID = @PlanningID AND pli.SeasonYearID = @SeasonYearID
...
VALUES ('DB_Version', '0.5.0.0000', '08454', GetDate())
GO
