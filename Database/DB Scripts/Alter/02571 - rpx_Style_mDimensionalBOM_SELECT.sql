...
/****** Object:  StoredProcedure [dbo].[rpx_Style_mDimensionalBOM_SELECT]    Script Date: 01/13/2012 03:56:17 ******/
...
BEGIN
		...
		SELECT @DataXmlFile=SchemaXML FROM pStyleBOMXML 
		...
		BEGIN
				...
				SET @SQLQUERY=@SQLQUERY+	'	ORDER BY SBD.ItemDim2Sort,SBD.ItemDim3Sort'
				
				EXEC (@SQLQUERY)
				
				SELECT * INTO #tblbeforeUpdate  FROM tblbeforeUpdate
				
				IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[tblbeforeUpdate]') 
				AND type in (N'U'))
				BEGIN	
					DROP TABLE [dbo].[tblbeforeUpdate]
				END
				...
				SELECT * FROM #tblFinalTableWithGroupID
	...
	BEGIN
		...
		SET @SQLQUERY1='SELECT dbo.fnx_GetMaterialType(SB.MaterialType) AS MaterialType'
				...
				SET @SQLQUERY1=@SQLQUERY1+	'	ORDER BY SBD.ItemDim2Sort,SBD.ItemDim3Sort'
		BEGIN
		...
		SELECT * FROM #tblFinalTable2WithGroupID 
...
IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='02571'))
BEGIN

	INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
	VALUES     ('DB_Version', '4.2.0000', '02571', GetDate())
