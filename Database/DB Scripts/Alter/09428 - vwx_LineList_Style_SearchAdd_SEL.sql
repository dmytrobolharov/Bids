IF OBJECT_ID(N'[dbo].[vwx_LineList_Style_SearchAdd_SEL]') IS NOT NULL
 DROP VIEW [dbo].[vwx_LineList_Style_SearchAdd_SEL]
GO

CREATE VIEW [dbo].[vwx_LineList_Style_SearchAdd_SEL]
AS
SELECT sh.StyleID, sh.StyleMasterID, sh.StyleWorkflowID, sh.StyleType, sh.WorkflowType, sh.RefNo, sh.StyleNo, sh.TempID, 
       ...
       sh.TargetPrice, sh.SellingPrice, sh.CustomField1, sh.CustomField2, sh.CustomField3, sh.CustomField4, sh.CustomField5, 
       ...
       sh.LinePlanItemID, sh.StyleStatusID, sm.F AS FabricType, sm.H AS FabricContent, sh.IntroSeasonYearID, sh.DivisionID, 
       sc.StyleCategory AS Category, sh.DesignSketchBackID, sh.DesignSketchBackVersion
FROM pStyleHeader sh
INNER JOIN pStyleCategory sc ON sh.StyleCategory = sc.StyleCategoryId 
LEFT JOIN pStyleMaterials sm ON sh.StyleMaterialID = sm.StyleMaterialID
...
VALUES ('DB_Version', '6.1.0000', '09428', GetDate())
GO
