IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dpx_SourcingQuotationStyles_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[dpx_SourcingQuotationStyles_SELECT]
GO

CREATE PROCEDURE [dbo].[dpx_SourcingQuotationStyles_SELECT]
...
ELSE
	...
	FROM
		...
		LEFT OUTER JOIN pSourcingQuotationBOMOption sqboi ON sqi.StyleQuoteItemID = sqboi.StyleQuoteItemID
		LEFT OUTER JOIN pSourcingHeader soh ON sqi.SourcingHeaderID = soh.SourcingHeaderID
	WHERE sh.StyleID = @StyleID AND sqi.StyleQuoteItemStatusId = @QuoteStatusID AND soh.SourcingHeaderID = @SourcingHeaderID
	GROUP BY
		sh.StyleID
		, sqi.StyleQuoteItemNo
		, sqi.StyleQuoteItemStatusId
...
VALUES ('DB_Version', '0.5.0000', '07894', GetDate())
GO
