...
BEGIN
	...
	SELECT StyleMaterialID, Qty, 
		...
		MaterialBOM,
		COALESCE(pMaterialTradePartner.CurrencyType, pMaterial.CurrencyType, @ConvCurrType)
	FROM pStyleBOM
	...
	WHERE pStyleBOM.StyleBOMDimensionId = @StyleBOMDimensionId
		AND (pStyleBOM.Dim1TypeSel = 1 OR pStyleBOM.Dim2TypeSel = 1 OR pStyleBOM.Dim3TypeSel = 1)
	GROUP BY pStyleBOM.StyleMaterialID, MaterialBOM,COALESCE(pMaterialTradePartner.CurrencyType, pMaterial.CurrencyType, @ConvCurrType)
	...
	BEGIN
		...
		BEGIN
			SELECT '   ' AS cellSpace, 'BOM Costing  ' AS [Cost Type], @ConvCurrType + ' ' + CAST(SUM(ConvTotal) AS nVARCHAR(10)) + '' AS [Total Cost], '   ' AS cellSpace1
			FROM         #avTemp_BOM WHERE MaterialBOM = 1
			UNION ALL
			SELECT '   ' AS cellSpace, 'Total Costing  ' AS [Cost Type], @ConvCurrType + ' ' +  CAST(SUM(ConvTotal) AS nVARCHAR(10)) + '' AS [Total Cost], '   ' AS cellSpace1
			FROM         #avTemp_BOM
		END
		ELSE
		BEGIN
...
VALUES ('DB_Version', '0.5.0000', '06983', GetDate())
GO
