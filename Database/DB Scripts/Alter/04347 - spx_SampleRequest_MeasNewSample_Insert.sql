/****** Object:  StoredProcedure [dbo].[spx_SampleRequest_MeasNewSample_Insert]    Script Date: 11/15/2012 16:52:26 ******/
...
AS 
	...
	BEGIN
		...
		SELECT @CurrentRevisedSpec = RevisedSpec FROM pMSEvalSampMeas
		WHERE SamplepomId = @CurrentSamplePomID
		AND SampleIndex = (@NewSubmit-1)
		AND AMLMeasHdrId=@AMLMeasHdrId
		AND SampleRequestHeaderId=@SampleRequestHeaderId
		
		-- Updating new Eval Size with old Revised Size		
		UPDATE pmsevalsampMeas
		SET EvalMeasValue = @CurrentRevisedSpec
		WHERE AMLMeasHdrId = @AMLMeasHdrId
		AND SampleRequestHeaderId = @SampleRequestHeaderId
		AND SampleIndex = @NewSubmit
...
VALUES     ('DB_Version', '5.0.0000', '04347', GetDate())
GO
