IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[vwx_LinePlanShowroomItem_SEL_Remove]') AND type = N'V' )
...
AS
SELECT  DISTINCT  a.LinePlanItemID, a.LinePlanRangeID, a.LinePlanRangeTypeID, a.LinePlanID, a.CUser, a.CDate, a.MUser, a.MDate, a.StyleTypeID, b.StyleID, 
	...
	d.StyleCostingDate, d.StyleCostingStatus, 
	CASE 
...
FROM dbo.pLinePlanItem a WITH (NOLOCK)
	...
	LEFT OUTER JOIN dbo.pStyleHeader b WITH (NOLOCK) ON a.StyleID = b.StyleID 
	LEFT OUTER JOIN dbo.pStyleSeasonYear c WITH (NOLOCK) ON a.StyleID = c.StyleID AND a.LinePlanItemID = c.LinePlanItemID 
	LEFT OUTER JOIN dbo.pStyleCostingHeader d WITH (NOLOCK) ON d.StyleSeasonYearID = c.StyleSeasonYearID 
	LEFT OUTER JOIN pLinePlanStyleAttributeItem e WITH (NOLOCK) ON e.LinePlanStyleAttributeItemID = a.LinePlanStyleAttributeItemID 
...
VALUES     ('DB_Version', '4.0.0000', '689', GetDate())
GO
