...
--If @StyleBOMDimensionId <> '00000000-0000-0000-0000-000000000000'
	...
	if @DetailsCnt>0
		...
		begin
			...
			Select @TotalCost = ((ISNULL(StyleCostingCustomField6, '0.00')) * (ISNULL(StyleQuoteItemCustomField16, '0.00'))) + ISNULL(@AdditionalCost, '0.00')
	...
	Select @CommAmt = (((ISNULL(StyleQuoteItemCustomField9, '0.00'))/100) * @TotalCost) 
	FROM pSourcingCommitmentItem WHERE SourcingCommitmentItemID=@SourcingCommitmentItemID and SourcingHeaderId=@SourcingHeaderId

	Select @AgentAmt = (((ISNULL(StyleQuoteItemCustomField11, '0.00'))/100) * (@TotalCost + @CommAmt)) 
	FROM pSourcingCommitmentItem WHERE SourcingCommitmentItemID=@SourcingCommitmentItemID and SourcingHeaderId=@SourcingHeaderId

	Select @DutyAmt = (((ISNULL(StyleQuoteItemCustomField7, '0.00'))/100) * (@TotalCost + @CommAmt + @AgentAmt)) 
	FROM pSourcingCommitmentItem WHERE SourcingCommitmentItemID=@SourcingCommitmentItemID and SourcingHeaderId=@SourcingHeaderId
	...
	Select @IntExtFOB = @TotalCost * (1 + ((ISNULL(StyleQuoteItemCustomField9, '0.00'))/100) + ((ISNULL(StyleQuoteItemCustomField11, '0.00'))/100) + (((ISNULL(StyleQuoteItemCustomField9, '0.00'))/100) * ((ISNULL(StyleQuoteItemCustomField11, '0.00'))/100)))
	FROM pSourcingCommitmentItem WHERE SourcingCommitmentItemID=@SourcingCommitmentItemID and SourcingHeaderId=@SourcingHeaderId
	
	Select @IntExtLDP = @IntExtFOB + @FreightAmt + (@IntExtFOB * ((ISNULL(StyleQuoteItemCustomField7, '0.00'))/100))
	FROM pSourcingCommitmentItem WHERE SourcingCommitmentItemID=@SourcingCommitmentItemID and SourcingHeaderId=@SourcingHeaderId
...
VALUES ('DB_Version', '0.5.0000', '05287', GetDate())
GO
