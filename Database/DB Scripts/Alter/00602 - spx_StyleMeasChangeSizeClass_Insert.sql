...
IF EXISTS (select * from sysobjects where id = object_id('spx_StyleMeasChangeSizeClass_Insert') and sysstat & 0xf = 4)
...
(CompanyGuidID = @CompanyID) AND (SizeClassGuidId = @OldSizeClassID) AND 
...
If @TotalCount>0 
	...
	--select count(*) as pmlcount1 from pamlmeaspom
		...
		WHERE (CompanyGuidID = @CompanyID) AND (SizeClassGuidId = @OldSizeClassID) AND 
		(SizeRangeGuidId = @OldSizeRangeID) AND (GarmentGuidID = @OldProductTypeID))))
...
SizeRange = @SizeRange, SizeClass = @SizeClass , MeasRowCnt=(select max(pom_row) from pamlmeaspom where AMLMeasHdrId = @AMLMeasHdrId) 
...
VALUES     ('DB_Version', '4.0.0000', '602', GetDate())
GO