/****** Object:  StoredProcedure [dbo].[spx_MaterialRequestSubmitWorkflowTreeWithoutColors_SELECT]    Script Date: 03/28/2013 13:43:28 ******/
...
BEGIN
	...
	FROM (
		...
		INNER JOIN dbo.pMaterialRequestWorkflowTemplateItem f WITH(NOLOCK) ON f.MaterialRequestWorkflowTempItemID = foo.MaterialRequestWorkflowTempItemID
...
BEGIN
	...
	FROM (
		...
		INNER JOIN dbo.pMaterialRequestSubmitStatus c WITH(NOLOCK)  ON c.StatusID = d.Status
		INNER JOIN dbo.pMaterialRequestWorkflow e WITH(NOLOCK) ON e.MaterialRequestWorkflowID = foo.MaterialRequestWorkflowID
		INNER JOIN dbo.pMaterialRequestWorkflowTemplateItem f ON f.MaterialRequestWorkflowTempItemID = foo.MaterialRequestWorkflowTempItemID
		LEFT JOIN dbo.uTradePartnerType pt WITH(NOLOCK) ON d.MaterialRequestPartnerTypeID = pt.CustomId
...
VALUES     ('DB_Version', '0.5.0000', '05435', GetDate())
GO
