/****** Object:  View [dbo].[vwx_LineList_SampleStyle_SEL]    Script Date: 06/16/2014 12:57:33 ******/
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vwx_LineList_SampleStyle_SEL]'))
...
SELECT     dbo.pStyleHeader.StyleID, dbo.pStyleHeader.WorkflowTemplateID, dbo.pStyleHeader.PlannedDueDate, dbo.pStyleHeader.EnforceDependency, 
                      ...
                      dbo.pLineFolderItem.LineFolderID, dbo.pLineFolderItem.LineFolderItemID, dbo.pLineFolderItem.LineFolderItemDrop, ISNULL(dbo.pStyleDevelopmentItem.StyleDevelopmentName, 
                      'Variation ' + CAST(dbo.pStyleDevelopmentItem.Variation AS nVarchar(2))) AS Variation,
                          (SELECT     COUNT(*) AS Expr1
                            FROM          dbo.pSampleRequestBOMTrade AS samples INNER JOIN
                                                   ...
                                                   dbo.pLineFolder AS lf ON lf.SeasonYearID = ssy.SeasonYearID AND dbo.pLineFolderItem.LineFolderID = lf.LineFolderID
                            WHERE      (samples.StyleID = dbo.pLineFolderItem.StyleID)) AS SampleRequestsCount, CASE WHEN EXISTS
                          (SELECT     *
                            FROM          dbo.pSampleRequestBOMTrade AS samples INNER JOIN
                                                   dbo.pStyleSeasonYear AS ssy ON samples.StyleSeasonYearID = ssy.StyleSeasonYearID INNER JOIN
                                                   dbo.pLineFolder AS lf ON lf.seasonYearId = ssy.SeasonYearID AND pLineFolderItem.LineFolderID = lf.LineFolderID
                            WHERE      samples.StyleID = dbo.pLineFolderItem.StyleID) THEN 1 ELSE 0 END AS HasSampleRequests
FROM         dbo.pStyleHeader LEFT OUTER JOIN
                      ...
                      dbo.pLineFolderItem ON dbo.pStyleHeader.StyleID = dbo.pLineFolderItem.StyleID LEFT OUTER JOIN
...
VALUES ('DB_Version', '0.5.0000', '08008', GetDate())
GO
