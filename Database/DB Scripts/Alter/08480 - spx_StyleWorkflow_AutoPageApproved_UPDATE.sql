...
BEGIN 
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				UPDATE pStyleWorkflow SET WorkStatus = 100, WorkStatusDate = CAST(GETDATE() AS DATE) WHERE StyleID = @StyleID 
					AND StyleSet = @StyleSet AND WorkflowID = @WorkflowID AND StyleSeasonYearID = @StyleSeasonYearID
			END	
		END
		ELSE IF @WorkflowStatus <> '102'
		BEGIN
			UPDATE pStyleWorkflow SET WorkStatus = 6, WorkStatusDate = NULL WHERE StyleID = @StyleID AND StyleSet = @StyleSet 
...
BEGIN
	...
	BEGIN
		IF @WorkflowStatus = '100'
		BEGIN
			UPDATE pStyleWorkflow SET WorkStatus = 100, WorkStatusDate = CAST(GETDATE() AS DATE) WHERE StyleID = @StyleID AND StyleSet = @StyleSet 
				AND WorkflowID = @WorkflowID AND StyleSeasonYearID = @StyleSeasonYearID
		END
		ELSE IF @WorkflowStatus <> '102'
		BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF @ApprovedCount = 0
			BEGIN
				UPDATE pStyleWorkflow SET WorkStatus = 6, WorkStatusDate = NULL WHERE StyleID = @StyleID 
					AND StyleSet = @StyleSet AND WorkflowID = @WorkflowID
			END	
		END
	END
...
VALUES     ('DB_Version', '0.5.0000', '08480', GetDate())
GO
