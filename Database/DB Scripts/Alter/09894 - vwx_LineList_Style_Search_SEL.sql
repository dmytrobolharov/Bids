IF OBJECT_ID(N'[dbo].[vwx_LineList_Style_Search_SEL]') IS NOT NULL
...
SELECT sh.StyleID, sh.StyleMasterID, sh.StyleWorkflowID, sh.StyleType, sh.WorkflowType, sh.RefNo, sh.StyleNo, 
       ...
       sh.Description, sh.StyleCategory, sh.SizeClassId, COALESCE(scc.Custom, sh.SizeClass) AS SizeClass, 
       ...
       sh.LinePlanItemID, sh.StyleStatusID, sm.F AS FabricType, sm.H AS FabricContent, ssy.StyleSeason, ssy.StyleYear, 
       ssy.SeasonYearID, ssy.StyleSeasonYearID, sh.IntroSeasonYearID, sh.DivisionID, sc.StyleCategory AS Category, 
       ISNULL(sy.Season + ' ', '') + ISNULL(sy.Year, '') AS SeasonYearName, sh.DesignSketchBackID, sh.DesignSketchBackVersion
...
VALUES ('DB_Version', '6.1.0000', '09894', GetDate())
GO
