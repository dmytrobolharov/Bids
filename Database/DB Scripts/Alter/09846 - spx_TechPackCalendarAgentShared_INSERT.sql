IF OBJECT_ID(N'[dbo].[spx_TechPackCalendarAgentShared_INSERT]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_TechPackCalendarAgentShared_INSERT]
GO

CREATE PROCEDURE [dbo].[spx_TechPackCalendarAgentShared_INSERT]
...
BEGIN
	...
	(sh.TechPackID IS NOT NULL) AND (pTechPackTeam.TeamID = @TradePartnerId) AND 
...
BEGIN
	INSERT INTO pStyleCalendarTemp
		(CalendarId, PKeyId, CalendarLinkId, CalendarLinkSubId, CalendarDate, CalendarType, CalendarStatus, CalendarDescription)
	SELECT  @CalendarId AS CalendarId, sh.StyleID, sh.TechPackID, sh.DevelopmentID, pTechPackActivity.CDate, 'TechPack' AS CalendarType, 'D', 
		sh.StyleNo + ' (' + COALESCE(sc.Custom, sh.SizeClass) + ') ' + sh.Description AS CalendarDescription
...
VALUES ('DB_Version', '6.1.0000', '09846', GetDate())
GO
