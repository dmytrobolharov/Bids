IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_SampleRequestListSeasonal_SELECT_API]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_SampleRequestListSeasonal_SELECT_API]
GO

CREATE PROCEDURE  [dbo].[spx_SampleRequestListSeasonal_SELECT_API](
...
(SELECT p.SampleRequestTradeID, p.StyleID, ssy.SeasonYearID, sh.StyleNo, sh.[Description] as StyleDesc, sh.DesignSketchID,sh.DesignSketchVersion,  ssy.StyleSeason + ' ' + ssy.StyleYear as SeasonYear, wfi.WorkflowItemName as BOMName,
...
CASE sbd.ItemDim1TypeName 
WHEN 'Color' THEN dbo.fnx_GetStreamingColorImagePath((SELECT ColorFolderID FROM pColorPalette WHERE ColorPaletteID = sbdi1.ItemDim1ID), sbdi1.ItemDim1ID, '100') 
ELSE NULL END AS ItemDim1Image,
CASE sbd.ItemDim2TypeName
WHEN 'Color' THEN dbo.fnx_GetStreamingColorImagePath((SELECT ColorFolderID FROM pColorPalette WHERE ColorPaletteID = sbdi2.ItemDim2ID), sbdi2.ItemDim2ID, '100') 
ELSE NULL END AS ItemDim2Image,
CASE sbd.ItemDim3TypeName 
WHEN 'Color' THEN dbo.fnx_GetStreamingColorImagePath((SELECT ColorFolderID FROM pColorPalette WHERE ColorPaletteID = sbdi3.ItemDim3ID), sbdi3.ItemDim3ID, '100') 
...
LEFT JOIN pStyleBOMDimensionItem sbdi3 ON p.StyleBOMDimensionID = sbdi3.StyleBOMDimensionID AND p.ItemDim3ID = sbdi3.ItemDim3ID) p
...
VALUES ('DB_Version', '6.1.0000', '10194', GetUTCDate())
GO