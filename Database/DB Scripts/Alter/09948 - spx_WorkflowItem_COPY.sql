...
BEGIN
 ...
 IF @@Error <> 0 OR @@RowCount <> 1 GOTO Error  
  ...
  Else If @WorkFlowId = '40000000-0000-0000-0000-000000000050'   
   ...
   IF EXISTS ( SELECT * FROM pStyleImageMeasurements WHERE MeasurementID = @WorkFlowItemId AND StyleID = @CopyStyleID)
	BEGIN
		...
		FROM pStyleImageMeasurements WHERE MeasurementID = @WorkFlowItemId AND StyleID = @CopyStyleID
	END
			...
			BEGIN
				...
				BEGIN
					...
					EXEC spx_StyleColorway_Color_INSERT_SELECT 
										...
										@CUser, 
										@CDate,
										@NewSeasonYearID = @NewSeasonYearId
...
VALUES ('DB_Version', '6.1.0000', '09948', GetUTCDate())
GO
