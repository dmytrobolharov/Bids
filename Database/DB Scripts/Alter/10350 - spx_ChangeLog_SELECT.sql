IF OBJECT_ID(N'[dbo].[spx_ChangeLog_SELECT]') IS NOT NULL
...
BEGIN
    ...
    BEGIN
		SELECT * INTO #tmpChangeMaterial FROM (			
			...
			UNION ALL
			-- include material colors change log into overall change log for material
			SELECT * FROM vwx_ChangeLogItemMaterial_SEL 
			WHERE ChangeTableID = 'D6121122-BBBB-3332-AFC1-ABCD56CBBDDD' AND ChangeTablePKID = @ChangeTransTablePKID
				AND ChangeDate BETWEEN @StartDate AND @EndDate
		) Change
    ...
    SELECT * INTO #tmpChangeDefaultColor FROM (			
			...
			UNION
			SELECT a.*, dbo.fnx_GetStreamingColorImageHTML(b.ColorFolderID, b.ColorPaletteID, 20, '') AS ColorCode
    ...
    END
    ELSE IF @ChangeTableID = '00000009-C1CC-DF11-AF06-005056C00008' --pStyleSourcing (old Sourcing workflow)
    BEGIN
    SELECT * INTO #tmpChangeDefaultSourcingOld FROM (			
			SELECT * FROM vwx_ChangeLogItem_SEL 
			WHERE ChangeTableID = @ChangeTableID AND ChangeTablePKID = @ChangeTransTablePKID
				AND ChangeDate BETWEEN @StartDate AND @EndDate	
			UNION --add sourcing header info
			SELECT * FROM vwx_ChangeLogItem_SEL 
			WHERE ChangeTableID = '00000005-CCCC-DF11-AF06-005056C00018' AND ChangeTablePKID in (select pStyleQuoteItem.StyleSourcingID from pStyleQuoteItem inner join pStyleSourcing on pStyleSourcing.StyleSourcingID = pStyleQuoteItem.StyleSourcingID WHERE pStyleQuoteItem.StyleQuoteItemID=@ChangeTransTablePKID)
  AND ChangeDate BETWEEN @StartDate AND @EndDate	
		) Change

		EXEC('SELECT * FROM #tmpChangeDefaultSourcingOld ' + @WhereCond)
		DROP TABLE #tmpChangeDefaultSourcingOld
    END
...
VALUES ('DB_Version', '6.1.0000', '10350', GetUTCDate())
GO
