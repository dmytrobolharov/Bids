/****** Object:  View [dbo].[vwx_SourcingStyle_QuoteItem_SELECT]    Script Date: 01/25/2013 15:56:44 ******/
...
/****** Object:  View [dbo].[vwx_SourcingStyle_QuoteItem_SELECT]    Script Date: 01/25/2013 15:56:45 ******/
SET ANSI_NULLS ON
GO
...
AS
SELECT DISTINCT 
                      '<img src=''../System/Icons/' + dbo.pStyleQuoteItemStatus.CustomIcon + ''' />' AS ImagePath, dbo.pStyleQuoteItem.StyleQuoteItemID, dbo.pStyleQuoteItem.StyleID, 
                      dbo.pStyleQuoteItem.StyleQuoteItemNo, dbo.pStyleQuoteItem.StyleQuoteItemShare, dbo.pStyleQuoteItem.StyleQuoteItemStatusId, 
                      ...
                      dbo.pStyleSeasonYear.SeasonYearID, dbo.pStyleSeasonYear.StyleSeason + ' ' + dbo.pStyleSeasonYear.StyleYear AS StyleSeasonYear, p.AgentID, p.AgentName, 
                      p.VendorID, p.VendorName, p.FactoryID, p.FactoryName,
                          (SELECT     SUM(ISNULL(PartnerTotal, '0.00')) AS Expr1
                            ...
                            WHERE      (StyleQuoteItemID = dbo.pStyleQuoteItem.StyleQuoteItemID)) AS PartnerCost,
                          (SELECT     SUM(ISNULL(PartnerTotal, '0.00')) AS Expr1
                            FROM          dbo.pSourcingQuotationBOMDetails AS pSourcingQuotationBOMDetails_1
                            WHERE      (StyleQuoteItemID = dbo.pStyleQuoteItem.StyleQuoteItemID)) + (ISNULL(dbo.pStyleQuoteItem.StyleQuoteItemCustomField8, '0.00') 
                      + ISNULL(dbo.pStyleQuoteItem.StyleQuoteItemCustomField10, '0.00') + ISNULL(dbo.pStyleQuoteItem.StyleQuoteItemCustomField12, '0.00') 
                      + ISNULL(dbo.pStyleQuoteItem.StyleQuoteItemCustomField13, '0.00')) + ISNULL
                          ((SELECT     SUM(ISNULL(Amount, '0.00')) AS Expr1
                              FROM         dbo.pSourcingAdditionalCost
                              WHERE     (QuoteItemID = dbo.pStyleQuoteItem.StyleQuoteItemID)), '0.00') AS PartnerExtendedCost, ISNULL
                          ((SELECT     SUM(ISNULL(Amount, '0.00')) AS Expr1
                              FROM         dbo.pSourcingAdditionalCost AS pSourcingAdditionalCost_1
                              WHERE     (QuoteItemID = dbo.pStyleQuoteItem.StyleQuoteItemID)), '0.00') AS AdditionalCost, ISNULL(dbo.pStyleDevelopmentItem.StyleDevelopmentName, 
                      'Variation ' + CAST(dbo.pStyleDevelopmentItem.Variation AS VARCHAR(5))) AS VariationName, CASE WHEN EXISTS
                          (SELECT     *
                            FROM          pStyleQuoteItemShare qis
                            WHERE      qis.StyleQouteItemID = pStyleQuoteItem.StyleQuoteItemID AND qis.TradePartnerID IN (p.AgentID, p.VendorID, p.FactoryID)) 
                      THEN 1 ELSE 0 END AS IsShared, dbo.pSourcingHeader.SourcingName, dbo.pSourcingHeader.SourcingType
FROM         dbo.pStyleQuoteItem INNER JOIN
                      dbo.pSourcingHeader ON dbo.pSourcingHeader.SourcingHeaderID = dbo.pStyleQuoteItem.SourcingHeaderID INNER JOIN
                      dbo.pSourcingTradePartner AS stp ON stp.SourcingTradePartnerID = dbo.pStyleQuoteItem.StyleQuoteTradePartnerID LEFT OUTER JOIN
                      ...
                      dbo.pStyleDevelopmentItem ON dbo.pStyleQuoteItem.StyleID = dbo.pStyleDevelopmentItem.StyleID LEFT OUTER JOIN
                      ...
                      dbo.pStyleSeasonYear ON dbo.pStyleSeasonYear.StyleSeasonYearID = dbo.pStyleQuoteItem.StyleSeasonYearID LEFT OUTER JOIN
...
VALUES ('DB_Version', '5.0.0000', '04899', GetDate())
GO
