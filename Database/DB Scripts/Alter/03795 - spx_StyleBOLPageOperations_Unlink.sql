IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_StyleBOLPageOperations_Unlink]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_StyleBOLPageOperations_Unlink]
GO
...
BEGIN
	SELECT @OperationId = OperationId, @MacroId = MacroId, @StyleID = StyleID, @StyleSet = StyleSet, @IsException = IsException, @IsLinked = IsLinked FROM pStyleBOLPageOperations WITH (NOLOCK) WHERE PageOperationId = @PageOperationId
	
	IF (@OperationId <> '') AND @IsLinked = 1
	BEGIN
...
VALUES ('DB_Version', '5.0.0000', '03795', GetDate())
GO
