...
/****** Object:  StoredProcedure [dbo].[spx_BodyPatternSheetModel_GET_UPDATE]    Script Date: 10/06/2010 12:33:20 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF EXISTS (SELECT * FROM sysobjects WHERE id = object_id('spx_BodyPatternSheetModel_GET_UPDATE') and sysstat & 0xf = 4)
    DROP PROCEDURE spx_BodyPatternSheetModel_GET_UPDATE
GO

CREATE PROC  [dbo].[spx_BodyPatternSheetModel_GET_UPDATE]
(
	@CADDB varchar(50),
	@Catalog varchar(50),
...
FROM pBodyPatternSheetModel where BodyID = @BodyID and IsLinked = 1
...
Where pBodyPatternSheetModel.Model_Code = (select Model_Code from #tempTblBody where rownum = @RowCnt)
       AND pBodyPatternSheetModel.BodyID = (select BodyID from #tempTblBody where rownum = @RowCnt)
    Select @RowCnt = @RowCnt + 1

EXEC [spx_BodyPatternSheetPiece_GET_UPDATE]  @Model_Code,@CADDB,@Catalog
...
select @Sortsql = 'SELECT 	[ID] ,
	...
	[NotchFile],
	[AnnotationFile],
	[StorageType] FROM pBodyPatternSheetModel   WHERE (BodyID = ' + CHAR(39) + Convert(varchar(36),@BodyID) + CHAR(39) + ' and Active = 1 )' +'order by ' + @strSortOrder 
...
VALUES     ('DB_Version', '4.0.0000', '596', GetDate())
GO
