IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[dpx_Material_Availability_Tracking_SELECT]') AND type in (N'P', N'PC'))
...
BEGIN
    ...
    SELECT 
        ...
        , CASE 
            ...
            WHEN DATEDIFF(DAY, GETDATE(), MAX(mrs.DueDate)) < 0 THEN CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(mrs.DueDate))) AS VARCHAR(5)) + 'd Late'
            ELSE CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(mrs.DueDate))) AS VARCHAR(5)) + 'd Remain'
          END AS DaysMessage    
        , CASE 
    ...
    INNER JOIN @tmpMaterialPartner tmp ON mtp.TradePartnerRelationshipLevelID = tmp.MaterialPartnerID
    GROUP BY 
          tm.MaterialID
        , tm.MaterialNo
    ...
    SELECT 
        ...
        , CASE 
            ...
            WHEN DATEDIFF(DAY, GETDATE(), MAX(srw.DueDate)) < 0 THEN CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(srw.DueDate))) AS VARCHAR(5)) + 'd Late'
            ELSE CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(srw.DueDate))) AS VARCHAR(5)) + 'd Remain'
          END AS DaysMessage    
        , CASE 
    ...
    INNER JOIN pSampleRequestBOMTrade srt ON sb.WorkflowItemId = srt.WorkFlowItemID AND srt.StyleSeasonYearID = ssy.StyleSeasonYearID
    ...
    INNER JOIN pSampleRequestWorkflowBOM srw ON srt.SampleRequestTradeID = srw.SampleRequestTradeID
    INNER JOIN @tmpStylePartner tsp ON srt.TradePartnerRelationshipLevelID = tsp.StylePartnerID
    GROUP BY 
          tm.MaterialID
        , tm.MaterialNo
        , tm.ComponentDescription
...
VALUES     ('DB_Version', '0.5.0000', '05568', GetDate())
GO
