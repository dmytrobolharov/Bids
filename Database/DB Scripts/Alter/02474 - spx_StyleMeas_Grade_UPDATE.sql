	...
	if @Incr <> 0 
		BEGIN
			if @IsLength <> 0 AND @ShrinkX <> 0
				BEGIN
					set @BeforeTreat = @ImperialIncr * (1-((@ShrinkX * 2.54)/100))
					set @Changed = 1
				END
			
			if @IsWidth <> 0 AND @ShrinkY <> 0
				BEGIN
					set @BeforeTreat = @ImperialIncr * (1-((@ShrinkY * 2.54)/100))
					set @Changed = 1
				END
			...
			if @Changed = 1 
				BEGIN	
					--Stored in the database as imperial 
...
IF(NOT EXISTS(SELECT * FROM sVersion WHERE LastScriptRun='02474'))
BEGIN

	INSERT INTO sVersion(AppName, Version, LastScriptRun, TimeStamp)
	VALUES     ('DB_Version', '4.1.0000', '02474', GetDate())
