...
IF EXISTS (select * from sysobjects where id = object_id('spx_StyleMeasRefreshExistingPOM') and sysstat & 0xf = 4)
    drop procedure spx_StyleMeasRefreshExistingPOM
GO


CREATE PROCEDURE spx_StyleMeasRefreshExistingPOM
(
...
Select @oldSampleCol = SizeCount from pGRSizeRangeDetail where ClassRangeId = @SizeRangeID and [Sample]=1
...
If @MaxPOMRow > 0
	...
	begin
		...
		Refcode , TolPlus, TolMinus, How2MeasText, SortField, IsOnQA,  ImageId, WSRowId, POM_Row,
		...
		v.TolPlus, v.TolMinus, v.How2MeasText, v.SortField, v.IsOnQA,  v.ImageId, t.WSRowId, t.Pom_Row, ' + @IncrNullCols + '			 
		...
		if @RowCounter>0	
			...
			begin								
				...
				SortId = t.SortField, How2MeasText = t.How2MeasText, ImageId = t.ImageId, Flag4QA = t.IsOnQA, Tolplus = t.Tolplus ,
				...
				and p.AMLMeasPomId=t.AMLMeasPomId and p.refcode=t.refcode	and  p.POM_Row=t.POM_Row where  p.amlmeashdrid=@AMLMeasHdrId  and p.pom_row <= @MaxPOMRow
...
VALUES ('DB_Version', '4.2.0000', '03022', GetDate())
GO
