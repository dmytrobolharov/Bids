/****** Object:  Trigger [tgx_pPlanningFlashEdit_PermissionMapping_INSERT]    Script Date: 06/27/2014 15:36:20 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[tgx_pPlanningFlashEdit_PermissionMapping_INSERT]'))
DROP TRIGGER [dbo].[tgx_pPlanningFlashEdit_PermissionMapping_INSERT]
GO

/****** Object:  Trigger [dbo].[tgx_pPlanningFlashEdit_PermissionMapping_INSERT]    Script Date: 06/27/2014 15:36:20 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[tgx_pPlanningFlashEdit_PermissionMapping_INSERT]
...
BEGIN
    ...
    BEGIN TRY
		...
		DECLARE @ParentWorkflowID UNIQUEIDENTIFIER = '00000000-0000-0000-0000-000000000006' -- Should be under Flash Edit in permissions
		...
		FOR SELECT PlanningFlashEditId, PlanningFlashEditName, 
		...
		BEGIN
			-- Adding new workflow to each Product Type in Planning Folder
...
VALUES ('DB_Version', '0.5.0000', '08135', GetDate())
GO