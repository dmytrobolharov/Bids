...
SELECT sh.StyleID, sh.StyleMasterID, sh.StyleWorkflowID, sh.StyleType, sh.WorkflowType, sh.RefNo, sh.StyleNo, 
       ...
       COALESCE(sr.SizeRangeCode, sh.SizeRange) AS SizeRange, COALESCE(scc.Custom, sh.SizeClass) AS SizeClass, sh.StyleCategory, sh.Description, 
       sh.SpecNo, sh.ConceptNo, sh.ConceptID, sh.DevelopmentNo, sh.DevelopmentID, sh.CustomerNo, sh.TempNo, sh.TempID, ssy.SeasonYearID, sy.Season, 
       sy.Year, ISNULL(sdi.StyleDevelopmentName, 'Variation ' + CAST(sdi.Variation AS nVarchar(2))) AS Variation, 
...
VALUES ('DB_Version', '6.1.0000', '09470', GetDate())
GO
