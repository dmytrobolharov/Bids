/****** Object:  StoredProcedure [dbo].[spx_Sourcing_StyleBOMDimItem_select]    Script Date: 11/12/2012 11:50:41 ******/
	...
	if @TotalCnt > 0
		begin											
			if @DimLevel=1
				begin
					INSERT INTO #tmpDimItem (DimID, DimName, Sort)
						SELECT cast(a.ItemDim1Id as nvarchar(50)) + @Active + cast(a.ItemDim1Active as nvarchar(5)), a.ItemDim1Name, cast(a.ItemDim1Sort as nvarchar(5)) FROM pSourcingQuoteStyleBOM a, pStyleBOMDimensionItem b  where a.StyleBOMDimensionId = @StyleBOMDimensionId 
						and b.ItemDim1Active = 1 and a.StyleBOMDimensionItemID = b.StyleBOMDimensionItemID
						and a.StyleQuoteItemId=@StyleQuoteItemId	 and a.ItemDim1Id is not null order by a.ItemDim1Sort, ItemDim1Name
			...
			if @DimLevel=2
				begin
					INSERT INTO #tmpDimItem (DimID, DimName, Sort)
					SELECT  cast(a.ItemDim2Id as nvarchar(50)) + @Active + cast(a.ItemDim2Active as nvarchar(5)), a.ItemDim2Name, cast(a.ItemDim2Sort as nvarchar(5)) FROM pSourcingQuoteStyleBOM a, pStyleBOMDimensionItem b  where a.StyleBOMDimensionId = @StyleBOMDimensionId
					and a.StyleQuoteItemId=@StyleQuoteItemId	
			...
			if @DimLevel=3
				begin
					INSERT INTO #tmpDimItem (DimID, DimName, Sort)
					SELECT  cast(a.ItemDim3Id as nvarchar(50)) + @Active + cast(a.ItemDim3Active as nvarchar(5)) , a.ItemDim3Name, cast(a.ItemDim3Sort as nvarchar(5)) FROM pSourcingQuoteStyleBOM a, pStyleBOMDimensionItem b  where a.StyleBOMDimensionId = @StyleBOMDimensionId
					and a.StyleQuoteItemId=@StyleQuoteItemId	
...
VALUES     ('DB_Version', '5.0.0000', '04326', GetDate())
