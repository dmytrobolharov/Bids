/****** Object:  StoredProcedure [dbo].[spx_BatchQGrid_MaterialReplace_Style_SELECT]    Script Date: 24.02.2015 11:45:54 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_BatchQGrid_MaterialReplace_Style_SELECT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_BatchQGrid_MaterialReplace_Style_SELECT]
GO
...
CREATE PROCEDURE [dbo].[spx_BatchQGrid_MaterialReplace_Style_SELECT] (
...
BEGIN 
	...
	WHERE c.BatchQueuePagePki IS NULL 
		...
		AND foo.StyleID NOT IN (
			SELECT psm2.StyleID
			from pStyleMaterials psm2
...
VALUES ('DB_Version', '6.1.0000', '09734', GetDate())
GO
