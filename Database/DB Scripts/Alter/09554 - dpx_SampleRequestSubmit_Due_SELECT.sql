IF OBJECT_ID(N'[dbo].[dpx_SampleRequestSubmit_Due_SELECT]') IS NOT NULL
 DROP PROCEDURE [dbo].[dpx_SampleRequestSubmit_Due_SELECT]
GO

CREATE PROCEDURE [dbo].[dpx_SampleRequestSubmit_Due_SELECT]
(
	@Flag int
)
AS
...
/* 3 = Sample workflows due this week.										*/
/* 5 = Sample workflows due this month. 									*/
/* 7 = Sample workflows due this quarter. 									*/
/* 10 = Sample workflows due this year.										*/
/****************************************************************************/
...
IF(@Flag = 1)	--Sample workflows due today.
	BEGIN
		...
		SELECT Users.FirstName + ' ' + Users.LastName AS AssignedTo, sh.StyleNo, 
		       COALESCE(sc.Custom, sh.SizeClass) AS SizeClass, 
			   COALESCE(sr.SizeRangeCode, sh.SizeRange) AS SizeRange, 
			pSampleWorkflow.GroupName, pSampleRequestSubmitStatus.Status, pSampleRequestWorkflow.DueDate, 
			pStyleSeasonYear.StyleSeason, pStyleSeasonYear.StyleYear, uTradePartner.TradePartnerName, uTradePartnerVendor.VendorName, 
			pStyleColorway.StyleColorNo, pStyleColorway.StyleColorName, pStyleColorway.StyleColorStandardID, sh.StyleID
		FROM pSampleRequestWorkflow INNER JOIN
			pStyleHeader sh ON pSampleRequestWorkflow.StyleID = sh.StyleID INNER JOIN
			pSampleRequestTrade ON 
			pSampleRequestWorkflow.SampleRequestTradeID = pSampleRequestTrade.SampleRequestTradeID INNER JOIN
			uTradePartnerVendor ON pSampleRequestTrade.TradePartnerVendorID = uTradePartnerVendor.TradePartnerVendorID INNER JOIN
			uTradePartner ON uTradePartnerVendor.TradePartnerID = uTradePartner.TradePartnerID LEFT OUTER JOIN
			pStyleColorway ON pSampleRequestWorkflow.StyleColorID = pStyleColorway.StyleColorID LEFT OUTER JOIN
			pStyleSeasonYear ON pSampleRequestTrade.StyleSeasonYearID = pStyleSeasonYear.StyleSeasonYearID LEFT OUTER JOIN
			pSampleRequestSubmitStatus ON pSampleRequestWorkflow.Status = pSampleRequestSubmitStatus.StatusID LEFT OUTER JOIN
			pSampleWorkflow ON pSampleRequestWorkflow.SampleWorkflowID = pSampleWorkflow.SampleWorkflowID LEFT OUTER JOIN
			Users ON pSampleRequestWorkflow.AssignedTo = Users.UserId
	...
	END
ELSE IF(@Flag = 2)	--Sample workflows due yesterday.
	BEGIN
		...
		ORDER BY
			...
			pSampleWorkflow.GroupName
	END
ELSE IF(@Flag = 3)	--Sample workflows due this week.
	BEGIN
		...
		ORDER BY
			...
			pSampleWorkflow.GroupName
	END
ELSE IF(@Flag = 5)	--Sample workflows due this month.
	BEGIN
		...
		ORDER BY
			...
			pSampleWorkflow.GroupName
	END
ELSE IF(@Flag = 7)	--Sample workflows due this quarter.
	BEGIN
		--Get the integer equivalent of the month and the year.
		SET @Year = DATEPART(yyyy, @DateTime)
		SET @Month = DATEPART(mm, @DateTime)
		...
		ORDER BY
			...
			pSampleWorkflow.GroupName
	END
ELSE IF(@Flag = 10)	--Sample workflows due this year.
	BEGIN
...
VALUES ('DB_Version', '6.1.0000', '09554', GetDate())
GO
