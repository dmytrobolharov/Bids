...
SELECT 
	  CASE WHEN SUM(CASE WHEN cmiws.WorkflowStatusID = tms.S THEN 1 ELSE NULL END) > 0 THEN 1 ELSE 0 END AS IsApproved
	, MAX (tms.StaticDueDate) AS DueDate
	, tms.MaterialID, cmi.CalendarMilestoneItemID, cmi.CalendarMilestoneItemName, cmi.MilestoneDate, cmi.SortOrder
...
GROUP BY cmi.CalendarMilestoneItemID, tms.MaterialID, cmi.CalendarMilestoneItemName, cmi.MilestoneDate, cmi.SortOrder
...
SELECT	
	...
	, cp.ColorCode
	, cp.ColorName
	, dbo.fnx_GetStreamingColorImagePath(cp.ColorFolderID, cp.ColorPaletteID, '100') AS ColorPath
	, mrs.MaterialRequestSubmitID
...
INNER JOIN (
	SELECT mrs1.MaterialTradePartnerColorID, mrs1.MaterialRequestSubmitID, mrs1.DueDate, mrs1.MaterialRequestSubmitWorkflowID 
	FROM pMaterialRequestSubmit mrs1
	LEFT OUTER JOIN pMaterialRequestSubmit mrs2 
		ON mrs1.MaterialTradePartnerColorID = mrs2.MaterialTradePartnerColorID 
		AND mrs1.MaterialRequestSubmitWorkflowID = mrs2.MaterialRequestSubmitWorkflowID 
		AND mrs1.Submit < mrs2.Submit
	WHERE mrs2.MaterialRequestSubmitID IS NULL
) mrs ON mrsw.MaterialRequestSubmitWorkflowID = mrs.MaterialRequestSubmitWorkflowID	
...
LEFT JOIN pMaterialTradePartnerColor mtpc
	INNER JOIN pColorPalette cp ON mtpc.ColorPaletteID = cp.ColorPaletteID
	ON mrs.MaterialTradePartnerColorID = mtpc.MaterialTradePartnerColorID
...
SELECT
	...
	, tmr.MaterialID, tmr.CalendarMilestoneItemID, tmr.CalendarMilestoneItemName, tmr.MilestoneDate, tmr.SortOrder
...
FROM (
	...
	GROUP BY mrw.MaterialRequestWorkflowRepID, tms.MaterialID, cmi.CalendarMilestoneItemID, cmi.CalendarMilestoneItemName, cmi.MilestoneDate, cmi.SortOrder) tmr
GROUP BY tmr.MaterialID, tmr.CalendarMilestoneItemID, tmr.CalendarMilestoneItemName, tmr.MilestoneDate, tmr.SortOrder
...
SELECT 
	...
	, NULL AS ColorName	
	, NULL AS ColorPath	
	, NULL AS MaterialRequestSubmitID
	...
	, tmmw.SortOrder AS GroupSort
	, tmmw.MilestoneDate
	, CASE WHEN COUNT(CASE WHEN tmmw.IsApproved = 1 OR tmmw.IsApproved IS NULL THEN NULL ELSE 1 END) = 0 THEN 1 ELSE 0 END AS IsComplete
	, CASE 
		WHEN COUNT(CASE WHEN tmmw.IsApproved = 1 OR tmmw.IsApproved IS NULL THEN NULL ELSE 1 END) = 0 THEN 'Complete'
		WHEN DATEDIFF(DAY, GETDATE(), tmmw.MilestoneDate) < 0 THEN CAST(ABS(DATEDIFF(DAY, GETDATE(), tmmw.MilestoneDate)) AS nVARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, GETDATE(), tmmw.MilestoneDate)) AS nVARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
		WHEN COUNT(CASE WHEN tmmw.IsApproved = 1 OR tmmw.IsApproved IS NULL THEN NULL ELSE 1 END) = 0 THEN 'LightGreen'
		...
		WHEN MAX(tmmw.MilestoneDate) IS NOT NULL THEN '#78b4ff'
		ELSE 'None'
	  END AS Color
FROM #tmpMatStatic tms
...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, GETDATE(), tmmr.MilestoneDate) < 0 THEN CAST(ABS(DATEDIFF(DAY, GETDATE(), tmmr.MilestoneDate)) AS nVARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, GETDATE(), tmmr.MilestoneDate)) AS nVARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	, NULL AS DueDate
	, CASE 
		...
		WHEN MAX(tmmr.MilestoneDate) IS NOT NULL THEN '#78b4ff'
		ELSE 'None'
	  END AS Color
FROM #tmpMatStatic tms
...
SELECT 
	...
	, CASE 
		...
		WHEN DATEDIFF(DAY, GETDATE(), MAX(tmw.StaticDueDate)) < 0 THEN CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(tmw.StaticDueDate))) AS nVARCHAR(5)) + 'd Late'
		ELSE CAST(ABS(DATEDIFF(DAY, GETDATE(), MAX(tmw.StaticDueDate))) AS nVARCHAR(5)) + 'd Remain'
	  END AS DaysMessage
	...
	, CASE 
		...
		WHEN MAX(tmw.StaticDueDate) IS NOT NULL THEN '#78b4ff'
		ELSE 'None'
	  END AS Color
FROM #tmpMatStatic tms
...
VALUES ('DB_Version', '6.1.0000', '10095', GetUTCDate())
GO