IF OBJECT_ID(N'[dbo].[spx_StyleBOMColorways_Insert]') IS NOT NULL
...
AS 
	...
	DECLARE @UpdateWorkflowStatus INTEGER = 0
	...
	SELECT @SeasonYearID = ssy.SeasonYearID, @StyleSeasonYearID = ssy.StyleSeasonYearID  
	...
	begin											
		...
		BEGIN
			...
			SET WorkStatus = 6, ActualStart = @CDate
			WHERE StyleId = @StyleID AND WorkFlowID = @WorkflowID AND 
				  StyleSet = @StyleSet AND StyleSeasonYearID = @StyleSeasonYearID AND 
				  WorkStatus = 1
		END	    
...
VALUES ('DB_Version', '6.1.0000', '09300', GetDate())
GO
