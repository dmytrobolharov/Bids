...
BEGIN
	...
	SELECT
		...
		, COALESCE(@ImageServer + '/ImageStream.ashx?IT=MAMAGIC&S=600&ID=' + CAST(fpc.FitPhotoCommentId AS NVARCHAR(40)), @ImageServer + '/ImageStream.ashx?IT=MAMAGI&S=600&ID=' + CAST(srfp.FitPhotoId AS NVARCHAR(40))) AS FilePath
	...
	FROM pSampleRequestSubmitBOM srs
		...
		INNER JOIN pSampleRequestSubmitFitPhotos srfp ON srs.SampleRequestTradeID = srfp.SampleRequestTradeID
			AND srs.SampleRequestWorkflowID = srfp.SampleRequestWorkflowID
			AND srs.Submit = srfp.Submit
			AND srs.StyleSet = srfp.StyleSet
		LEFT JOIN pSampleRequestSubmitFitPhotosComments fpc ON srfp.FitPhotoId = fpc.FitPhotoId
...
VALUES ('DB_Version', '6.1.0000', '09878', GetDate())
GO
