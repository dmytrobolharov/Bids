IF OBJECT_ID(N'[dbo].[spx_StyleMeasGRMeasurementsPOM_Insert]') IS NOT NULL
 DROP PROCEDURE [dbo].[spx_StyleMeasGRMeasurementsPOM_Insert]
GO

/****** Object:  StoredProcedure [dbo].[spx_StyleMeasGRMeasurementsPOM_Insert]    Script Date: 01/16/2015 17:49:49 ******/
SET ANSI_NULLS ON
GO
...
SELECT @WashType = sh.WashType FROM pAMLMeasHdr mh 
LEFT JOIN pStyleHeader sh ON mh.StyleId = sh.StyleID 
...
SELECT @AMLMeasHdrId, POMTempItemID, POMId, POMAlternatesId, POMcode, (
		...
		CASE WHEN @WashType = 'NONWASH' THEN NonWashTolPlus ELSE TolPlus END AS TolPlus,
		CASE WHEN @WashType = 'NONWASH' THEN 
			CASE left(NonWashTolMinus, 1)
			WHEN '-'
				THEN NonWashTolMinus
			ELSE convert(DECIMAL(18, 4), ('-' + cast(NonWashTolMinus AS NVARCHAR(20))))
		END	ELSE CASE left(TolMinus, 1)
			WHEN '-'
				THEN TolMinus
			ELSE convert(DECIMAL(18, 4), ('-' + cast(TolMinus AS NVARCHAR(20))))
		END	END	AS TolMinus, 
		How2MeasName, How2MeasText, ImageId, 0, 0, IsOnQA, Critical, IsLinked, COALESCE(cast(pomSort AS INT), 0), @CUser
	, @CDate, @CUser, @CDate
FROM pGRMeasurementsPOMTemplateItem WITH (NOLOCK)
WHERE POMTempID = @POMTempID AND bKey = 1
AND (@OldAMLMeasHdrId IS NULL OR 
	(@OldAMLMeasHdrId IS NOT NULL AND POMCode IS NOT NULL AND POMCode <> ''
		AND POMCode IN (SELECT RefCode FROM pAMLMeasPOM WHERE AMLMeasHdrId = @OldAMLMeasHdrId AND RefCode IS NOT NULL AND RefCode <> '' ))) -- WI#22831
...
VALUES ('DB_Version', '6.1.0000', '09255', GetDate())
GO