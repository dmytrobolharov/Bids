/****** Object:  StoredProcedure [dbo].[spx_NPMSampleRequestSilhoDataTable_PIVOT]    Script Date: 07/01/2013 12:21:55 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spx_NPMSampleRequestSilhoDataTable_PIVOT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[spx_NPMSampleRequestSilhoDataTable_PIVOT]
GO
...
CREATE PROCEDURE [dbo].[spx_NPMSampleRequestSilhoDataTable_PIVOT](
...
b.TradePartnerID,  COALESCE(b.TradePartnerVendorID,'00000000-0000-0000-0000-000000000000') ,
...
TradePartnerID, TradePartnerVendorID , TradePartnerName, AgentID, AgentName, VendorID, VendorName, FactoryID, FactoryName,
...
a.TradePartnerID, a.TradePartnerVendorID,  b.TradePartnerName, tpm.AgentID, tpm.AgentName, tpm.VendorID, tpm.VendorName, tpm.FactoryID, tpm.FactoryName,
...
Select @Vcnt=COUNT(*) from #tmpSample3 where (TradePartnerVendorID<>'00000000-0000-0000-0000-000000000000')
begin
	If @Vcnt>0
		begin
			update #tmpSample3 set AgentName=a.TradePartnerName , VendorName=c.vendorname
			from #tmpSample3 a
			INNER JOIN uTradePartner b ON b.TradePartnerID = a.TradePartnerID
			INNER JOIN uTradePartnerVendor c ON c.TradePartnerVendorID = a.TradePartnerVendorID 
			and c.TradePartnerID = b.TradePartnerID
			where ( a.TradePartnerVendorID<>'00000000-0000-0000-0000-000000000000')
			
			update #tmpSample3 set AgentID=c.AgentID,VendorID=c.VendorID from #tmpSample3 a
			INNER JOIN uTradePartner b ON b.TradePartnerID = a.TradePartnerID
			INNER JOIN vwx_TradePartnerRelationshipLevels_SEL c ON 
			Upper(a.agentname)=Upper(c.agentname) and Upper(a.vendorname)=Upper(c.vendorname)  and c.FactoryName is null
			where ( a.TradePartnerVendorID<>'00000000-0000-0000-0000-000000000000')
			
			Update pSampleRequestTrade set TradePartnerRelationshipLevelID=c.TradePartnerRelationshipLevelID
			from pSampleRequestTrade a INNER JOIN #tmpSample3 b ON a.SampleRequestTradeID = b.SampleRequestTradeID
			and a.TradePartnerID = b.TradePartnerID and a.TradePartnerVendorID=b.TradePartnerVendorID
			INNER JOIN vwx_TradePartnerRelationshipLevels_SEL c ON b.AgentID=c.AgentID and  b.VendorID=c.VendorID
			and  c.FactoryID is null
			where (a.TradePartnerVendorID<>'00000000-0000-0000-0000-000000000000')
...
VALUES ('DB_Version', '0.5.0000', '05937', GetDate())
GO
